...
| 규칙 | 검증 방법 |
|------|----------|
| 시가 대비 +29.5% 매수 | `strategy.py`에서 29.5 리터럴 검색, 계산식 확인 |
| 투자대금 25% 비중 | 매수 수량 계산 로직에서 0.25 적용 확인 |
| 중복 매수 차단 | 미체결/보유 종목 체크 로직 존재 확인 |
| 매수가 대비 -7.5% 손절 | 손절 조건식에서 -7.5 리터럴 확인, 체결가 기준 확인 |
| 익일 09:00 청산 | 스케줄러 09:00 트리거, 갭상승 +10% 분기 확인 |
| 트레일링 스탑 -2% | 고점 추적 로직, -2% 하락 시 매도 트리거 확인 |

### 5. 주문 안전성 시나리오

```markdown
## 정상 흐름
1. 종목 스캔 → 필터 통과 → WS 구독 → 시가+29.5% 감지
2. 매수가능금액 확인 → 25% 비중 계산 → 시장가 매수
3. 체결통보 수신 → trade_history INSERT(COMPLETED)
4. 잔고 반영 → 프론트 갱신
...
## 손절 트리거
1. 보유 중 현재가 하락 → 매수가 대비 -7.5% 도달
2. 즉시 시장가 매도 → 체결 → trade_history UPDATE
3. 매도 실패 시 → 재시도(최대 3회) → 실패 시 system_logs 기록 + team-leader 알림

## 익일 갭상승 청산
1. 09:00 시가 확인 → 매수가 대비 +12% (≥10%)
2. 트레일링 스탑 가동 → 고점 추적
3. 고점 대비 -2% 하락 → 시장가 매도
