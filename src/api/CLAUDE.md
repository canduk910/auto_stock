...
- **사이클 145 (2026-06-16) — `inquire_stock_basics` FHKST01010100 raw 0 덮어쓰기 금지 graceful (결함 #2, HIGH 매매 안전성 직결)**: 운영 사례 영구 영속 (Supabase MCP 진단) = 2026-06-16 07:54~07:59 KST = boot `force=True` 영역 영구 영속 (장 시작 *전*) 시점 + stock_master 2,697 ticker 중 `acml_tr_pbmn > 0` = **3 종목만** 잔존 silent 결함. 근본 원인 = FHKST01010100 응답 영역 영구 영속 `acml_tr_pbmn=0` + `acml_vol=0` 정상 (장 시작 전 거래 없음) → `inquire_stock_basics` merge 영역 영구 영속에서 기존 raw 영역 영구 영속 덮어쓰기 → `stock_master.list_by_filter(acml_tr_pbmn ≥ min_trade_amount=20_000_000_000)` 0건 silent → donchian/VB/LTV 후보 0 결함. **시정 (+20L net)**: merge 영역 영구 영속에서 `("acml_tr_pbmn", "acml_vol")` 키 영역 영구 영속 = `int(str(value).replace(",", "") or 0) == 0` 가드 → 0 값 영역 영구 영속 `continue` skip → 기존 raw 키 영역 영구 영속 보존 (전일 영업일 거래대금 보존). `("lstn_stcn", "prdy_vrss", "hts_avls")` 영역 = 변경 0 (정상 덮어쓰기). 비숫자 영역 = `except (ValueError, TypeError)` graceful → 정상 영역 영구 영속 그대로 merge. **사이클 81 G-AST1 영구 강화** (raw 덮어쓰기 금지 영역 영구 영속) + 사이클 88 G-REJECT graceful 영속. **회귀 가드 4 케이스 (`tests/unit/api/test_cycle145_raw_preserve.py`)**: G-145-RAW-1 (acml_tr_pbmn=0 응답 → 기존 raw 키 영역 영구 영속 보존 + 다른 정상 키 영역 영구 영속 정상 merge `hts_avls` / `lstn_stcn`) + G-145-RAW-2 (acml_tr_pbmn>0 정상 응답 → 정상 덮어쓰기) + G-145-RAW-3 (acml_vol=0 시 동일 영역 영구 영속 보존) + G-145-RAW-4 (AST 정적 가드 영역 영구 영속 = `numeric_value == 0` 영역 영구 영속). **영속 의무 매트릭스**: 사이클 81 G-AST1 영구 영속 강화 + 사이클 88 G-REJECT graceful 영속 + 사이클 107 CTPF1002R + FHKST01010100 merge 패턴 영속 (변경 0 영역 = 0 가드 영역 영구 영속만 추가) + 사이클 108 list_by_filter (`acml_tr_pbmn ≥ min_trade_amount`) 정합 영속 + 사이클 122~144 영속 (영향 0). **매매 안전성 직접 검증**: scanner 매수 진입 *전* 영역 한정 (사이클 38 명문화 영속) + stock_master 영역 영구 영속 raw 보존 → list_by_filter 정상 작동 → donchian/VB/LTV 후보 영구 영속 → 매매 안전성 영향 0 (기존 raw 보존 영역 영구 영속). **D+1 운영 효과 (2026-06-17~)**: 16:30 `_stock_master_master_load_once` 자동 발화 + 16:10 `_stock_master_basics_refresh_once` 자동 발화 영역 영구 영속 = 거래대금 정상 영역 영구 영속 적재 → list_by_filter 정상 작동 → donchian/VB/LTV `prepare()` 호출 시 유의미한 후보 영역 영구 영속.
- **사이클 144 (2026-06-16) — `inquire_stock_basics` graceful 분기 가시화 강화 (카드 #27 LOW)**: 사이클 130 권고 카드 #27 (NEW LOW, 사이클 129/132 인계) 영역 종결. **현행 silent fail 영역 영구 영속**: 사이클 107 도입 FHKST01010100 호출 실패 영역이 `try/except: logger.exception` + `price_data = {}` graceful fallback 만 영역 영구 영속 = scanner `_stock_master_basics_refresh_once` summary 영역 영구 영속 가시화 불가 (사이클 129 OPSQ1002 SESSION FULL 영역 graceful 처리 영역 영구 영속 silent). **시정 (production +53L)**: `condition.py` 모듈 전역 `_graceful_failed_counter: dict[str, int]` 영역 영구 영속 신규 + `_graceful_failed_lock: asyncio.Lock` (FastAPI 동시 요청 + BackgroundTasks 영역 영구 영속 동시성 보호) + 3 헬퍼 영역 영구 영속 (`_record_graceful_failed(reason: str)` 비동기 카운터 증가 / `get_graceful_failed_counts() -> dict` 스냅샷 사본 반환 / `reset_graceful_failed_counts()` 초기화). `inquire_stock_basics` FHKST01010100 except 분기 영역 영구 영속에 `_record_graceful_failed("fhkst01010100_failed")` 호출 신규 (try/except 영역 영구 영속 외부 영향 0 영구 영속). **scanner summary 영역 영구 영속 +21L**: `_stock_master_basics_refresh_once` 시작 시 `reset_graceful_failed_counts()` 호출 영역 영구 영속 (단일 task 영역 영구 영속 측정 의무) + 종료 시 `get_graceful_failed_counts()` 수집 + `summary["graceful_failed"]` 키 영역 영구 영속 ("fhkst01010100_failed" 카운터) + emit 로그 `[stock_master_basics_refresh_summary] ... graceful_failed_fhkst=%d ...` 필드 추가 영역 영구 영속. **회귀 가드 10 케이스 (`tests/unit/api/test_cycle144_graceful_visibility.py`)**: G-144-COUNTER-1~4 (모듈 전역 dict + 카운터 증가 + snapshot + reset 영속) + G-144-INT-1~3 (FHKST01010100 RuntimeError → 카운터 +1 + 양쪽 정상 → 변경 0 + KisApiError "OPSQ1002 SESSION FULL" → 카운터 +1, 사이클 129 영역 영구 영속 가시화 직접 검증) + G-144-SUMMARY-1~2 (summary `graceful_failed` 키 + emit `graceful_failed_fhkst=N` 필드) + G-144-AST-1 (inquire_stock_basics graceful 분기 `_record_graceful_failed` 호출 ≥ 1건 AST 정적 가드, 미래 동일 영역 silent 결함 영구 차단). **영속 의무 매트릭스**: 사이클 17 KIS LMS chain (변경 0) + 사이클 38 명문화 (logging 영역 한정) + 사이클 79 G-AST2 / 81 G-AST1 (영향 0) + 사이클 88 G-REJECT graceful 영속 (영역 강화 = silent → 가시화) + 사이클 107 `inquire_stock_basics` CTPF1002R + FHKST01010100 merge 패턴 영속 (변경 0 영역 = except 분기 카운터 호출만 추가) + 사이클 122~143 영속 (영향 0) + CLAUDE.md "절대 깨지 말 것" 8 영역 영속. **매매 안전성 무영향 확정 영구 영속**: logging 영역 + 카운터 영역 한정 (process-local in-memory, uvicorn 단일 워커 영속 의무) + `src/engine/risk.py` / `src/engine/order_engine.py` / `src/realtime/` / `src/auth/` 변경 0 + 매수 진입 hot path 영향 0 + 매도/익일청산/15:20 강제청산 hot path 무관. **운영 효과 영구 영속**: 6/16 KST 16:10 `_stock_master_basics_refresh_task` 자동 발화 영역 영구 영속에 `[stock_master_basics_refresh_summary]` 로그 영역 영구 영속에서 `graceful_failed_fhkst=N` 필드 영구 영속 = 운영자 즉시 진단 ("FHKST01010100 영역 영구 영속 KIS SESSION FULL 영역 영구 영속 N건 graceful 처리 영역 영구 영속") + Loki 파싱 영역 영구 영속에서 사이클 129 영역 영구 영속 silent 결함 추세 영역 영구 영속 측정 가능. **사이클 129 OPSQ1002 SESSION FULL 영역 영구 영속 인계 영역 종결**: 사이클 132 카드 #27 영역 영구 영속 silent fail 가시화 결핍 영역 영구 영속 시정 완료. team-leader 자체 결정 영역 영구 영속 (사용자 결정 Q1=A 영속 영역 영역 영구 영속 사이클 143 commit `ed8298b` + CI success 4분 26초 + EC2 Deploy success 35초 영속 영역 영구 영속).
- **사이클 108 (2026-06-11) — `inquire_stock_basics` `hts_avls` 5번째 키 보강 영구 영속 (Plan Phase A 완료, 시총 영역 데이터 영구 영속 확보)**: 사이클 104 인계 Q5=B (LOW 위험 자문 생략 영구 영속) + 사이클 107 raw 보강 의존성 해소 영구 영속 완료 → Plan Phase A 데이터 영역 영구 영속 활용 가능 → 사이클 108 = `inquire_stock_basics` `hts_avls` 1 키 보강 + `stock_master.list_by_filter()` 신규 메서드 + VB/LTV/BFB `_scan_universe()` 전환 통합 영구 영속. 사용자 결정 Q1=A 시총 영역 = `hts_avls` (KIS FHKST01010100, 백만원 단위 영구 영속, KIS 정본 인용 의무 영속 = `chk_inquire_price.py` 영역 영구 영속 + 사이클 98 G-DOC1 답습). **시정 (production 1 파일, +1L 순증)**: 사이클 107 4 키 (`acml_tr_pbmn` + `lstn_stcn` + `acml_vol` + `prdy_vrss`) → 사이클 108 5 키 보강 영구 영속 = `hts_avls` 추가 (FHKST01010100 응답 영역 = 시가총액 백만원 단위 영구 영속). for loop 4 키 → 5 키 영역 영구 영속 (`if key not in merged_raw` 영역 영구 영속 덮어쓰기 금지 영속). **CTPF1002R 기존 키 덮어쓰기 금지 설계 영구 영속**: 사이클 81 G-AST1 영구 영속 정합 (`bfdy_clpr` 영역 영구 영속 + 사이클 100 영역 3 prefix OR 영구 영속 + 사이클 101 `_full_universe_load_once` 영역 자동 반영 영속 + `hts_avls` 덮어쓰기 금지 영역 영구 영속). KIS MCP 정본 영구 영속 = `chk_inquire_price.py` main 호출 영역 인용 의무 영구 영속 (사이클 98 G-DOC1 답습 영구 영속). **회귀 가드 영역 (HIGH 영역)**: `tests/unit/api/test_cycle108_hts_avls.py` 영역 = `hts_avls` 5번째 키 영속 영구 + 덮어쓰기 금지 영속 + AST 영구 가드 영속. **호출 영역 영구 영속**: `stock_master.upsert_one(ticker, ...)` 경로 영구 영속 + 사이클 101 `_full_universe_load_once` 배치 호출 영역 영구 영속 (2,800 종목 × 2 KIS 호출 + 50ms sleep 영구 영속 = 약 5분 소요 영역 영구 영속). **운영 효과 영구 영속 (push + EC2 자동 배포 후)**: `inquire_stock_basics` 1회 호출 → stock_master.raw 에 `acml_tr_pbmn` + `lstn_stcn` + `acml_vol` + `prdy_vrss` + `hts_avls` 자동 포함 영구 영속 → 사이클 108 `stock_master.list_by_filter(min_market_cap=, min_trade_amount=)` 데이터 영역 영구 영속 의존성 해소 영역 영구 영속. **영속 의무 매트릭스**: 사이클 32 R4 universe guard 영속 + 사이클 38 명문화 영속 (`inquire_stock_basics` = 매수/매도 진입 직전 lazy 호출 영역 영구 영속 + 매수 진입 전 영역 한정 영속) + 사이클 81 G-AST1 영속 (`bfdy_clpr` + `hts_avls` 덮어쓰기 금지 영속) + 사이클 88 G-REJECT 영속 + 사이클 98 G-DOC1 영속 (KIS 정본 인용 의무 영구 영속 = `chk_inquire_price.py` 영역 영구 영속) + 사이클 101 영속 (`_full_universe_load_once` 자동 반영 영구 영속) + 사이클 102 G-REJECT 영속 + 사이클 106 영속 영구 확인 (lifecycle race 차단 영역 영구 영속) + 사이클 107 영속 영구 확인 (raw 보강 4 키 영역 영구 영속). **매매 안전성 무영향 확정 영구 영속** (`inquire_stock_basics` = 종목 마스터 캐시 영역 한정 영구 영속 + 매수/매도 진입 직전 lazy 호출 영역 영구 영속 + 매매 hot path 직접 영향 0 영구 영속 + Rate Limit 50ms sleep KIS LMS chain 안전 영역 영구 영속)
- **사이클 107 (2026-06-11) — `inquire_stock_basics` raw 보강 (CTPF1002R + FHKST01010100 merge, Plan Phase A 데이터 영역 의존성 영구 해소)**: **Phase 1 진단 결정적 발견 (KIS MCP 정본 영구 영속 확정)**: CTPF1002R 응답 67 컬럼 영역 = 3 키 (`acml_tr_pbmn` 누적 거래 대금 + `lstn_stcn` 상장 주수 + `prdy_vol` 전일 거래량) **모두 부재 영구 영속 확정**. FHKST01010100 (`inquire_price` 주식현재가 시세, KIS 정본 `chk_inquire_price.py` 인용 의무 = 사이클 98 G-DOC1 답습) 응답 영역 = 3 키 모두 **존재 영구 영속 확정** (`acml_tr_pbmn` + `lstn_stcn` + `acml_vol` 누적 거래량 (`prdy_vol` 영속 동등) + `prdy_vrss` 전일 대비). **시정 (production 1 파일, +44L 순증)**: `inquire_stock_basics` 본체 보강 = CTPF1002R 호출 *후* `await asyncio.sleep(0.05)` Rate Limit (사이클 83/91/97 답습 영속 = KIS LMS chain 안전 영역 영구 영속) → FHKST01010100 추가 호출 (`kis_get_quote(STOCK_PRICE_URL, "FHKST01010100", ...)` 시세성 풀 라우팅 영속 + 메인 fallback 영속) → try/except graceful (`price_data = {}` fallback + `[inquire_stock_basics] FHKST01010100 호출 실패 graceful` 로그) → `merged_raw = dict(ctpf_output)` 영역 영구 영속 + for loop 4 키 조건부 merge (`acml_tr_pbmn` + `lstn_stcn` + `acml_vol` + `prdy_vrss`, `if key not in merged_raw` 영역 영구 영속). **CTPF1002R 기존 키 덮어쓰기 금지 설계 영구 영속** = 사이클 81 G-AST1 영구 영속 정합 (`bfdy_clpr` 영역 영구 영속). **graceful fallback 영역 영구 영속**: FHKST01010100 RuntimeError / KisApiError 영역 = CTPF1002R 단독 raw 반환 (호출자 보호 영구 영속). CTPF1002R 에러 = 전파 (기존 영역 영구 영속). **회귀 가드 12 신규 (HIGH 4 케이스, `tests/unit/api/test_cycle107_inquire_stock_basics_merge.py`)**: HIGH-1 양쪽 API 호출 영역 영구 영속 + CTPF 선행 순서 영역 영구 영속 (2 sub-case) + HIGH-2 raw 3 키 포함 영역 + `bfdy_clpr` 보존 영역 + 덮어쓰기 금지 영역 (3 sub-case) + HIGH-3 graceful fallback 영역 (RuntimeError / KisApiError / CTPF 에러 전파, 3 sub-case) + HIGH-4 AST 영구 가드 영역 (FHKST01010100 문자열 영구 영속 + inquire-price URL 영역 영구 영속 + 60줄 윈도우 영역 + 3 키 merge 영역, 4 sub-case). **운영 효과 영구 영속**: `inquire_stock_basics` 1회 호출 → stock_master.raw 에 `acml_tr_pbmn` + `lstn_stcn` + `acml_vol` 자동 포함 영구 영속 / 2,800 종목 × 2 KIS 호출 + 50ms sleep = 약 5분 소요 (사이클 101 `_full_universe_load_task_loop` 배치 시 적용 영구 영속) / 향후 Plan Phase A (사이클 108+) `list_by_filter(min_market_cap=, min_trade_amount=)` 데이터 영역 영구 영속 의존성 해소. **영속 의무 매트릭스 9 영역 전수 영속**: 사이클 32 R4 / 38 명문화 / 81 G-AST1 (`bfdy_clpr` 덮어쓰기 금지 영구 영속) / 88 G-REJECT / 98 G-DOC1 (KIS 정본 `chk_inquire_price.py` 인용 의무 영구 영속) / 101 영속 (`_full_universe_load_once` → `inquire_stock_basics` 호출 영역 자동 반영) / 102 G-REJECT / 106 영속 (`_full_universe_load_task_loop` 영역 변경 0) / CLAUDE.md "절대 깨지 말 것" 8 영역. **매매 안전성 무영향 확정 영구 영속** (종목 마스터 캐시 영역 한정 + 매수/매도 진입 직전 lazy 호출 영역 + 매매 hot path 직접 영향 0 + Rate Limit 50ms sleep KIS LMS chain 안전 영구 영속)
