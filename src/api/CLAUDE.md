...
- **분할 fetch (날짜 윈도우 100건 경계) + VCP backfill 225일**:
  ...
  - **`fetch_daily_candles` 와 별도 함수**다 (그쪽은 memcache 5분 + single-flight + `asyncio.shield` 유지). 호출자 = `scanner._stock_master_daily_load_once` (VCP universe 225일, `_DAILY_LOAD_VCP_BACKFILL_DAYS`). 매매 hot path 무관 — 데이터 적재 한정, 매수 진입 전 영역. 가드 `tests/unit/api/test_cycle172_daily_ranged_backfill.py`
