...
- **사이클 153 (2026-06-16) — `list_by_filter()` 영역 KOSPI200 / KOSDAQ150 인자 추가 영구 영속**: 사용자 결정 Q2=A `is_kospi200: bool | None = None` + `is_kosdaq150: bool | None = None` 인자 (사이클 108 nxt_tradable 패턴 답습). 양쪽 모두 True 시 **OR 합집합** 영역 영구 영속 (donchian_swing FUNNEL_STAGES[0] "코스피200+코스닥150 합집합" 의무 정합). 한쪽만 True/False 시 AND 영역 영구 영속 (PostgREST `.eq()` 인덱스 활용 영구 영속 = `idx_stock_master_is_kospi200` + `idx_stock_master_is_kosdaq150` 부분 인덱스). 둘 다 None 시 무필터 (회귀 보존). **사이클 121 (2026-06-12) silent 결함 영구 영속 시정** = donchian_swing.py::_scan_universe() 영역에 인자 전달 누락 영역 영구 영속. 운영 DB 실측 (2026-06-16) = KOSPI200 1796 종목 + KOSDAQ150 149 종목 = ~1945 합집합 영역 영구 영속. 회귀 가드 = `tests/unit/db/test_cycle153_list_by_filter_index_flags.py` (5 케이스). 매매 안전성 무영향 영역 영속 (사이클 38 명문화 + scanner 매수 진입 전 한정).
