...
AWS RDS PostgreSQL CRUD 모듈. 현재 DB 클라이언트 정본 = **`pg.py` (asyncpg 네이티브 async)**. `supabase.py` 는 롤백용으로 병존하나 어느 db 모듈도 참조하지 않는다(미사용).

> 사이클별 변경 이력: `docs/HARNESS_CHANGELOG.md`
> Supabase→RDS 이전(M0~M6) 전모: `docs/HARNESS_CHANGELOG.md` 최상단 엔트리

## pg.py — asyncpg 연결 풀 + 쿼리 헬퍼 (현재 클라이언트 정본)

- `settings.database_url` (asyncpg DSN, `?sslmode=require`) 로 `init_pool()` 이 전역 풀 생성 (`min_size=2`/`max_size=10`/`max_inactive_connection_lifetime=300`/`command_timeout=30`). `main.py` lifespan 시작 시 `init_pool()`, 종료 시 `close_pool()` 1회씩.
- **`_init_conn(conn)`** (신규 물리 연결마다 실행되는 init 훅): JSONB/JSON codec 등록 (`set_type_codec(encoder=json.dumps, decoder=json.loads)`) — asyncpg 기본 `str` 반환을 dict 로 강제 왕복. **⚠️ codec 누락 시 `system_config.get_cash_usage_ratio` 등 `isinstance(raw, dict)` 기대 코드가 전부 silent 폴백 → 매매 파라미터 오작동**. KST 타임존은 `_init_conn` 이 아니라 `init_pool()` 의 `server_settings={"timezone": "Asia/Seoul"}` (연결 핸드셰이크 파라미터)로 지정 — `init=` 훅 내 `SET TIME ZONE`(세션 레벨)은 asyncpg 풀이 release 시 RESET 하여 UTC 로 되돌아가는 silent 결함 유발(M2a 통합 검증 발견, trade_history 읽기 `to_char(...,'+09:00')` 가 UTC 렌더 → KST 날짜 하루 밀림).
...
## asyncpg 계약 패턴 (M0~M6 이전 확정)

전 db 모듈이 준수하는 asyncpg 바인딩/렌더 계약. 신규 CRUD 작성 시 의무.

- **JSONB** — codec (`_init_conn`) 가 dict/list 를 왕복 처리. **raw dict/list 를 그대로 바인딩** (호출부에서 `json.dumps` 사전 적용 금지 — 이중 인코딩). 읽기도 codec 이 dict 로 복원.
...
- **DATE** — `src/db/_kst.py::to_date(x)` 로 강제 변환 후 바인딩. asyncpg 는 DATE 컬럼에 Python `date` 객체를 요구 (str 을 넘기면 `'str' object has no attribute 'toordinal'` 즉시 실패) → `to_date` 가 str/date/datetime/None 을 수용 (사이클 M6 라이브 핫픽스).
- **NUMERIC** — asyncpg 가 `Decimal` 로 반환 (손익 정밀도 보존 — `get_trade_pairs` 등).
...
- `get_logs(limit=100, log_level=None, *, from_date=None, to_date=None, page=1, size=None)`: 페이징 + KST 기간 필터. 반환 `{"items": list[dict], "total": int, "total_pages": int}`. `size` 미지정 시 `limit` 흡수 (하위 호환). `from_date`/`to_date` 명시 시 `timestamp >= "{date}T00:00:00+09:00"` / `<= "{date}T23:59:59.999999+09:00"` KST 강제. `total` 은 별도 `SELECT count(*)` fetchval, `total_pages = ceil(total / size)`
...
- `insert_log_report()`: **cycle283 D6 — `INSERT … ON CONFLICT (target_date) DO UPDATE SET` upsert.** SET 절은 **base 9컬럼만**(summary/findings/metrics/model + 토큰 5) — `ext_*` 6컬럼과 `created_at` 은 **절대 넣지 않는다**(한 컬럼이라도 새면 20:20 클라우드 루틴 결과가 조용히 지워진다. `upsert_external_report` 의 SET 절(ext_* 6)과 **교집합이 공집합**이라 어느 순서로 쓰든 상호 보존된다). 호출 주체 2개 — 20:05 1차 스냅샷(`daily_metrics_snapshot`, model·토큰 NULL)과 21:30 완전판(`generate_daily_log_report`)이 같은 행을 쓴다. `on_conflict="nothing"` 은 비파괴 모드(기존 행이 있으면 손대지 않고 None)지만 **현재 프로덕션 호출자가 0건**이다 — 오용 방어용 파라미터이고, `POST /api/log-reports/run` 의 비파괴는 **라우트 선조회 가드**(`_is_complete_report`: `metrics.snapshot_pass` 아님 ∧ `summary` 가 비지 않음 ∧ OpenAI 실패 placeholder 아님 ∧ `model` 채워짐)가 혼자 담당한다. 라우트가 `"nothing"` 을 쓰지 않는 이유 = 20:05~21:30 엔 1차 스냅샷 행이 이미 있어 `DO NOTHING` 이면 수동 복구의 완전판이 조용히 버려진다. **사이클 58 V-2 (2026-06-04)**: `input_tokens / output_tokens / total_tokens / latency_ms / cost_estimate_usd` 5 keyword 추가 (모두 기본 None — NULL INSERT). `cost_estimate_usd` 는 `Decimal | None` → DB 저장 시 `float` 변환.
...
- 🔴 **`tick_channel_gap_hold_enabled` — cycle295 (2026-09-16) 로 소비처 소멸.** getter/setter 도 함께 삭제됐다. 운영 DB 에 값 `false` 가 **잔존**한다(읽는 코드가 없어 지우지 않았다 — DELETE 는 되돌리기 어려운 운영 조치라 승인 대상이고 얻는 것이 0행이다). ⚠️ **이 키 이름 재사용 금지** — 같은 이름을 반대 의미로 되살리면 저장된 `false` 가 조용히 적용된다
...
- 범위 외 입력은 `ValueError`. DB 호출은 `pg.fetch`/`pg.execute` (asyncpg 네이티브 async)
...
- DB 호출은 `pg.fetch`/`pg.execute`/`pg.executemany` (asyncpg 네이티브 async). read 는 `_with_retry` 내장
...
- CRUD 함수 (`stock_master_daily.py` 미러):
  ...
  - `get_financial_series(ticker, div_cls="0", limit=3)` — `pg.fetch` (`_with_retry` 내장 read retry, 사이클 187 계보). 최근 N기 (`stac_yymm` DESC). div_cls "0"=년/"1"=분기
  - `max_stac_yymm(ticker, div_cls="0")` — 신선도/백필 게이트 키 (스캐너 사용)
  - `count_all()` — 적재 진단
- DB 호출은 `pg` (asyncpg) 경유 + raw JSONB 덮어쓰기 금지 (사이클 81 G-AST1 답습)
...
- 매핑 등록 (`_order_qty/_order_strategy/_order_ticker`) 은 **반드시 async DB write 진입 전 동기 영역** 완료 (시장가 즉시체결 race 차단)
