...
- **`insert_snapshot(*, target_date, strategy_id, step_no, step_name, survived_tickers=None, excluded_sample=None, survived_count=None, excluded_count=0, step_conditions=None, is_provisional=False) -> dict | None`** — 인자는 전부 keyword-only 다. `(target_date, strategy_id, step_no)` **UPSERT** 라 같은 단계는 최신 1행만 남는다. `snapshot_at` 은 **그 행의 마지막 쓰기 시각**이다. 첫 INSERT 는 컬럼 기본값 `now()`(migration 030)가 채우고, 덮어쓸 때는 `DO UPDATE SET … snapshot_at = now()` 가 채운다. 두 경우 모두 DB 시계 하나만 쓴다(애플리케이션 `datetime` 을 바인딩하지 않는다). 즉 `snapshot_at` 은 「그 값이 언제 쓰였나」이지 「그 행이 언제 처음 생겼나」가 아니다(하루 쓰기 순서 = `src/engine/CLAUDE.md` 「funnel 스냅샷 캡처」 절). `snapshot_at` 을 조건으로 쓰는 소비처는 `routes/market_ops.py` 하나다(`src/routes/CLAUDE.md` `/api/market-ops` 행).
...
- `list_snapshots(*, target_date, strategy_id=None, raise_on_error=False)`: 특정 영업일 + 전략의 모든 단계(`step_no` ASC). 조회 예외는 기본값이면 WARNING 을 남기고 `[]` 를 돌려준다. `raise_on_error=True` 면 예외를 그대로 던진다 — `funnel_capture` ④ 가 DB 장애를 「저녁 캡처 없음」과 가르려고 쓴다
- `list_recent_by_strategy(strategy_id, days=7)`: 최근 N영업일 추이 — `target_date <= 오늘` 이라 저녁 미리보기(다음 거래일) 행은 나오지 않는다
...
- **KIS 폴백 조건 3개 (DB 를 쓰기 전에 이 순서로 검사한다)**:
  ...
  2. **신선도 게이트** — 기준이 두 갈래다. `max_bas_dd(ticker)` 가 `None`(판정 불가)이면 어느 쪽이든 graceful 통과한다.
     - `expected_head` **있음**(키워드 전용, 「직전 영업일」) → `latest < expected_head` 면 폴백(`reason=stale`). 벽시계 달력 판정은 보지 않는다. 6전략 `prepare()` 가 `StrategyBase._resolve_expected_daily_head()` 로 구해 넘긴다.
     - `expected_head` **없음**(`None`) → 달력 판정 `(today − latest).days > DAILY_STALENESS_DAYS` 면 폴백. 인자 없는 호출부(kojiro `recompute_held_atr` · `llm_buy_gate`)와 휴장일 조회가 실패한 날의 prepare 가 이 갈래를 탄다.
  ...
  - 폴백 자체가 실패하면 DB 값을 그대로 쓴다(graceful). 헬퍼 3 = `_row_has_lock` / `_extract_raw`(raw 추출 DRY) / `_kis_fallback`(락·신선도·부족 공통 + 실패 시 DB graceful). 폴백은 `[prepare_db_fallback] ticker= reason= db= kis=` 1행을 남긴다
