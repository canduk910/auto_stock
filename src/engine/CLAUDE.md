...
**stale_watcher_core.py** (사이클 135 — `check_and_resubscribe_stale` 영역 grace 가드 `_is_within_grace(t)` 영역 신규: `if t in ticker_last_tick: return False` (G-GRACE-7 사이클 29 005935 보호 직접 검증) + `ack_at not datetime: return False` (G-GRACE-6 ACK race 보호) + try/except 안전 폴백 (mock 환경 영역 = 사이클 63 Phase 2-A3 mock 회귀 가드 보존). `_ack_map_raw if isinstance(_ack_map_raw, dict) else {}` 안전 가드. 매매 안전성 무영향 (G-SAFETY-2 risk/order_engine import 0 직접 검증))
