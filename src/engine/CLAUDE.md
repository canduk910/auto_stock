...
| `quote_token_refresh.TIME_QUOTE_TOKEN_REFRESH` | 15:45 | **cycle269 → cycle270** — 보조 시세 계정 접근토큰 강제 재발급 (`quote_token_refresh.task_loop`). 활성 계정 전부 `TokenManager.revoke()`→`issue()` 순차(cycle270 — `issue()` 단독은 KIS 동일 토큰 반환으로 앵커 불이동, 09-10 실측; 61s gap, 7계정 ≈ 7분, 요약 행 ≤15:52 가 정상). 매매 무관(시세 풀 전용) · 주계정 무접촉 · WS `approval_key` 무영향. T−10분(자연 재발급 문턱)도 장 마감 뒤여야 하므로 15:35 이 아니라 15:45 |
