...
- `_scan_loop()` (5분 주기, 09:30~): VB+LTV+donchian + 모든 전략 보유 합집합 재구독 + `_reprepare_breakout_if_empty()` (VB/LTV `_scanned_tickers=[]` 시 prepare() 1회 재시도, donchian 제외) + `_resubscribe_stale_priority(cap=10)` + `_report_tick_coverage()`
