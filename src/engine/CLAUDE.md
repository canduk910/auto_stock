...
risk.py(on_tick) → order_engine.py(체결통보·DB persistence) → scheduler.py(시간 가드·run/settle) ← boot_manager.py(_boot 본체, 사이클 51) / stale_tracker.py(StaleTrackerState, 사이클 48) / stale_manager.py(K stale 진단·캐시·force_retry·silent inactive·universe guard·delta unsubscribe·**K stale watcher 본체**·**5분 우선 재구독** 헬퍼 **11 함수 + 10 상수**, 사이클 60 Phase 2-A1 + 사이클 61 Phase 2-A2 + **사이클 63 Phase 2-A3 — refactor #2 완료**) / sell_rejection.py(SellRejectionTracker, 사이클 55 R-1 + 사이클 57 V-1 알람)
...
- **사이클 62 가격 필터 (2026-06-05) — 매수 진입 전용 가격대 차단**: `RiskManager._get_price_filter_cached() -> PriceFilter` (60s TTL `time.monotonic()` 캐시, 사이클 56-E `buy_block` 패턴 답습) — `system_config.get_price_filter()` 조회. 분기 위치는 `check_exit_signal` 분기 *후* + `check_buy_signal` *전* (사이클 38 명문화 영속 — 매도/익일청산/손절/15:20 강제청산 영향 0). **Q2 비교 가격 fallback** (domain-expert 자문 옵션 A): `scanner.ticker_prev_close.get(ticker)` 우선 → 미확보 시 `current_price` fallback → 둘 다 미확보 시 graceful 통과 (필터 미적용). 갭상승 시점 결함 (작전주 회피) 영구 차단. **3 모드** (Q4 자문): `mode=HARD` 범위 외 → 매수 차단 + `[price_filter_skip] ticker=... cmp_price=... reason=below_min/above_max mode=HARD min=... max=...` INFO + DailyEmitCap (`_price_filter_skip_logged_today`) 1회/ticker/일 cap. `mode=WARN` 범위 외 → 매수 진행 + `[price_filter_warn]` WARNING (운영자 임계 조정 1주 운영 기간 권고). `mode=OFF` → 분기 미진입 (디폴트). **일일 카운터** `_price_filter_skip_count: dict[str, int]` (`hard`/`warn`) — `scheduler._settle()` 직전 `[price_filter_daily_summary] mode=... min=... max=... hard_skip=N warn_skip=M` INFO 1행. **Q3 추가 가드** (사용자 명시): `order_engine.py:L411` (매수 폴백) + `L664` (매도 폴백) 시장가 거부 5호가 폴백 시 가격 필터 재평가 0건 — `risk_manager._get_price_filter_cached()` 호출 0건 (E-4 AST 정적 회귀 가드 영속). `PUT /api/system/price-filter` 직후 `risk_manager.invalidate_price_filter_cache()` 즉시 호출 — 60s 캐시 도중 사용자 토글 즉시 반영. **회귀 가드**: 백엔드 33 케이스 (A 5 + B 4 + C 2 + D 2 + E 4 + F 5 + G 2 + H 1 + I 5 + C-Route 3) + 프론트 5 케이스 = **38 케이스 / 12 파일**. **`RiskManager.reset_daily_state()` 확장** (사이클 56-D 답습): cap 2 (`_risk_silent_skip_logged_today` + `_price_filter_skip_logged_today` + `_price_filter_warn_logged_today`) + 일일 카운터 (`_price_filter_skip_count`) + reasons dict 일괄 reset
