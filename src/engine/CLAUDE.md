...
3. `order_engine.py::is_nxt_window` 가 `15:30~20:00` 을 **NXT 창으로 본다** — KRX 애프터 거부가 그 창에
   떨어지면 `stock_master.nxt_tradable=False` 를 잘못 쓴다(프리장 영구 차단 + WS blind 파급).
4. 애프터 시간대 시장가 주문은 거부된다 — `is_market_order_disallowed` 지정가 폴백이 받아내는지,
   `is_market_closed_rejection` 으로 먼저 떨어지는지는 **거부 `msg1` 원문 실측 전까지 모른다**.
5. `OrderDivision` 에 애프터 호가유형이 없다(`LIMIT="00"` / `MARKET="01"` 둘뿐).
6. ETF/ETN 제외는 우리 유니버스가 이미 ETF 를 빼므로 **무영향**.
