...
**kojiro_gap_observe.py** (cycle268 leaf, 2026-09-09 — `[kojiro_gap_observe]` 판독 마커, cap 1회/(ticker,caller,verdict)/일, never-raise. cycle273-pre(09-10)로 14번째 필드 `ws_collapse=blocked|allowed|-`(WS 시가였다면 붕괴 가드 판정이 어땠을지 — 반사실) 순수 append. ⚠️ 경로 B(`caller=on_tick`) 행의 `ws_*` 5필드는 `risk.py:495` 가 호출 전에 같은 값을 캐시에 덮어 **동어반복** — 오염 판정은 경로 A(`_swing_buy_poll_loop`) 행만 유효, `ws_open` 부재 시 `-` 가 다수(정상))
...
| `quote_token_refresh.TIME_QUOTE_TOKEN_REFRESH` | 21:30 | **cycle269 → cycle270** — 보조 시세 계정 접근토큰 강제 재발급 (`quote_token_refresh.task_loop`). 활성 계정 전부 `TokenManager.revoke()`→`issue()` 순차(cycle270 — `issue()` 단독은 KIS 동일 토큰 반환으로 앵커 불이동, 09-10 실측; 61s gap, 7계정 ≈ 7분, 요약 행 ≤21:38 가 정상). 매매 무관(시세 풀 전용) · 주계정 무접촉 · WS `approval_key` 무영향. **cycle270-B(2026-09-10 사용자 결정)** 15:45 → 21:30 이동 — 24h 토큰이라 시각은 자유, 제약은 T·T−10분이 장중 밖 + 7분 직렬화 창이 20:00~20:15 REST 집중 창·20:20/20:30 루틴과 안 겹칠 것(`test_c9_schedule_time_invariants` 가 scheduler `TIME_*` 전수와 창 충돌 0 을 강제) |
