...
risk.py(on_tick) → order_engine.py(체결통보·DB persistence) → scheduler.py(시간 가드·run/settle) ← boot_manager.py(_boot 본체, 사이클 51) / stale_tracker.py(StaleTrackerState, 사이클 48) / stale_manager.py(K stale 진단·캐시·force_retry·silent inactive·universe guard·delta unsubscribe 헬퍼 9 함수 + 10 상수, 사이클 60 Phase 2-A1 + 사이클 61 Phase 2-A2) / sell_rejection.py(SellRejectionTracker, 사이클 55 R-1 + 사이클 57 V-1 알람)
