...
- `_stale_watcher_loop()` / `_check_and_resubscribe_stale()` (K, 2026-05-12): `start()` 안에서 `_session_task` 발화 직후 `_stale_watcher_task = asyncio.create_task(_stale_watcher_loop())` 시작. `STALE_WATCHER_INTERVAL_SECS=120s` 주기로 `kis_ws.get_subscribed_tickers()` 와 `scanner.ticker_last_tick` 비교 → `STALE_FRESHNESS_SECS=60s` 초과면 stale. 종목별 누적 `_stale_retry_count` retry 카운터: 1~10회 → `_send_subscribe(TICK_TR_ID, t, subscribe=True)` 재발송, 11~20회(`> STALE_FORCE_REREGISTER_AFTER=10`) → `unsubscribe`+`subscribe(bypass_limit=True)` 강제 재등록, 20회 초과 → skip(`skipped_giveup` 카운트, 다음 `_scan_loop` 사이클에 위임). 전체 fresh 회복 시 `_stale_retry_count.clear()` 자동 리셋. Rate Limit 보호 50ms sleep, 종목별 예외 격리. 본체 예외는 ERROR 로그로 흡수해 다음 사이클 진행. `[stale_watcher] subscribed=N stale=K resubscribed=A force_reregistered=B skipped=C` INFO + `write_log` 1행. `start()` finally + `stop()` 의 task_attr 튜플에 `_stale_watcher_task` 포함(좀비 task 방지). `_reset_daily_state()` 가 `_stale_retry_count.clear()` 추가. F1(재연결 1회) + `_scan_loop`(5분) + K(120s) 3중 안전망. 2026-05-12 11:48 fresh=1/stale=26 운영 사고 대응. **사이클 7-C (2026-05-18) 풀 통합**: 단일 세션 직접 호출(`kis_ws._send_subscribe` / `kis_ws.unsubscribe` / `kis_ws.subscribe`) → 풀 헬퍼(`kis_ws_pool.resend_subscribe_for_ticker` / `kis_ws_pool.unsubscribe_in_pool` / `kis_ws_pool.subscribe(priority='HIGH', bypass_limit=True)`) 위임. 풀의 `_ticker_to_session` 분배 추적을 활용해 정확한 세션(메인/보조)에서 재전송/재등록. 강제 재등록 시 `unsubscribe_in_pool` 이 추적 dict 에서 제거 → 이어지는 `subscribe` 가 라운드로빈 재선택 (보조 0개 시 메인 fallback 정상). **사이클 9 (2026-05-18) KIS 차단 회피 안전망**: KIS Open API 공지(2026-05-18 — 무한 연결/종료, 검증 없는 무한 등록/해제 시 IP/앱키 일시 차단 예정) 대응으로 트래픽 완화: `STALE_WATCHER_INTERVAL_SECS` 30 → 120 (분당 발화 1/4), `STALE_FORCE_REREGISTER_AFTER` 3 → 10 (10회 × 120s = **20분 stale 누적** 후에만 강제 재등록 — 단발 시세 끊김에서 즉시 unsub/sub 차단). 회피 산식: 보조 5개 × 40 종목 × 강제 재등록 트래픽 800/분 → 200/분 미만 + 강제 임계 분기로 실제는 그 1/10 이하. FRESHNESS=60 보존(5/12 사고 대응 의도 그대로). 회귀 가드: `tests/unit/engine/test_stale_watcher_thresholds.py` 7 케이스 + `tests/integration/test_stale_watcher.py` / `test_stale_watcher_pool.py` 임계값 가정 동기 갱신. **사이클 13 (2026-05-18) stale 회복 강화**: `STALE_FORCE_REREGISTER_AFTER` 10 → **5** 단축 (5회 × 120s = 10분 stale 누적 후 강제 재등록 — 단발 silent inactive 회복 강화). `STALE_WATCHER_INTERVAL_SECS=120` / `STALE_FRESHNESS_SECS=60` 보존. 분기 자동 축소: `*2` 가드 20→10, 6~10회 force, 11회 초과 skip. 분당 추가 트래픽 ~20 req (KIS 한도의 2%) 무시 가능 + 사이클 9 health monitor 자동 비활성 안전망. 회귀 가드 동기 갱신(임계 가정 10→5, 케이스 수 보존). **사이클 13-D (2026-05-18)** `_scan_loop` 통합 구독 직후 `_resubscribe_stale_priority(cap=10)` 호출 분기 추가 — `scanner.ticker_last_tick` 기반 stale 종목(`> STALE_FRESHNESS_SECS`)을 결정적 정렬 후 상위 10개에 `kis_ws_pool.subscribe(priority='HIGH', bypass_limit=True)` 즉시 재구독 (50ms sleep, 종목별 예외 격리, 본체 예외 흡수). K watcher 120s + `_scan_loop` 300s 외 별도 5분 주기 자연 회복 경로. 회귀 가드: `tests/unit/engine/test_scan_loop_stale_priority.py` 3 케이스. **사이클 13-E (2026-05-18) 진입점 풀 통합**: `_check_and_resubscribe_stale()` 진입점 `subscribed = kis_ws.get_subscribed_tickers()` (메인 단독) → `subscribed = kis_ws_pool.get_subscribed_tickers()` (풀 전체). 사이클 7-C 풀 통합 시 진입점 갱신 누락 결함 — 메인 비어있을 때(보유 0, 시세는 보조 quote-1 에 집중) `if not subscribed: return` 으로 K watcher 본체 영원히 발동 안 하던 결함 차단. 사이클 13 B (`STALE_FORCE_REREGISTER_AFTER 10→5`) 실효성 회복 + 005930 등 KIS WS silent inactive 종목 강제 재등록 경로 정상화. 회귀 가드: `tests/integration/test_stale_watcher_pool.py` D-6 신규 1 케이스 + 기존 5 케이스 monkeypatch 풀 헬퍼로 갱신
...
- **사이클 14-A (2026-05-18) `_report_tick_coverage()` 풀 통합 (사이클 11 짝궁 누락 fix)** — `src/engine/scheduler.py::_report_tick_coverage()` 의 source `from src.realtime.websocket import kis_ws` + `kis_ws.get_subscribed_tickers()` → `from src.realtime.websocket_pool import kis_ws_pool` + `kis_ws_pool.get_subscribed_tickers()`. 사이클 11 에서 `get_scan_status()` 만 풀 통합되고 짝궁 메서드 `_report_tick_coverage()` 가 함께 갱신되지 않은 누락 결함. 2026-05-18 KST 12:57~14:25 운영 사고 — 풀 25 종목 구독 (`[stale_watcher] subscribed=25 stale=18`) 중인데 `[tick_coverage] subscribed=0 fresh=0 stale=0` 출력 → 운영자가 "전부 끊김"이라고 오인. fix 후 system_logs `[tick_coverage] subscribed=N (N>0)` 정상 노출. `_resubscribe_stale_priority` 의 source(`scanner.ticker_last_tick`) 와 독립이라 영향 0, 응답 로그 포맷/임계/system_logs prefix 100% 보존. 회귀 가드: `tests/unit/engine/test_report_tick_coverage_pool.py` 3 신규 케이스(풀 합집합 25 / 메인 0+보조 25 운영 사고 재현 / 풀 0 회귀) + 기존 5 케이스(`test_tick_coverage.py` 3 + `test_observability_logs.py` 2) `kis_ws` → `kis_ws_pool` mock target 동기 갱신
