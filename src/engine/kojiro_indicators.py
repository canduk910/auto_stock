"""고지로(小次郎) 대순환 순수 지표 — EMA 대순환 스테이지 · 대순환 MACD · Wilder ATR.

레퍼런스 `_workspace/kojiro_ma/kojiro/indicators.py` verbatim vendoring
(`src/engine/quant_score.py` 선례 = 순수 함수 모듈). DB/HTTP/시계 절대 미접촉.
8영역(risk/order_engine/realtime/auth/api/order.py/session.py/scanner.py/
strategy_registry.py) 미접촉 — 오프라인 `prepare` 격리, 매매 hot path 무접촉.

입력은 pandas DataFrame (index: 날짜 오름차순, columns: open/high/low/close/volume).
`enrich(df, cfg)` 가 전략에 필요한 모든 지표 컬럼을 추가한다.

**ATR 정의 = Wilder `ewm(alpha=1/period)`** (period=20). donchian `_atr`(단순평균)·
`stock_master_daily.get_atr`(단순평균 근사) 와 정의가 다르므로 재사용 금지 —
손절선(`buy_price - 2*ATR`) 정의 일관성의 단일 진실원.
"""
...
class KojiroIndicatorConfig:
    """지표 파라미터 — 설계서 §2. 5/20/40 = 전략 정체성 상수 (튜닝 금지)."""
...
def enrich(df: pd.DataFrame, cfg: KojiroIndicatorConfig) -> pd.DataFrame:
...
def crossed_up(line: pd.Series, sig: pd.Series, i: int = -1) -> bool:
    """골든크로스: 직전 봉에서 line<=sig, 현재 봉에서 line>sig. (조기진입용, Phase1 dormant)"""
...
def crossed_down(line: pd.Series, sig: pd.Series, i: int = -1) -> bool:
    """데드크로스. (보수모드 절반청산용, Phase1 dormant)"""
    return bool(line.iloc[i - 1] >= sig.iloc[i - 1] and line.iloc[i] < sig.iloc[i])
