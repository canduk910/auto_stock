...
| `long_tail_volatility` | VB 방식 + 전일대비 `min_prdy_rate%`↑. `_limit_up_reached` set으로 모드 관리 | 당일 모드 -3% / 상한가 모드 -5% + 익일 갭/트레일링. **15:20 상한가 미도달 일괄 청산** / 상한가 모드는 익일 NXT 프리 청산 + POST_NXT 손절 모니터링. `_next_day_clear_pending` 가드 | **MAIN 단독** (사이클 26: PRE_NXT + POST_NXT 제거, KRX ONLY. 매수만. 보유 손절 평가는 보드 가드 무관 작동) |
...
- **VB/LTV `DEFAULT_TRADABLE_BOARDS` = ("main",) 유지 (사이클 26)** — KRX ONLY 정책. PRE_NXT 복구 금지 (NXT 갭상승 위험 + SK하이닉스 시가 결함). POST_NXT 추가 금지 (VB OVERNIGHT 보유 결함). DB `strategy_config.params.tradable_boards` 도 함께 갱신
