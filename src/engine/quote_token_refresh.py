...
## 왜 21:30 인가 (2026-09-10 사용자 결정 — 종전 15:45 에서 이동)
...
제약은 두 가지뿐이다: (a) T 와 **T − 10분이 모두 KRX 장중(09:00~15:30) 밖**
(b) 7계정 × 61s ≈ 7분의 직렬화 창이 REST 를 많이 쓰는 예정 작업과 겹치지 않을 것.
24시간 토큰이라 시각 자체는 자유다 — 사용자(09-10): "장마감 후 20:00 이후에
해도 되지 않아? 어차피 24시간 유지라면 굳이 15:45 에 할 필요가 없어보여."

종전 15:45 는 16:00 일봉 적재 **앞** 빈 시간을 고른 값이었고(15:45~15:52 완료),
21:30 은 아래 창을 전부 피한 값이다(비충돌):
- 20:00 자문 / 20:00:05 전체 유니버스 적재 / 20:10 정산·로그 분석 — 20:00~20:15
  는 CLAUDE.md 금기 창이고 REST 도 몰린다 → T 와 문턱(21:20) 모두 그 뒤
- 20:20 일일 리포트 · 20:30(목) 주간 자문 클라우드 루틴 — 리포터 API 만 읽고
  KIS 토큰을 쓰지 않지만, 재시작이 겹치지 않도록 그 뒤로 둔다
- 21:30~21:38 에는 예정 작업이 없다(다음은 익일 07:48 유니버스 · 07:55 부팅)
- 15:45 시절의 걱정이던 "16:00 적재 침범" 은 이제 성립하지 않는다
...
판독법: 배포 D+1 부터 `[quote_token_refresh]` 요약이 매일 21:30 대에 1행씩
남고, 보조 계정의 "토큰 발급 완료(label=quote-*)" 가 21:20~21:38 창 밖에서는
...
# 매일 강제 재발급 시각 (KST). 상단 "왜 21:30 인가" 참조 — T 와 T−10분이 **모두**
# KRX 장중 밖이고 20:00~20:15 REST 집중 창·루틴 뒤여야 한다는 제약이 이 값을 결정한다.
# 2026-09-10 사용자 결정으로 15:45 → 21:30 이동(cycle270-B).
TIME_QUOTE_TOKEN_REFRESH = time(21, 30)
