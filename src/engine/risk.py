...
def _note_pre_window_krx_frame(ticker: str) -> None:
    ...
    **실측 완료(2026-09-21, 5영업일 180행)** — K1(시가 단일가)에는 0건이 맞고,
    프레임이 오는 것은 그 안에 중첩된 **K2(장전 시간외 종가 08:30~08:40,
    전일 종가 고정)** 뿐이다. 추론은 사실이 틀린 게 아니라 **범위가 좁았다**.
    판별식·잠금 셋은 `tick_channel_clock.note_pre_window_frame` docstring 이 정본이다.
    🔴 **행위는 바꾸지 않는다** — 프레임을 막는 게이트를 더하는 것은 매매 행위
    변경이고, K2 는 막을 것이 없다(LTV 의 `min_prdy_rate` 가 첫 잠금).
