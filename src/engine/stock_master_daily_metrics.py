"""사이클 122 (2026-06-12) + 사이클 133 (2026-06-15) — stock_master_daily metrics facade.

사이클 133 카드 #24 영속:
- 공통 헬퍼 `make_metrics_collector` 위임 facade (사이클 67 패턴 답습).
- 행위 보존 의무: emit prefix `[stock_master_daily_load_summary]` + summary 키 영속.

emit prefix:
- [stock_master_daily_load_summary] total=N fetched=M upserted_rows=K
  skipped_fresh=L failed=J elapsed_ms=I mode=...
...
(
    ...
    _daily_load_collector,
) = make_metrics_collector(
    prefix="stock_master_daily_load_summary",
    summary_keys=(
        "total", "fetched", "upserted_rows", "skipped_fresh",
        "failed", "elapsed_ms", "mode",
    ),
    str_keys=("mode",),
    use_last=True,
)
