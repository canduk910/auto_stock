...
function stopTitle(h: Holding): string {
  ...
  // 🔴 이 숫자에 닿기 **전에** 팔리는 경로가 따로 있다. 둘로 갈라 적는다 —
  //    「무조건」과 「가격 조건부」를 섞으면 운영자가 위험을 과대·과소평가한다.
  //    20일 신고가 스윙의 2영업일 청산은 **돌파고점 아래일 때만** 발동한다
  //    (`donchian_swing.py`: days_held ≥ n ∧ current_price < breakout_high).
  return (
    `${base}. ⚠️ 이 가격에 닿기 전에 팔릴 수 있다 — ` +
    '보유일수만으로 팔리는 것: 눌림목 돌파 5영업일 · 15:20 일괄매도 · 익일청산. ' +
    '다른 조건이 함께 붙는 것: 20일 신고가 스윙은 2영업일 뒤 돌파고점 아래면 판다 · ' +
    '대순환은 스테이지가 끝나면 판다.'
  )
}
