...
export const PARAM_SCHEMA_FIXTURE: ParamSchemaData = {
  ...
  "params": [
    ...
    },
    {
      ...
      "help": "N일 신고가 돌파 판정의 N. 일봉 fetch 일수(`max(long_ma+5, donchian+5)`)도 이 값이 정한다 — 🔴 상한의 진짜 근거는 보유 기간이 아니라 **읽기 클램프**다. `db/stock_master_daily.get_recent_daily` 가 `max(1, min(days, 100))` 로 자르므로 95 를 넘으면 더 요청해도 100봉만 온다. 보유 자체는 retention 390일 (≈261 영업일, cycle299)이라 넉넉하니 '보관 기간이 늘었으니 상한을 올려도 된다'는 판단은 틀린다. 사이클 212 에서 '진입 정체성'으로 PARAM_RANGES 에서 제외됐다."
    },
    {
      "key": "volume_period",
      "label_ko": "거래량 평균 기간",
      ...
      "help": "돌파일 거래량을 비교할 이동평균 기간. 🔴 상한을 정하는 것은 보유 기간이 아니라 `get_recent_daily` 의 100행 읽기 클램프다(보유는 retention 390일 ≈261 영업일)."
    },
    {
