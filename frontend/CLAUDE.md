...
- **시각 표시는 KST 강제** — `Intl.DateTimeFormat('ko-KR', { timeZone: 'Asia/Seoul', ... })` 또는 `toLocaleString('ko-KR', { timeZone: 'Asia/Seoul', hour12: false })`. `new Date(iso).getHours()/getFullYear()` 등 브라우저 로컬타임 추출 금지 (도커 빌드 UTC / 다른 TZ 환경 어긋남). 적용: `TradeHistoryGrid.formatDate/formatTime` (Intl.DateTimeFormat formatToParts) / `DailyReportTab.formatDateTime` / 모든 timestamp 필드. 백엔드 `_to_kst` 헬퍼와 동일 컨벤션
  - **cycle256 (2026-09-05) — 단일 진실원 `src/utils/kst.ts`**: `formatKstHHMM(iso)`(HH:mm, `hour12:false`) · `formatKstDateTime(iso)`(`yyyy-MM-dd HH:mm:ss`, `formatToParts` 조립) · `kstTodayISO(now?)`. 잘못된 입력은 `'—'`. **새 KST 표기는 이 유틸에 위임**한다(신규 `Intl.DateTimeFormat`/`toLocale*String` 생성 금지 — `utils/__tests__/kst.test.ts` K4 텍스트 가드가 위임 컴포넌트 목록 `DELEGATING_COMPONENTS` 로 잠근다). 현재 위임 = `PortfolioRiskCard`(출력 byte 동일 실증). 기존 13 사이트와 `DailyReportTab.formatDateTime` 은 **사이트별 출력 스냅샷 승인 후 점진 이관**(DailyReportTab 은 `toLocaleString` 서식 `2026. 9. 7. 9시 5분 0초` → `2026-09-07 09:05:00` 으로 바뀌는 **사용자 가시 변경**이라 결정 대기 — HEAD 서식은 ICU 의존이라 Node/브라우저 간 다르다). ⚠️ **타입 검사는 `npx tsc -b`** — 루트 `tsconfig.json` 이 `files: []` 솔루션 형식이라 `npx tsc --noEmit` 은 파일을 0개 검사한다(cycle256 F3 실측: `--listFiles | wc -l` → 0). `GateLevel` 유니온 등 타입 계약은 `tsc -b`(=`npm run build` 1단계)만 잰다.
