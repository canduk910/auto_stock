...
- **ScanMonitor**
  ...
  - **BFB/VCP 가시화 (사이클 13, 2026-05-18)**: `BREAKOUT_KEYS` 4종 확장 (`volatility_breakout` / `long_tail_volatility` / `bull_flag_breakout` / `vcp_breakout`) + `BREAKOUT_LABELS` "눌림목 돌파"/"VCP 변동성 수축" 라벨 추가. BFB/VCP 도 `isBreakout` 분기로 운영시간 안내 + 타겟 가격 테이블 자동 재사용 (둘 다 MAIN only 단일 보드). 백엔드 응답 키 무변경 — 기존 `strategies[key].scanned_count` / `targets` / `params` 만 사용 (BFB/VCP 도 이미 `get_scan_stats()` 보유). 회귀 가드: `frontend/src/components/__tests__/ScanMonitor.bfb_vcp.test.tsx` 3 케이스 (C13-A 전체 탭 카운트 카드 / C13-B BFB 타겟 가격 / C13-C VCP 타겟 가격)
