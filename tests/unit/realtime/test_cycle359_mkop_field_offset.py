...
`[0] MKSC_SHRN_ISCD` 와 같은 모양). 그런데 `src/api/market_operation.py::parse_market_op_payload`
와 `src/realtime/handler.py::_handle_market_op` 은 통합 표(`docs/kis/domestic-stock-realtime.md`
「장운영정보 (통합)」 — 종목코드 칸 없음)를 그대로 따라 `fields[0]` 을 `TRHT_YN` 으로 읽는다.
그래서 모든 칸이 한 칸씩 앞으로 당겨진다.

| 라이브 위치 | 실제 의미 (ticker-first) | 실측 값 | 지금 파서가 넣는 곳 |
|---|---|---|---|
...
가짜 VI 는 `_vi_active_tickers` 에 들어가고, `55`/`57` 은 장중에 바뀌지 않으므로
`[market_op_vi_release]` 가 한 번도 안 나온다. 그 결과 K stale watcher 와 5분 우선 재구독이
**보유 종목**을 그날 21:30 정산까지 건너뛴다(`[stale_skip_market_op]`).

## 두 번째 덫 (칸만 고치면 안 된다)

칸을 바로 읽으면 `55`/`57` 이 `iscd_stat_cls_code` 로 간다. 그런데 `_is_code_active` 는
`{"", "0", "N", "n"}` 만 비활성으로 보므로 `55`·`57`·`00`(그 외 종목) 이 전부 「종목상태 이상」
= **거래정지 취급**이 된다. 가짜 VI 가 가짜 거래정지로 이름만 바뀌고 재구독 skip 은 그대로다.
그래서 이 파일은 **파싱 값**과 함께 **최종 행위**(`is_ticker_stale_excluded` · 재구독 여부)를 단언한다.
...
현재 코드에서 실패해야 하는 테스트는 `xfail(strict=True, raises=AssertionError)` 다 —
수정이 착지하면 XPASS 가 되어 strict 로 붉어지므로 그때 마커를 지운다.
`raises=AssertionError` 는 하네스 오류(ImportError·AttributeError)로 「우연히 실패」하는 것을
xfail 로 삼키지 않기 위해서다. 같은 하네스의 **대조군**(마커 없음)이 초록인지도 함께 본다.
...
_XFAIL = pytest.mark.xfail(
    ...
    reason="cycle359 F-B — H0UNMKO0 첫 칸(종목코드) 미인식 + 종목상태 55/57 을 이상으로 판정",
)
...
async def test_A1_websocket_layer_takes_tr_key_from_first_payload_field() -> None:
    ...
    종목코드라는 사실은 우리 코드가 이미 전제하고 있다** — 같은 칸을 파서가 `TRHT_YN` 으로
    다시 읽는 것이 모순이다. 이 테스트는 그 전제를 고정한다(수정 전후 모두 초록).
    """
...
def test_B1_trht_yn_is_second_field_not_ticker() -> None:
    """거래정지 여부는 [1] 이다. 지금은 [0](종목코드)이 들어가 거래정지 판정이 영원히 거짓."""
...
def test_B2_all_fields_follow_ticker_first_layout() -> None:
    ...
    assert got == ("N", "(null)", "AB1", "112", "", "", "55", "N", "", ""), got


@_XFAIL
def test_B3_vi_code_is_not_credit_status() -> None:
    """VI 칸에 종목상태(55 신용가능)가 들어가면 안 된다 — 운영 `[market_op_vi_active] vi_code=55` 의 원인."""
...
def test_C1_live_frame_without_vi_is_not_stale_excluded(ticker: str, frame: str) -> None:
    ...
    ⚠️ 칸만 고치고 `_is_code_active` 를 그대로 두면 55/57 이 「종목상태 이상」으로 읽혀
    이 테스트는 **여전히 실패**한다(가짜 VI → 가짜 거래정지로 이름만 바뀜).
    """
...
def test_C2_vi_on_then_off_releases() -> None:
    """[8]=Y 프레임 → VI 활성, 다음 [8]=N 프레임 → 해제. 지금은 55 로 활성된 뒤 **영원히 안 풀린다**."""
    mom.record_market_op_event(parse_market_op_payload("003160", FRAME_003160_VI_ON))
    assert "003160" in mom.get_vi_active_tickers()        # 지금도 참 — 단 이유가 틀렸다(55)
...
def test_C3_halt_frame_is_detected_as_halt_not_vi() -> None:
    ...
    지금은 [1]=Y 가 `tr_susp_reas_cntt` 로 가고 58 이 VI 칸으로 가서 「VI」로 오분류된다.
    """
...
def test_C5_halt_status_58_still_excluded_control() -> None:
    """대조군(지금도 초록) — 58 = 거래정지 지정 종목은 stale 회피 대상이다. 수정 후에도 유지."""
    event = MarketOpEvent(
...
async def test_D2_board_callback_receives_real_mkop_code() -> None:
    """`_on_board` 에 넘기는 장운영 코드는 [3] 이다. 지금은 [2](거래정지 사유 `(null)`)가 간다.

    🔴 수정 위치가 `src/realtime/handler.py:746` = **8영역**이다(승인 필요). 파서만 고치는
    최소 수정안에서는 이 테스트가 xfail 로 남는다 — 행위 영향은 없다(메모 §4 참조:
    `session.is_call_auction_now` 는 110/121 만 보는데 라이브 코드는 `AB1` 이라 고쳐도 같은 분기).
    """
...
async def test_E1_control_held_stale_ticker_is_resubscribed(monkeypatch) -> None:
    """대조군(지금도 초록) — 장운영 프레임이 없으면 stale 보유 종목은 HIGH 로 재구독된다.

    이 하네스가 초록이어야 E2 의 실패가 「하네스 고장」이 아니라 「가짜 VI skip」 때문임이 선다.
    """
