"""사이클 90 L-2 — 사이클 88 G-REJECT-1/2/3 영속 가드 (사이클 90 영역 무영향).

Red 명세 (`_workspace/red/cycle90_manual_refresh_universe_api.md`):

사이클 90 = POST refresh-universe 라우트 영역 (사이클 89 영속 영역의 단순
진입점 추가). 사이클 88 G-REJECT-1/2/3 매트릭스 영역과 완전 분리:
- G-REJECT-1 (4중 안전망 영속) — 영향 0
- G-REJECT-2 (종목별 stale 영속, `ticker_last_tick`) — 영향 0
- G-REJECT-3 (4 dict 분리 영속, `_subscriptions` / `_subscriptions_acked` /
  `_ticker_to_session` / `ticker_last_tick`) — 영향 0

기대 동작 (Green, 사이클 90):
- 사이클 88 G-REJECT-1/2/3 AST 가드 영속 (변경 0)
- 사이클 90 시정 영역이 G-REJECT 영역 침범 0
- 사이클 89 영속 영역 영향 0

Red 상태 (사이클 90): 사이클 88 G-REJECT 영역 침범 시 FAIL (영속 위반).
...
def test_l2_cycle88_g_reject_test_file_persists():
    """L-2: 사이클 88 G-REJECT-1/2/3 AST 가드 파일 영속 확인.
    ...
    Red 상태 (사이클 90): G-REJECT 파일 부재 또는 일부 누락 시 FAIL.

    Green (사이클 90): backend-dev 가 사이클 90 시정해도 G-REJECT 무변경 → PASS.
    ...
    assert _G_REJECT_PY.exists(), (
        f"\n사이클 90 L-2 위반 — 사이클 88 G-REJECT 가드 파일 부재:\n"
        f"  파일: {_G_REJECT_PY}\n"
    ...
    for reject_id in ("G-REJECT-1", "G-REJECT-2", "G-REJECT-3"):
        assert reject_id in source, (
            f"\n사이클 90 L-2 위반 — 사이클 88 {reject_id} 영역 누락:\n"
            f"  파일: {_G_REJECT_PY.name}\n"
            f"  영속 의무: 사이클 88 G-REJECT-1/2/3 모두 영속"
        )


def test_l2_cycle88_ticker_last_tick_persists_in_scanner():
    """L-2: 사이클 88 G-REJECT-2 영역 (`ticker_last_tick` 영속) 검증.
    ...
    - 사이클 29 005935 사고 패턴 영구 차단
    - 사이클 90 = POST 라우트 영역 (routes) = `ticker_last_tick` 영역과 분리 = 영향 0
    """
    ...
    assert "ticker_last_tick" in source, (
        ...
        f"  - 사이클 90 = POST 라우트 영역 = ticker_last_tick 영역과 분리 = 영향 0"
    )


def test_l2_cycle88_4_dict_separation_persists_in_websocket():
    """L-2: 사이클 88 G-REJECT-3 영역 (4 dict 분리 영속) 검증.
    ...
    - 사이클 90 = POST 라우트 영역 = 4 dict 영역과 분리 = 영향 0
    """
    ...
    assert websocket_py.exists(), (
        f"\n사이클 90 L-2 위반 — `src/realtime/websocket.py` 부재 (영속 위반)"
    )
    ...
    for dict_name in ("_subscriptions", "_subscriptions_acked"):
        assert dict_name in ws_source, (
            f"\n사이클 90 L-2 위반 — 사이클 88 G-REJECT-3 dict 누락 (websocket.py):\n"
            f"  누락 dict: `{dict_name}`\n"
    ...
    assert "_ticker_to_session" in pool_source, (
        f"\n사이클 90 L-2 위반 — 사이클 88 G-REJECT-3 `_ticker_to_session` 누락 (websocket_pool.py):\n"
        f"  영속 의무: 사이클 88 G-REJECT-3 (4 dict 분리 영속) 영구\n"
        f"  - SubscriptionRegistry 단일 dict 통합 영구 차단\n"
