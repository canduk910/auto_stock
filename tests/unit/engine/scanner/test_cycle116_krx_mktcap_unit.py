"""사이클 116 — KRX MKTCAP (원 단위) → KIS hts_avls (백만원 단위) 환산 매핑 가드.

사이클 115 통합 KRX 1차 분기 활성 시 사이클 108 `list_by_filter` 정합 의무.
사이클 108 영역: `hts_avls × 1_000_000 ≥ min_market_cap` (백만원 단위 가정).
KRX `MKTCAP` 영역: 원 단위 직접 → 변환 없이 그대로 사용 시 1,000,000배 왜곡.

시정: `_full_universe_load_krx_primary` 영역에서 KRX `MKTCAP / 1_000_000` → `hts_avls` 키.
...
def _simulate_krx_raw_merge(trd_row: dict) -> dict:
    """`_full_universe_load_krx_primary` 의 raw merge 영역 정밀 재현 (사이클 116 영역 포함).
...
def test_h1_krx_mktcap_to_hts_avls_conversion():
    """HIGH-1: KRX MKTCAP (원) → hts_avls (백만원) 환산 정합.

    예: 삼성전자 시가총액 500조원 = 500_000_000_000_000 원 → hts_avls = 500_000_000 (백만원).
    """
    trd_row = {"ISU_CD": "005930", "ISU_NM": "삼성전자", "MKTCAP": "500000000000000"}
    raw = _simulate_krx_raw_merge(trd_row)
    assert raw["hts_avls"] == 500_000_000  # 500조원 → 5억 (백만원 단위)
    assert raw["MKTCAP"] == "500000000000000"  # 원 단위 raw 영속


def test_h2_list_by_filter_compatibility_via_hts_avls():
    """HIGH-2: 사이클 108 list_by_filter 호환성 — hts_avls × 1_000_000 영역.

    예: min_market_cap=100조원 (100_000_000_000_000 원) 필터 →
    삼성전자 hts_avls=500_000_000 × 1_000_000 = 500조원 ≥ 100조원 → 통과.
    """
    ...
    assert hts_avls * 1_000_000 >= min_market_cap
...
def test_m2_mktcap_persistence_no_overwrite():
    ...
    assert raw["hts_avls"] == 1_000_000  # 1조원 / 1_000_000 = 100만 (백만원)
...
def test_g_ast1_source_code_conversion_present():
    ...
    # 환산 영역 핵심 패턴 (`mktcap_won // 1_000_000` + `krx_raw["hts_avls"]`)
    assert "// 1_000_000" in source, "MKTCAP → hts_avls 환산 영역 부재 (사이클 116 시정 회귀)"
