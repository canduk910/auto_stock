...
def test_fail_n_days_triggers_stop_loss_after_n_days():
    ...
    원 픽스처(매수 05-20 수 / 평가 05-25 월)는 달력 5일이지만 **영업일은 3일**
    (21목·22금·25월) 이라 n_days=5 에 미달한다. `n_days` 값(5)을 낮추면 테스트 의도가
    "N일 도달 시 발화"에서 "더 짧은 N 에서 발화"로 변질되므로, **n_days 를 유지하고
    평가일을 05-27(수) 로 옮겨** 정확히 5 영업일(21·22·25·26·27)을 만든다.
    `_trading_days` 캐시가 비어 있으므로 weekday 폴백 경로를 타며, 주말 05-23/24 는
    제외된다 — 폴백조차 기존 달력일보다 엄격히 낫다는 점도 함께 검증된다.
    """
    strat = _make_strat(n_days=5)
    ticker = "005930"
    buy_date = date(2026, 5, 20)  # 5 영업일 전 (05-21·22·25·26·27)
