...
def test_buy_when_breakout_with_volume_then_buy(strat):
    ...
    # cycle228 (A7) — 거래량은 tick_volume 실측 주입 (구 손주입 = P0-1 은폐 장치,
    # 영구 폐기). record 는 freeze **안** — KST 날짜 키 자기 리셋 정합.
    ...
    with freeze_time("2026-05-08 10:00:00"):
        ...
        tick_volume.record_acml_vol("005930", 500_000)  # 300_000 × 1.5 = 450_000 초과
...
def test_buy_when_breakout_but_low_volume_then_none(strat):
    ...
    with freeze_time("2026-05-08 10:00:00"):
        tick_volume.reset_for_test()
        tick_volume.record_acml_vol("005930", 400_000)  # 450_000 미만
        assert strat.check_buy_signal("005930", 12_100, 11_900) == Signal.NONE
    tick_volume.reset_for_test()
