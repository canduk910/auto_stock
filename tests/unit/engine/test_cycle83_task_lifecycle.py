"""사이클 83 G-LC1 — `_scan_pool_eager_refresh_task` lifecycle 검증
(asyncio mock 기반).

명세 (`_workspace/red/cycle83_scan_pool_eager_refresh.md`):

정적 grep (G-AST1) 외에 *실제 stop() 호출 시점에 task lifecycle 동작* 을 검증.
사이클 79 G-LC1 (`test_cycle79_task_lifecycle.py`) 직답습 패턴.

기대 동작 (Green, 사이클 84):
- `done()=False` 인 `_scan_pool_eager_refresh_task` mock 주입 후 `stop()` 호출
  → cancel() 호출 1회 + setattr None 1회

Red 단계 (사이클 83):
- 현재 `stop()` task_attrs 튜플에 `_scan_pool_eager_refresh_task` 미포함
...
# ---------------------------------------------------------------------------
# G-LC1 — stop() 호출 시 _scan_pool_eager_refresh_task cancel + setattr None
# ---------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_g_lc1_stop_cancels_scan_pool_eager_refresh_task():
    """G-LC1: `stop()` 호출 시 `_scan_pool_eager_refresh_task` 가 cancel() + await
    + setattr(None) 처리되어야 한다 — 좀비 task 0 보장 (사이클 79 G-LC1 답습).

    검증:
    - active mock (done()=False) 주입
    - stop() 호출 후:
      1. task.cancel() 호출 1회
      2. sched._scan_pool_eager_refresh_task 가 None 으로 재대입

    Red 상태 (사이클 83): 현재 `stop()` 의 task_attrs 튜플에
    `_scan_pool_eager_refresh_task` 미포함 → cancel() 호출 0회 + attribute 보존
    → FAIL.

    Green (사이클 84): backend-dev 가 task_attrs 튜플에 추가 + 신규 task
    구조 도입 → cancel 1회 + None 재대입 → PASS.
    """
    sched = TradingScheduler()
    sched._running = True

    # 사이클 83 검증 대상 task 단독 주입 (다른 task 는 None — getattr None 분기)
    fake_eager_task = _fake_task(done=False)
    sched._scan_pool_eager_refresh_task = fake_eager_task
    ...
    # 1. cancel() 호출 1회 (사이클 79 G-LC1 패턴 답습)
    assert fake_eager_task.cancel.call_count == 1, (
        f"\n사이클 83 G-LC1 위반 — `_scan_pool_eager_refresh_task.cancel()` "
        f"호출 누락:\n"
        f"  현재 호출 횟수: {fake_eager_task.cancel.call_count} (= 1 필요)\n\n"
        f"  사이클 83 카드 #82-A 옵션 1 = 신규 백그라운드 task 도입\n"
        f"  → stop() task_attrs 튜플 추가 의무 (사이클 79 G-AST1 패턴 답습).\n\n"
        f"  Green 시정 (사이클 84 backend-dev): scheduler.py::stop() 의 \n"
        f"  task_attrs 튜플에 `\"_scan_pool_eager_refresh_task\"` 추가.\n"
        f"  사이클 42 `_heartbeat_metrics_loop` 좀비 task 패턴 답습 (영구 가드)."
    )

    # 2. setattr(None) 재대입 (사이클 13-E-1 패턴)
    assert sched._scan_pool_eager_refresh_task is None, (
        f"\n사이클 83 G-LC1 위반 — `_scan_pool_eager_refresh_task` attribute 가 "
        f"None 으로 재대입되지 않음:\n"
        f"  현재값: {sched._scan_pool_eager_refresh_task!r}\n\n"
