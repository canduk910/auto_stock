...
한다. 본 테스트는 다음 5 케이스를 강제한다:
  ...
  D: `donchian_period`, `long_ma_period` 가 INT_PARAMS 에 등록 → 정수 캐스트
...
# 신규 8 키 명세 (단, min_prdy_rate 는 이미 PARAM_RANGES 에 존재 → 회귀 보존만 검증)
_NEW_RANGE_KEYS = {
...
_NEW_INT_KEYS = {"donchian_period", "long_ma_period"}
...
def test_int_params_includes_donchian_and_long_ma_period():
    """donchian_period, long_ma_period 가 INT_PARAMS 에 등록되어 정수 캐스트 대상."""
...
def test_validate_recommendations_rejects_out_of_range_new_keys(caplog):
    ...
    # WARNING 3건 (각 키 1건씩) — 메시지 prefix "추천 값 범위 초과"
    warning_msgs = [
...
def test_validate_recommendations_casts_donchian_period_to_int():
    """donchian_period 25.7 → 26 (round 후 int)."""
    from src.engine.recommendation_engine import _validate_recommendations
...
def test_existing_validate_flow_still_works():
    """기존 케이스(buy_threshold 통과, ghost_param 제거)가 그대로 동작."""
    from src.engine.recommendation_engine import _validate_recommendations
