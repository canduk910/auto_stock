...
장기 baseline 원천**이다. 이 파일이 사라지면:

- 3년을 넘는 과거분은 **영구 재취득이 불가능**하다(FRED 자체가 더 이상 주지 않는다).
- 하이일드 스프레드 화면의 백분위 판정이 "10년 기준" 에서 "최근 3년 기준" 으로 **영구
  퇴행**한다 — seed JSON 을 다시 넣어도 seed 시점(2026-09-16) 이후 자연 누적분만 살아 있고,
  그 이전 유실분은 seed 재적재로 복구된다(멱등이므로 안전) 하지만 seed 자체를 잃으면 그마저
  안 된다.
