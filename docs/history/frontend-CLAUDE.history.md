> 원본: `frontend/CLAUDE.md` · 이관: 2026-09-17

React 대시보드 정본에서 걷어낸 경위·실측 수치·결정 근거. 규약 = [`README.md`](README.md).
원문 그대로 옮긴다(append-only). 사이클별 보고 원문은 [`../HARNESS_CHANGELOG.md`](../HARNESS_CHANGELOG.md) 에 있다.

정본은 2026-09-17 까지 834행이었고 그 절반이 사이클 번호를 제목으로 단 변경 일지였다
("사용자 보고 verbatim → 진단 → 시정 → 검증 결과 → 운영 효과 → 영속 의무 매트릭스" 형식이 8회 반복).
그 일지가 여기로 온다. 이관 블록은 `---` 두 줄 사이가 원문이고, 그 안의 소제목·번호도 원문 그대로라
이 파일의 목차와 겹쳐 보인다.

## 디렉토리

### 2026-09-03 cycle243 — `client.ts` "baseURL 만 · 인터셉터 없음" 시점 확인

cycle246 이 `withCredentials: true` 를 넣어 이 서술이 바뀌었다.

원문(`frontend/CLAUDE.md:20`, 2026-09-17 이관):
...
### 2026-07 lazy 도입 — 초기 번들 819→656kB 실측과 5페이지 시절 목록

현재는 9 페이지가 lazy 다.

원문(`frontend/CLAUDE.md:39`, 2026-09-17 이관):
...
### 2026-07-24 도입 → 2026-09-12 cycle288 나브 고정으로 뒤집은 경위

`max-w-7xl` 1280px 캡 민원으로 도입했고, 나브 내부 컨테이너까지 동적 폭을 받다가 슬라이더 깜박임(사용자 실측)으로 cycle288 이 나브를 고정했다.

원문(`frontend/CLAUDE.md:41 · 43 · 48-49`, 2026-09-17 이관):
...
### 2026-09-12 cycle288 — 나브 분리·묶음 결정과 적대 검증 지적 2건

`aria-haspopup` 거짓 약속 · e2e 4건 파괴(`getByText` 가 모바일 헤더의 숨은 span 을 집음).

원문(`frontend/CLAUDE.md:51 · 53 · 55 · 69 · 75`, 2026-09-17 이관):
...
### 2026-05-22 ~ 2026-06-26 사이클 39·41·171·175·178 — ScanMonitor STAGES 진단·시정 서사

VCP 키 매핑 시정 · 조건 툴팁 · 잠정 배지 · 합집합 mislabel · 죽은 단계 제거의 사용자 보고와 진단 과정. 현재 계약은 정본 「ScanMonitor STAGES 계약」 한 줄이다.

원문(`frontend/CLAUDE.md:79 · 81 · 83 · 85`, 2026-09-17 이관):
...
### 2026-09-05 cycle261 — 가을 팔레트 전환·별칭 충돌 4건 실측·서체 캐시 결함

AutoStock→DK Stock 개명, `teal≡sky`·`emerald≡blue` 별칭 충돌, ΔE2000 거리 10.5→39·15.9 복원, `/fonts/*.ttf` 가 `no-store` 로 떨어져 4.9MB 재다운로드하던 결함.

원문(`frontend/CLAUDE.md:89 · 92-93 · 95-96`, 2026-09-17 이관):
...
### 2026-09-05~06 cycle256 — `Recommendations.formatDateTime` timeZone 부재 결함과 위임 결정

cycle256-F/G 사용자 결정 인용과 F3 실측(`tsc --noEmit` 이 0개 파일 검사).

원문(`frontend/CLAUDE.md:97-98`, 2026-09-17 이관):
...
### 2026-07-24/25 — kojiro 후보 종목명 배선 경위 (violet 시절 · 정산 20:10 시절)

종목명을 volatile `ticker_names` 에서 `_candidates[ticker]` 로 옮긴 견고성 작업. kojiro 색은 이후 cycle261 이 navy 900 으로, 정산은 cycle283 이 21:30 으로 옮겼다.

원문(`frontend/CLAUDE.md:113`, 2026-09-17 이관):
...
### 2026-08-06 → 2026-08-25 — VCP/BFB 진단 화면 도입 배경과 P0-1 유령 키 확정

「도입 이래 체결 0건」의 원인 추적 화면으로 만들었고, 원인은 2026-08-25 에 매수 최종 관문의 유령 키 거래량 컷으로 확정됐다. P0-1 은 cycle228 게이트 전환으로 종결됐다.

원문(`frontend/CLAUDE.md:114`, 2026-09-17 이관):
...
### 2026-08-06 — StrategyFunnel 병목 강조 도입 경위

원문(`frontend/CLAUDE.md:115`, 2026-09-17 이관):
...
### 2026-05-20 사이클 21 · 사이클 18 — 5 전략 깔때기 원안(VB 8단계)과 한화에어로스페이스 사례

L117~118 의 `price_filtered`/`mcap_pass`/`trade_amount_pass` 는 사이클 178 이 죽은 키로 제거한 단계다(정본 안에 신·구가 함께 남아 있던 자기모순). 사이클 18 라벨의 계기는 한화에어로스페이스(012450) 16:43 운영 결함이다.

원문(`frontend/CLAUDE.md:116-124`, 2026-09-17 이관):
...
### 2026-05 ~ 2026-08 — `PARAM_LABELS` 24키 화이트리스트 편집기 시절

cycle278 이 카탈로그 기반 `StrategyParamsEditor` 로 교체하면서 이 절은 거짓이 됐다. `ExchangeBoardRow` 의 SOR 라디오는 cycle287 이 선택지에서 걷어냈다.

원문(`frontend/CLAUDE.md:128-140 · 145`, 2026-09-17 이관):
...
### 2026-08-18 이전 — 합계로 단위를 추론하던 `totalW <= 1.01` 분기

오염 시에만 깨어나 유효숫자를 파괴하고 "3전략 33.3% 균등분배" 화면으로 결함을 위장했다. 금기와 그 이유 한 문장은 루트 `CLAUDE.md` 핵심 안전 규칙에 있다.

원문(`frontend/CLAUDE.md:147`, 2026-09-17 이관):
...
### 2026-07-31 사이클 D — `BuyBlockSection` 4 모드 실차단 시절 + 무력 배너 도입 경위

레짐 매수 게이트는 사이클 I(2026-08-03)가 제거했고 지금은 표시/관찰 전용이다.

원문(`frontend/CLAUDE.md:179-188`, 2026-09-17 이관):
...
### 2026-06-06 ~ 06-08 사이클 65·75·80 — retry:1 확장과 Playwright LIFO 결함 chain

hotfix #1~#4 의 커밋 해시·PASS 수·결함 chain 서사. 현재 계약은 정본 「테스트 규약」 ①②③④ 네 줄이다.

원문(`frontend/CLAUDE.md:190-206`, 2026-09-17 이관):
...
### 2026-06-06 사이클 62 → 64 — 3 모드·9 testid 에서 단일 필터·7 testid 로 단순화한 경위

원문(`frontend/CLAUDE.md:208-219`, 2026-09-17 이관):
...
### 2026-06-09 ~ 2026-09-07 사이클 85·124·126·127·128·129·150·169·266 — 사이클별 변경 일지 전문

사용자 보고 verbatim · 근본 원인 · 사용자 결정(Q1=A 등) · 운영 실측 수치(13분 39초 토스트, 7,146행, 일봉 보유 49.5%) · 영속 의무 매트릭스 반복. 현재 계약은 정본의 화면 단위 StockMaster 절이고, 신규 데이터 UI 동기화 8단계 절차와 cycle266 방어 변환은 거기 남아 있다.

원문(`frontend/CLAUDE.md:221-503`, 2026-09-17 이관):
...
### 2026-09-11 cycle276 — 사용자 지시 원문과 B-2 종전 키 방식 결함

원문(`frontend/CLAUDE.md:509 · 511-540`, 2026-09-17 이관):
...
### 2026-05-20 사이클 6 검색 통합 · 2026-09-05 W2 ext 정규화 hotfix 경위

객체 자식을 그대로 렌더해 탭 전체가 빈 화면이 되던 실측과 백엔드 이관 권고.

원문(`frontend/CLAUDE.md:547 · 549 · 551`, 2026-09-17 이관):
...
### 2026-08-04 — 섹터 컬럼 추가 시점 표기

원문(`frontend/CLAUDE.md:566`, 2026-09-17 이관):
...
### 2026-05-20 ~ 05-22 사이클 21·35·37·43 — 끊김 영역 이전과 단계별 확장 · 사이클 17 StreamStatus 롤백

원문(`frontend/CLAUDE.md:578-588`, 2026-09-17 이관):
...
### 2026-08-03 사이클 I — 레짐 매수 게이트 제거 반영 서술

원문(`frontend/CLAUDE.md:595`, 2026-09-17 이관):
...
### 2026-09-03~04 cycle243·246·247 — 로그인 다이얼로그가 두 번 뜨던 결함의 추적 전문

"프론트 코드 변경은 0이다" 취소선 · nginx 접근 로그 실측(user 필드 `-`, 재입력 후 `ubuntu`) · Safari 시크릿 창에서 아이콘 넷만 401 → 12초 뒤 재요청까지의 서사.

원문(`frontend/CLAUDE.md:612-636`, 2026-09-17 이관):
...
### 2026-09-04~05 cycle246·249 — 템플릿 주석 키 평문 노출 실측과 리포터 스코프 도입 경위

렌더 결과에 키가 2회 등장하던 실측. nginx/백엔드 인프라 계약의 정본은 루트 `CLAUDE.md` 「Docker / 배포」 절과 `src/routes/CLAUDE.md` 다.

원문(`frontend/CLAUDE.md:643-675`, 2026-09-17 이관):
...
### 2026-06-11 사이클 106 — 절 전문

현재 남는 계약은 정본 StockMaster 절의 두 줄(안내 배너 문구 · 갱신 실패 토스트 문구)뿐이다.

원문(`frontend/CLAUDE.md:684-722`, 2026-09-17 이관):
...
### 2026-06-11 ~ 2026-08-02 사이클 103·186·F — 절 전문

코미코 STOP_LOSS 가시화 배경 · 파일 라인 수 · `#FF3333`/`#3366FF`(cycle261 이전 hex, 지금은 `designSystem.v2.test.ts` 가 소스에서 금지하는 값) · navItems 7→9(cycle288 이전) · 영속 의무 매트릭스 · 운영 효과.

원문(`frontend/CLAUDE.md:724-792`, 2026-09-17 이관):
...
### 2026-06-11 사이클 104 — 절 전문

현재 남는 계약은 정본 「테스트 규약」 ④(Playwright glob ↔ Vite 모듈 resourceType 가드)와 `/api/strategies` e2e mock 단일 형식이다.

원문(`frontend/CLAUDE.md:794-834`, 2026-09-17 이관):
