> 원본: `src/realtime/CLAUDE.md` · 이관: 2026-09-17

정본은 [`src/realtime/CLAUDE.md`](../../src/realtime/CLAUDE.md). 이 파일은 거기서 걷어낸 원문을
...
→ CHANGELOG: 사이클 17 행. 현행 계약(호출자 0건·재도입 금지·G-DEAD-2)은 정본에 남아 있다.

### 2026-09-17 이관 — `get_subscriptions_by_session` 도입 사이클 표기
...
### 2026-09-17 이관 — 세션 라벨 통일(사이클 43) → `quote-N` 파서 잔존 시정(194·195) 경위
...
→ CHANGELOG: 사이클 43 · 194 · 195 행.

## websocket.py — KisWebSocket 연결 관리

### 2026-09-17 이관 — 사이클 197 구독 한도 WARNING cap 도입 경위와 스팸 실측
...
### 2026-09-17 이관 — 사이클 42 `label` kwarg · PINGPONG 가시성 도입 경위와 5/22 운영 관찰 수치
...
### 2026-09-17 이관 — WS action aggregation 옵션 E-1(사이클 74) 결정·예상 효과
...
### 2026-09-17 이관 — 사이클 73 `write_log` 직접 호출 제거와 dup 2.40x→≤2.0x 실측
...
### 2026-09-17 이관 — 사이클 135 `_subscribed_at` grace period 도입 전문(Q3=A · P95 마진 표 · 14 케이스)
...
→ CHANGELOG: 사이클 135 행. 자문 원문 = `_workspace/domain_consult/cycle135_websocket_grace_period.md`.

### 2026-09-17 이관 — 사이클 16 AES 키 격리 가드 도입 전문(2026-05-19 사고 11건)
...
### 2026-09-17 이관 — 사이클 17 OPSP0002 backoff 도입·60s→300s 연장 전문(15:15 ratio=0.0% 사고)
...
### 2026-09-17 이관 — 사이클 25-B `_resubscribe_stale_priority` 분리 표기
...
→ CHANGELOG: 사이클 25-B 행.

## get_subscribed_tickers / get_acked_tickers

### 2026-09-17 이관 — cycle293 "합집합이 참이 됐다" 선언과 프로브 격리 기준 전환 경위
...
→ CHANGELOG: cycle293 행.

## 시세 채널

이관 시점의 절 전문이다. cycle293(2단계) 서술 위에 cycle294(3단계)를 얹고 그 위에 다시
cycle295 철회를 덧칠한 세 층이 그대로 들어 있다. 현행 규칙은 정본의 같은 절을 읽는다.

### 2026-09-17 이관 — 「시세 채널」 절 전문 (cycle257 → cycle293 → cycle294 → cycle295)
...
→ CHANGELOG: cycle257 · cycle293 · cycle294 · cycle295 행.
철회 명세 원문 = `_workspace/red/cycle295_gap_hold_removal_spec.md`.

## handler.py — 메시지 처리

### 2026-09-17 이관 — 실시간 체결가 항목의 "구 서술은 거짓이다" 정정 각주
...
→ CHANGELOG: cycle293 · cycle294 행.

### 2026-09-17 이관 — 체결통보 `[9]`/`[16]` 반대 표기와 08-28 257720 오독 경위
...
→ CHANGELOG: cycle235 행.

## cycle264 (2026-09-06~07) — `[open_scope_observe]` 시가 스코프 shadow 관측 (**행위 변경 0**)

절 전문이다. "아직 열려 있는 것 — cycle265 / 후속 F-2" 는 cycle272(VB·LTV `main` 목표가
기준가 = KRX REST 단일 출처)로 착지해 이관 시점에 이미 참이 아니다.

### 2026-09-17 이관 — cycle264 관측 절 전문
...
→ CHANGELOG: cycle264 · cycle272 행.
포렌식 원문 = `_workspace/analysis/entry_price_0900_20260906/`.

## 사이클 102 (2026-06-11) — 시세 구독 영역 전면 재검토 (3 영역 통합 가시화)

절 전문이다. 인용된 행번호(`websocket.py:162~165`·`:607~609`, `handler.py:56~59`·`:101~113`·
`:223~238`, `scheduler.py L2567~L2571`)는 이관 시점 실측과 이미 어긋나 있었다.

### 2026-09-17 이관 — 사이클 102 절 전문
...
### 2026-09-17 이관 — TICK 3채널 행의 단계별 실 구독 경위와 라이브 실측 표본
...
→ CHANGELOG: cycle252 · cycle253 · cycle293 · cycle294 · cycle295 행.
H0NXCNT0 행의 「NXT 단독 연속 구간(09-15 = 15:40~16:00)의 보유(HIGH)만」 은 cycle295 철회 뒤
코드에 그 창이 없다(`tick_channel_clock.switch_windows` 는 `pre_to_krx` 하나).

## 안전 규칙 (멀티 세션)

### 2026-09-17 이관 — K stale watcher 재SEND 폐기 표기 · 사이클 24 silent inactive 도입(fresh=0) 표기
...
→ CHANGELOG: 사이클 17 · 24 행. 현행 임계는 `SILENT_INACTIVE_FRESH_RATIO_THRESHOLD = 0.2`
(`src/engine/stale_session_recovery.py`)이고 세션 상대 판정(cycle241)은 루트 `CLAUDE.md` 가 정본이다.

## §외부 LLM 검토 영구 기록 (사이클 88, 2026-06-09)

절 전문이다. 현행으로 남긴 것은 G-REJECT-1~3 금기 세 줄뿐이다.

### 2026-09-17 이관 — 외부 LLM 검토 절 전문
...
→ CHANGELOG: 사이클 88 행.
의견서 원문 = `_workspace/external_llm_reviews/2026-06-09_realtime_review.md`.
