> 원본: `_workspace/00_leader_trading_rules.md` · 이관: 2026-09-17

매매 규칙 정본에서 걷어낸 경위·실측 수치·결정 근거. 규약 = [`README.md`](README.md).
...
### 2026-07-20 · 2026-08-03 · 2026-09-10 — kojiro 밴드·신선도·랭킹 성분 경위
...
- **후보 점수 랭킹** (2026-07-20 도입, 2026-09-10 cycle273c 성분①② 원설계 복원): 후보 > 슬롯/섹터캡 경합 시 `0.4×(MACD3 3봉기울기/3/종가) + 0.3×(띠폭/직전5봉평균 − 1) + 0.3×6→1신선도`(후보풀 min-max 정규화 가중합)로 최적 셋업 우선 매수. 이미 계산되나 미사용(dormant)이던 대순환 MACD3·띠폭 지표 배선. **매수 후보 정렬만** — strict entry 자격·청산 임계 무변경, `rank_w_*` 가중치 PARAM_RANGES 제외(정체성 상수). **cycle273c** — 성분①(MACD3기울기)의 `/종가` 누락(주가 순위표화, ρ=+0.853)과 성분②(띠폭확장률)의 단일봉+`1e-9` 분모(6→1 전환 직후 상시 폭발)를 원설계(`/3/종가`, 분모=직전5봉평균)로 복원. shadow 관측 `[kojiro_band_observe]` 동반(leaf `src/engine/kojiro_band_observe.py`) — 복원 전후 두 식을 한 행에 남겨 대조.
...
- **브레이크이븐 플로어 (cycle220, 2026-08-18 다크런치)**: `breakeven_promote_atr`(기본 **0=비활성**, 활성 권장 1.5 — donchian P1 선례). 활성 시 `high_since_buy ≥ 매수가 + mult×ATR` 도달 이력이 있으면 2ATR 손절선을 `max(현행선, 매수가)` 로 승격해 기존 `_stop_floor` tighten-only 래칫에 영속(ATR 팽창해도 유지, 재시작 시 `recompute_held_atr` 이 H-1 복구 고점으로 재도출). **플로어일 뿐 트레일 아님** — 샹들리에 2.5ATR(조임 금기, domain-consult `kojiro_exit_loss_review.md`)가 위쪽 추세 청산을 계속 담당해 fat-tail 랠리 미절단. Σ리스크캡 `_position_stop_price` 도 동일 산식 미러(4선 max). 근거 = 08-18 실측 샹들리에 3청산 전부 +6.7~10.7% 도달 후 전량 반납. **활성화 = N=10 재튜닝 시 DB `strategy_config.kojiro.params.breakeven_promote_atr=1.5` UPDATE + 재부팅**(PARAM_RANGES 미편입 = AI 튜닝 제외).
...
### 2026-05-11 ~ 2026-09-05 — 1주 폴백·이중제한·K축·ρ축 도입 경위와 실측
...
- **랏 명목 ρ축 상한 `max_lot_ratio_mult` (K_ρ = 2.5, cycle245 · 2026-09-04)** — K축(`max_lot_units`, ATR 척도)이 **이 랏을 실제로 심사하지 못한 모든 랏**의 명목을 `K_ρ × position_ratio × 예산` 이하로 자르고, 1주도 못 사면 **매수하지 않는다**. 심사 판정 = `sizing_mode=="turtle"` ∧ `ticker` ∧ `risk_pct>0` ∧ 예산>0 ∧ ATR 해석 성공 — 대상은 **관문을 지나는 모든 랏**이다(비터틀 5전략 전부(momentum·VB·LTV·BFB·VCP) + 터틀이지만 ATR 배관이 끊긴 랏 + **cycle254(2026-09-05)부터 K축 심사에 성공한 랏도 포함**). **두 캡은 `min` 으로 후심사한다**(구 결정 ⑦ "상호배타 — `min` 합성 없음" 폐기) — 사이즈드 터틀 랏·`position_ratio` 낙하 랏은 `compute_unit_qty_guarded` 의 notional 상한으로 이미 컷오프 이하라 항등적으로 무접촉이고, **실효는 1주 폴백 랏뿐**이다. 판정기(`_lot_units_cap_governs`) 자체가 실패한 경우(`probe_error`)만 fail-open 으로 남는다.
  ...
  - **잔여 노출 — cycle254(2026-09-05)로 닫힘** — 옛 잔여 노출(kojiro 3.86배 · donchian 5.00배, K축이 심사한 터틀 1주 폴백 랏이 항등식 밖이던 상태)은 구 결정 ⑦ "상호배타"를 폐기하고 ρ축이 K축 심사 랏도 `min` 으로 후심사하도록 바꾸며 닫혔다(donchian 컷오프 195,150원 · kojiro 323,952원). 사이즈드 터틀 랏·PR 낙하 랏은 `compute_unit_qty_guarded` 의 notional 상한으로 이미 컷오프 이하라 이 변경으로 수량이 바뀌지 않는다 — 영향은 저ATR 고가 1주 폴백 랏(차단)뿐.
...
## 2. 전략 구성 — WebSocket 구독

### 2026-05-12 — G/E1/E2 구독 가시성·우선순위·거절 감지 도입 경위
...
## 2. 전략 구성 — 외부 백테스트 서버 통합

### 2026-05-15 ~ 2026-05-16 — Phase 0~6 구현·결함 A/B/C/D' 시정 전문
...
### 2026-09-12 ~ 2026-09-14 — cycle287/290/291 라우팅 전환 서술과 라이브 증거
...
**cycle287(2026-09-12) — 위 표는 이제 "DB 에 저장된 값" 이고, 실제로 나가는 거래소는
시각이 정한다.** `order_engine._route_exchange_by_clock` 이 09:00~15:30(정규장)·
16:00~20:00(2026-09-14 신설 KRX 애프터마켓)에는 전략의 `exchange` 값과 무관하게
...
**cycle291(2026-09-13, 사용자 결정 "(나)안") — NXT 프리장(08:00~08:50) 매수 호가유형이
`27`(GTP지정가)로 승격됐다.** `27` 은 2026-09-14 신설 NXT 프리마켓 전용호가로, 그
정체성은 **미체결 잔량을 거래소가 프리마켓 종료(08:50)에 일괄 취소**하는 것이다.
지금까지 프리장에 보내던 `00`(시장가 사전 변환 지정가)은 그 자동취소를 받지 못해
미체결이 NXT 정규장(09:00:30~)으로 새고, 그 누출이 **역선택 방향으로만** 치우쳐
있었다(프리장 목표가는 `main` 시가로 09:00:05 에 다시 계산되므로, 넘어간 주문은
이미 폐기된 목표가의 잔상이다). `ORD_UNPR` 은 `step_up(현재가,5)` — 오늘 `00` 이
보내는 값과 **완전히 같다**. 게이트 4중(실전·NXT·`market_state` 표에서 그 시각 유효·
프리장) 중 하나라도 실패하면 `00` 이 그대로 나간다(fail-safe, 파라미터 없음 — 롤백은
1커밋 revert). **매도는 바꾸지 않는다** — GTP 매도는 08:50 자동취소가 `_selling` 을
≈09:45 까지 잠가 손절 재평가를 억제하고, 현행 `00`은 09:00:30 NXT 정규장에서 시세
아래 지정가라 거의 확실히 체결돼 안전망으로 기능한다(청산 경로를 좁히지 않는다는
루트 CLAUDE.md 원칙 판정). 상세 = `src/engine/CLAUDE.md` §order_engine.py 「cycle291」
절, 자문 = `_workspace/domain_consult/cycle291_pre_nxt_gtp_20260913.md`.
...
## 3. 전략 A: 상한가 모멘텀 — 당일 손절

### 2026-05-07 ~ 2026-06-11 — 손절 임계 시계열과 코미코(183300) 회고
...
## 4. 전략 B: 변동성 돌파 — 보드별 시가 확정

### 2026-05-14 — board 자동 결정 race 사고와 Red 테스트 의무
...
## 5. 전략 C: 롱테일 변동성 돌파 — 09:00 직후 진입 보류

### 2026-09-06 — cycle262 도입 근거 통계·한계·in-sample 경고 전문

### 09:00 직후 진입 보류 `open_entry_hold_secs` (cycle262, 2026-09-06) — **지혈이며 근본 시정이 아니다**
...
- **왜**: 목표가의 기준 시가가 KRX 09:00 시가가 아니라 통합 채널 `H0UNCNT0` `fields[7]`
  (= **세션 시가**, 프리장 체결이 있었으면 프리장 시가)다. 09:00~09:01:30 코호트는
  "체결가 ≥ KRX시가+offset" 을 **20/20 전부 위반**(이후 구간 53/93 = 57%), Fisher p=6.4e-5.
  LTV 는 09:00 에 보드가 main 으로 넘어갈 때 `_open_confirmed['main']` 이 False 라
  **같은 프리장 시가로 main 목표가를 다시 굳힌다**(09:35 스탬프 실측 3/3 불일치).
- **⚠️ 지혈이다**: 오염된 `[7]` 은 **일-스코프 상수**라 90초 뒤에도 값이 그대로다. 이 보류는
  통계적으로 최악인 구간만 피할 뿐 오염 자체를 고치지 않는다. **근본 시정** = `[7]` 에
  `[24] OPRC_HOUR` 스코프 필터를 걸어 KRX 개장 이후의 시가만 채택하는 것이고,
  `src/realtime/**` = **8영역**이라 별도 승인 + 별도 `domain-consult` 사안이다(후속 티켓).
- **보드 무관 — 시간창 단독 판정**. `tradable_boards` 로 분기하지 않는다(09:00:00~09:00:30
  은 세션 트래커 30초 주기 탓에 보드가 아직 `pre_nxt` 로 잡힐 수 있는데 그건 stale 캐시
  산물이라, 보드로 나누면 그 30초가 통째로 구멍이 된다). **LTV 의 08:00~09:00 진짜
  프리장 매수는 창 밖이라 무접촉**이다(그 구간의 `[7]` 은 그 보드의 올바른 기준가다).
- **판정 자리 = 돌파 발사점**(계좌 SOFT 게이트 뒤). 최상단 금지 — 보류 구간에 `_prev_price`
  baseline 이 동결돼 해제 후 첫 틱이 거짓 돌파가 되고(cycle233 C233-F1), 무엇을 살
  뻔했는지도 기록할 수 없다. **보류 중에도 baseline 갱신은 계속된다.**
- **fail-open 이 계약**: 키 부재·`None`·빈 문자열·파싱 실패(`inf`/`nan` 포함)는 전부
  **0 = OFF = 현행 행위**. 읽는 쪽 `[0, 600]` 클램프(오타 `10000` 이 오전을 통째로
  막는 것을 차단). fail-closed 는 P0-1 유령 키가 BFB/VCP 를 전 기간 체결 0건으로 만든 그 방향.
- **관측 2종** (둘 다 INFO, 관측 실패는 매수 판정을 **절대** 바꾸지 않는다):
  - `[open_entry_hold_config] strategy= hold_secs= until= source=` — 그날 적용값 카나리아.
    cap 은 **1회/(전략, 값)/일**(계약 문언 "1회/전략/일" 에서 의도적 이탈 — 장중 PUT
    롤백 후 바뀐 값을 확인할 마커가 0개가 되는 cycle245 R1 사각을 막는다). 정상 운영에선
    값이 안 바뀌므로 **1행/일**이고, 값을 바꾼 날에만 2행이 된다.
    ⚠️ `source` 는 **출처가 아니라 값 동등성 추론**이다 — 적용값이 `DEFAULT_PARAMS` 와
    같으면 `default`, 다르면 `db`. `PUT` 으로 90 을 재확정한 날에도 `default` 로 보인다
    (진짜 출처는 `_load_strategy_config`·`update_params` 가 제자리 덮어쓰기라 관측 불가).
    ⚠️ **알려진 비대칭** — 계좌 SOFT 게이트(cycle233)가 활성인 날에는 **LTV 의 config
    카나리아가 0행**이다. LTV 는 `GATE_FIRST_FILES` 멤버라 게이트 If 가 `check_buy_signal`
    의 **첫 문장**이어야 하고(cycle233 M6, AST 로 봉인) 로그 emit 은 부작용이라 그 앞에
    둘 수 없다. VB 는 게이트가 발사 직전이라 영향 없다. 게이트 활성일에 LTV 적용값을
    확인하려면 `PUT` 응답 또는 `strategy_config` 를 직접 본다(후속 F-6).
  - `[open_entry_hold_blocked] ticker= board= current_price= target= board_open= prev= k=
    offset= elapsed_secs= prdy_close=` — **would_buy 정본**, 1회/(ticker, 전략)/일.
    이 사이클의 핵심 산출물이다. ⚠️ **`system_logs` 의 전략 INFO 는 실측상 최근 며칠만
    잔존**하므로 재검토 창(2주)이 닫히기 전에 일일 추출 적재가 필요하다(후속 티켓).
- **롤백**: `PUT /api/strategies/{id}/params {"open_entry_hold_secs": 0}` — **즉시** 반영
  (라우트가 in-memory `config.params` 를 덮는다). `strategy_config` SQL UPDATE 는
  **다음 백엔드 재시작에서만** 반영되고, cycle232 D6 가 보유 중 장중 재시작을 금지하므로
  **장중 실효 수단은 PUT 뿐**이다. 키가 전략별이라 VB 90 을 유지한 채 LTV 만 끌 수 있다.
- **`PARAM_RANGES`/`INT_PARAMS` 편입 금지** — 진입 정체성 상수. AI 자문 자동 적용 경로에도
  넣지 않는다(최근 손실을 목적함수로 삼는 튜너는 n≤20 에 과적합해 "최근 손실 거래를 지우는
  값" 으로 수렴한다 — cycle223 선례). AST 가드 G-262-1 이 런타임 dict + 소스 리터럴 이중으로 강제.
- **⚠️ DB 선반영 금지** — `_load_strategy_config`(scheduler)도 `PUT /api/strategies/{id}/params`
  (routes)도 **"코드에 이미 있는 키만 덮는" 오버레이**라, 배포 전 PUT 은 미지 키를 조용히
  버리는 **무음 실패**이고 `params` JSONB 를 통째로 덮어 먼저 넣은 SQL 값까지 지운다
  (cycle245 가 실제로 겪은 경로). **배포 후에 PUT 한다.**
- **선택 마커 `[open_entry_hold_release]` 는 미구현**(자문 §2.6 세 번째 마커, 후속 F-4).
  그때까지 "막고 나서 더 좋은 값에 샀나" 는 `trade_history` 조인으로만 복원되고
  **"못 샀다" vs "더 좋은 값에 샀다" 가 구분되지 않는다.**
- **해제 조건(자문 ⑧, 조건부 만료)** — (i) 근본 시정(`[24] OPRC_HOUR` 스코프 필터) 배포
  **그리고** (ii) 시정 후 20 거래일 동안 `[open_entry_hold_blocked]` 가 그 창에서 발화 0~희소.
  **둘 다 충족 전에는 유지**한다. **자동 재검토 트리거(⑨)** = 블록된 거래의 **추정** 손익이
  2주 연속 양(+)이면 창 축소·해제를 재논의한다(레짐이 갭업→갭앤고로 바뀌면 막은 거래가
  승자가 된다). 그 추정은 진입 다리만 로그로 남고 **청산 다리는 시뮬레이션**이다 —
  선행 리뷰 n=36 에서 실측 평균 +58.5bp vs 시뮬 +59.0bp, 상관 **0.79**. 보고할 때
  "실측"이 아니라 **"추정(상관 0.79)"** 으로 표기한다.
- **한계(인용 전 필독)** — ① **in-sample**: 90초라는 폭을 그것으로 판정될 바로 그 20건에서
  읽어냈다. 손익 축(p=0.039/0.047)은 다중검정 보정 시 사라질 수 있고, **행동의 근거는
  손익이 아니라 메커니즘(위반율 p=6.4e-5)** 이다 ② **완치가 아니다**: 09:01:30 이후에도
  57.0% 가 위반한다. 보류 후 VB 성적이 기대만큼 안 좋아져도 그건 이 조치의 실패가 아니라
  **근본이 안 고쳐졌다는 증거**이고, 좋아져도 근본 시정을 미룰 근거가 되지 않는다
  ③ **벽시계 임계의 구멍**: 세션 트래커가 30초 지터로 늦게 뒤집히면 같은 거래가 09:01:31
  에 그대로 난다. 실측 경계 = **09:01:34 1건**(07-22 064350, 위반 −76.2bp)으로 창 밖 4초다.

---

## 5. 전략 C: 롱테일 변동성 돌파 — `main` 기준가 출처

### 2026-09-10 — cycle272 좁은 목 설계와 8영역 무접촉 증명
...
## 5. 전략 C: 롱테일 변동성 돌파 — AI 매수평가 `llm_gate_*`

### 2026-09-10 ~ 2026-09-11 — cycle274 신호 시점 → cycle276 주문 시점 이동 경위와 사용자 지시 원문
...
## 5. 전략 C: 롱테일 변동성 돌파 — 15:20 상한가 유지 확인

### 2026-09-25 cycle352 — B안 채택 경위와 실측

정본 문장 원문(`_workspace/00_leader_trading_rules.md:22`, 2026-09-25 이관):

---

- **cycle352 15:20 상한가 유지 확인** — 근거 = `_workspace/domain_consult/cycle352_ltv_limit_up_trailing.md` 실측. 상한가 모드의 실현 손실은 이미 `overnight_stop_loss`(운영 DB −3.5%)에서 멈추고, 「23% 반납」은 고점 기준 평가이익 감소폭이라 실손이 아니다(모집단 373건 중 15% 넘게 되밀린 경우는 2%). 이름 그대로의 당일 트레일링(폭 7~15%)은 전부 평균 수익을 깎아 채택하지 않았다. 대신 「15:20 에 아직 상한가 근처인가」한 번만 물어, 아니면 그날 종가에 판다 — 평균 수익은 거의 그대로이고 −5% 이하 손실 표본이 절반으로 준다.
...
## 6. 전략 D: 20일 신고가 스윙 — 일중 시세 REST 폴링

### 2026-05-15 — 결함 B 배경 실측과 Red 테스트 의무
...
### 2026-05-27 — 사이클 48 유니버스 소스 교체(거래량순위 API) 경위
...
## 6-E. 전략 E: 눌림목 돌파 — funnel 단계별 사유

### 2026-06-01 — 사이클 50 진단 인프라 도입 배경(폴 검출 0 전멸)
...
## 6-E. 전략 E: 눌림목 돌파 — 매수 거래량 컷

### 2026-08-25 ~ 2026-08-28 — P0-1 유령 키 `acml_vol` 진단과 종결(cycle227 배관 → cycle228 게이트)
...
## 6-F. 전략 F: 변동성 수축 돌파 — 추세 필터

### 2026-05-27 — 사이클 48 추세필터 0건 결함 진단과 EMA 하향 근거
...
## 6-F. 전략 F: 변동성 수축 돌파 — 조정 시퀀스

### 2026-05-31 — 사이클 49 Pullback "마지막 폭 0.0%" 결함 시정 4항 전문
...
## 6-F. 전략 F: 변동성 수축 돌파 — 매수 거래량 컷

### 2026-08-25 ~ 2026-08-28 — P0-1 유령 키 `acml_vol` 진단과 종결(VCP 몫)
...
## 6-F. 전략 F: 변동성 수축 돌파 — 구독 배선

### 2026-05-27 — 사이클 48 BFB/VCP 구독 배선 편입(PR #15 코드리뷰) 경위
...
## 7. 공통 규칙 — AI 자문 고도화 · PARAM_RANGES 확장 · 비중조절 사유 분리

### 2026-05-12 ~ 2026-05-17 — Phase J4 / Phase B / Phase A2 / 사이클 1 명세 전문
...
## 작업 로그 (사이클 1~48 · 사이클 H)

### 2026-05-13 ~ 2026-08-02 — 사이클별 명세·자율 결정·회귀 가드·베이스라인 전문
...
## 6-E. 전략 E: 눌림목 돌파 — 부분 매도 1차 구현 재량

### 2026-05-27 ~ 2026-08-06 — 정본에서 걷어낸 원문
...
> 원본: `_workspace/00_leader_trading_rules.md` · 이관: 2026-09-17 · 아래 각 항목은 정리 1차 뒤 남아 있던 시점 주석·폐기 값을 걷어낸 원문 그대로다.

### ### 외부 백테스트 서버 통합 — 흐름 행

```
`_enqueue_backtest_jobs` 가 12 row(6 전략 × `params_kind` = current|recommended) INSERT → 60초 주기 `_backtest_poll_loop` → 완료분을
```

### ### 외부 백테스트 서버 통합 — YAML DSL 행

```
**표현 불가 3종** = `long_tail_volatility` / `bull_flag_breakout` / `vcp_breakout` →
```

### ### 익일 청산 (다음 영업일 NXT 프리 08:00 → NXT 거래가능 여부로 분기) — (a) 갭상승 미달
...
### ## 6. 전략 D: 20일 신고가 스윙 (donchian_swing) 상세 — 매수 규칙 — 매수 평가 채널
...
### ## 6-E. 전략 E: 눌림목 돌파 (bull_flag_breakout) 상세 — 셋업 검증 — 음봉 비율
...
### 2026-09-17 cycle298 이관 — 07:55 부트 시각표 · 사이클 92 시각 이설 주석 · 두 시간 상수 행

`TIME_BOOT`(07:55) 과 `TIME_POST_NXT_OPEN`(15:40) 은 런타임 참조가 0건이다(2026-09-17 grep 전수).
`_boot()` 는 `scheduler.start()` 안에서 즉시 돌고(`scheduler.py:600`), POST_NXT 전환과
`post_nxt` 시가 확정은 `TIME_KRX_MAIN_CLOSE`(15:30) 직후다(`scheduler.py:865-874`).
아래가 그 전까지 정본에 있던 원문이다(사이클 92 시각 이설 주석 = 신·구 병기라 함께 이관).
...
그 밖에 `_boot()` 에 `07:55` 를 붙여 부르던 서술 7곳(L225 Eager 갱신 · L312·L446·L519·L654 절 제목 ·
L477 트레일링 영속 · L513 거래대금 컷 · L926 `allocate_funds` 재호출 · L1009 보조 계정 복귀)을
`_boot()`(기동 직후) 로 바꿨다. 보드 축 서술(`post_nxt` 15:40~20:00 · 야간 매매 절 제목)은
`session._BOARD_SCHEDULE` 기준으로 **사실이라 그대로 뒀다**.

→ CHANGELOG: cycle298 행


### 2026-09-17 cycle298 이관 — 시각표 15:40 행의 「15:39:10 사전 구독」

시각표 15:40 행 꼬리의 `(15:39:10 사전 구독)` 을 걷어냈다. 그 시각에 도는 코드가 없다 —
`src` 전체에서 15:39 를 쓰는 런타임 코드는 0건이고(2026-09-17 grep 전수), 유일한 출처는
`src/engine/session.py:60` 의 주석 한 줄이다. 그 주석은 8영역 파일 안이라 이번에는 그대로 뒀다
(사용자 결정 2026-09-17 — 규칙 문서만 고친다. 주석은 다음에 `session.py` 를 승인받아 만질 때 함께).
...
## 전략별 자금 비중 — 랏당 최대 유닛 상한 `max_lot_units`
