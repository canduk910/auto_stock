> 원본: `src/engine/strategies/CLAUDE.md` · 이관: 2026-09-17

전략 계약 정본에서 걷어낸 경위·실측 수치·결정 근거. 규약 = [`README.md`](README.md).
원문 그대로 옮긴다(append-only). 사이클별 보고 원문은 [`../HARNESS_CHANGELOG.md`](../HARNESS_CHANGELOG.md) 에 있다.

이관 블록은 `---` 두 줄 사이가 원문이고, 그 안의 소제목·번호·강조도 원문 그대로라
이 파일의 목차와 겹쳐 보인다. 절 제목은 이관 당시 정본의 절 제목을 그대로 쓴다.

## (머리말)

### 2026-07 — kojiro 다크런치 표기

kojiro 는 2026-08 운영 DB 에서 `enabled=True` 로 돌고 있었고 같은 파일 표 kojiro 행과 모순이었다.

원문(`src/engine/strategies/CLAUDE.md:3`, 2026-09-17 이관):
...
### 2026-08-28 cycle228 — BFB·VCP 매수 게이트 개방 배너 원문

P0-1 유령 키 `acml_vol` 경위와 cycle227 관측 마커 `[*_vol_gate_observe]` 은퇴 경고를 담고 있었다.

원문(`src/engine/strategies/CLAUDE.md:7-29`, 2026-09-17 이관):
...
→ CHANGELOG: cycle227 · cycle228 행

### 2026-09-06 cycle262 — VB·LTV `open_entry_hold_secs` 배너 원문

'지혈이며 근본 시정이 아니다' 판단, 20/20·53/93 Fisher 실측, 통합 채널 `fields[7]` 오염 근거, 후속 F-1/F-4 와 재검토 창 2주.

원문(`src/engine/strategies/CLAUDE.md:31-69`, 2026-09-17 이관):
...
→ CHANGELOG: cycle262 행

### 2026-09-10 cycle272 — VB·LTV `main` 목표가 REST 단일 출처 배너 원문

통합 채널 `fields[7]` 오염 경위, WS 호출부 byte 동일 검증, 9→19라운드 확장(cycle276 후속 A-2) 이유.

원문(`src/engine/strategies/CLAUDE.md:70-94`, 2026-09-17 이관):
...
→ CHANGELOG: cycle272 행

### 2026-09-11 cycle274→cycle276 — AI 매수평가 훅 이동 배너 원문

신호 시점 훅(cycle274) → 주문 시점 이동(cycle276) 경위, (전략,종목) 래치 → 주문번호 래치, `slip_bp` 개명과 부호 반전, 토큰 한도 400→2000 실측. 4키가 VB·LTV 둘뿐이던 시기의 서술이다.

원문(`src/engine/strategies/CLAUDE.md:95-128`, 2026-09-17 이관):
...
→ CHANGELOG: cycle274 · cycle276 행

### 2026-09-12 cycle286 C2-a — LTV `main` 15:20 컷 배너 원문

사용자 지적, 15:20~15:39:59 노출 분석(K4/K5/N5), 세그먼트 sha 불변 검증.

원문(`src/engine/strategies/CLAUDE.md:131-149`, 2026-09-17 이관):
...
→ CHANGELOG: cycle286 행

### 표 행 원문 — `volatility_breakout` · `long_tail_volatility`

사이클 26 KRX ONLY → 사이클 38 LTV 3보드 복원 변천.

원문(`src/engine/strategies/CLAUDE.md:154-155`, 2026-09-17 이관):
...
### 표 행 원문 — `bull_flag_breakout` · `vcp_breakout`

2026-08-08 유니버스 확대 결정값(거래대금 20억→15억, `max_scan_stocks` 100/200→4000, 지수 제약 해제 근거), 사이클 211 완화 0.382→0.5, 사이클 173/196 220일 미실현, 비지수 641종목 95.8% 실측.

원문(`src/engine/strategies/CLAUDE.md:157-158`, 2026-09-17 이관):
...
원문(`src/engine/strategies/CLAUDE.md:159`, 2026-09-17 이관):
...
### 2026-08-08 — 개수 캡(`max_positions`) 생애주기 자문 원문(T0~T4 자본 단계표 · 정량 근거 3 · FREEZE 조치)

자본 단계별 로드맵과 그때의 FREEZE 조치 ①②③, 미결 결정 5, 사용자 원질문.

원문(`src/engine/strategies/CLAUDE.md:178-192`, 2026-09-17 이관):
...
### 2026-09-05 cycle254 — 관문 순서·두 캡 합성 전환 원문

K축(cycle242)·ρ축(cycle245) 도입 경위와 cycle254 의 합성 전환 서술이 한 문단에 겹쳐 있었다.

원문(`src/engine/strategies/CLAUDE.md:193`, 2026-09-17 이관):
...
→ CHANGELOG: cycle242 · cycle245 · cycle254 행

### 2026-09-04 cycle242 — 터틀 키 7종 가드 부재 발견

터틀 키 7종 항목 말미에 붙어 있던 문장이다.

원문(2026-09-17 이관):

---

⚠️ 종전 6종은 **기계 가드 없이 문서 주장뿐**이었다(`grep min_vol_floor_pct tests/unit/ast/` 0건) — 후속에서 같은 가드에 편입 권장.

---

### 2026-08-06 P1.5 — BFB 익일 청산 오해 정정 원문

2026-08-06 정정 경위(그 전 거짓 서술 원문 인용 포함)와 그 시각의 운영 DB 실측 `enabled=True`·`weight=0.10`.

원문(`src/engine/strategies/CLAUDE.md:201`, 2026-09-17 이관):
...
### 2026-08-06 P1 — 청산 파라미터 `_position_setup`/`_effective_setup` 원문

2026-08-04 kojiro 삼영무역 동일 클래스 진단과 필드별 갱신 규약의 도출 경위.

원문(`src/engine/strategies/CLAUDE.md:202`, 2026-09-17 이관):
...
### 2026-08-06 H-1 — `high_since_buy` 영속 원문

보정 헬퍼 도입 전 donchian/VCP 에 복사본 2벌이 있었고 kojiro 가 3번째가 될 참이었다는 경위.

원문(`src/engine/strategies/CLAUDE.md:203`, 2026-09-17 이관):
...
### 2026-05-20 사이클 21 — VB/LTV/momentum 깔때기 노출 원문

`_empty_scan_stats()` 키 목록의 도입 경위 서술.

원문(`src/engine/strategies/CLAUDE.md:208-212`, 2026-09-17 이관):
...
### 2026-06-11 사이클 108 — `volume-rank` 폐기 전후 원문

사이클 33 `acml_vol` → 사이클 48 `volume-rank` → 사이클 108 `list_by_filter` 소스 변천이 세 항목에 나뉘어 서로 모순되게 남아 있었다.

원문(`src/engine/strategies/CLAUDE.md:213-215`, 2026-09-17 이관):
...
### 사이클 48 · 211 · 49 — BFB/VCP 임계 완화·결함 시정 원문

완화 근거(funnel 281→80→12, 오프라인 스윕 3→10)와 사이클 49 '마지막 폭 0.0%' 결함의 root cause.

원문(`src/engine/strategies/CLAUDE.md:216-219`, 2026-09-17 이관):
...
→ CHANGELOG: 사이클 48 · 49 · 211 행

### 1주 폴백 — cycle242/245/254 경위 원문

폴백 1주가 설계 유닛의 평균 4.94배·최대 15.61배가 되던 실측, 2026-09-04 LTV 사고와 그날 예산 기준 컷오프 표.

원문(`src/engine/strategies/CLAUDE.md:223`, 2026-09-17 이관):
...
### funnel·prepare 사이클 문단 12개 원문(사이클 39+41 · 132 · 143 · 156 · 157 · 158 · 148 · C3 · G · 170 · 173 · 50)

도입 배경·회귀 가드 수·검증 수치·다음 사이클 인계가 문단마다 붙어 있었다.

원문(`src/engine/strategies/CLAUDE.md:224-235`, 2026-09-17 이관):
...
### 2026-05-27 사이클 48 — BFB/VCP `breakout` 구독 편입 경위

구독 우선순위 항목 안에 있던 서술이다.

원문(2026-09-17 이관):

---

**사이클 48 (2026-05-27)**: `scheduler._collect_breakout_tickers()` 가 4 돌파 전략(VB/LTV + bull_flag_breakout/vcp_breakout)을 순회 → BFB/VCP 후보도 동일 `breakout` LOW 그룹으로 편입. BFB/VCP 는 폴링 루프 없이 `risk.on_tick` 으로만 매수 평가하므로 이 구독 없이는 0건 지속(PR #15 P1). `_resubscribe_stale_priority` 의 stale 재구독도 positions/next_day_clear 소속이 아닌 후보는 LOW (사이클 25-B, 2026-05-20).

---

### 사이클 26 → 38 — LTV 보드 복원 경위

LTV `DEFAULT_TRADABLE_BOARDS` 항목 안에 있던 서술이다.

원문(2026-09-17 이관):

---

복원 (사이클 38, 2026-05-22)** — 사용자 운영 의도 복원. 연속 상한가 종목 익일 청산 모드 + 야간 매수. 사이클 26 KRX ONLY 정책 폐기. DB `strategy_config` 는 운영 중 보존된 3 보드 유지

---

## donchian·BFB·VCP 개별 파라미터 규약

### 절 제목 원문 — 「사이클 23 파라미터 확장 (2026-05-20)」

소절 제목도 사이클 번호와 시정 경위를 달고 있었다(`### donchian \`breakout_fail_n_days\` (P2-2) — **사이클 223 (2026-08-21): 영업일 기준 + 재도출 off-by-one 시정 + PARAM_RANGES 제외**` 등).
...
### 2026-08-21 사이클 223 — 보유일 달력일→영업일 · 재도출 off-by-one 원문

달력일 산정이 청산을 최대 2~3일 앞당기던 결함.

원문(`src/engine/strategies/CLAUDE.md:271`, 2026-09-17 이관):
...
### 2026-08 사이클 226 — 돌파선 0 방어 3중 원문

이전 검증 누락, 사이클 225 '행위 변경 0' 계약의 부분 갱신과 `reason=not_called` 도달 불가 서술, '현재 잠복' 판단.

원문(`src/engine/strategies/CLAUDE.md:273-291`, 2026-09-17 이관):
...
→ CHANGELOG: 사이클 225 · 226 행

### 2026-08-24 사이클 225 — 복구 경로 침묵 4층 관측화 원문

192820 사고, 관측 4층의 발견 경위, `logger.debug` 단독이 `system_logs` 에 닿지 않던 이유.

원문(`src/engine/strategies/CLAUDE.md:294-310`, 2026-09-17 이관):
...
### 사이클 224 — `[days_held_observe]` 상시 관측 원문

그 전에는 폴백 플래그·실제 발화 때만 로그돼 완전 무음이던 실측 사각.

원문(`src/engine/strategies/CLAUDE.md:311-322`, 2026-09-17 이관):
...
### 2026-09-02 사이클 237 — 청산 계열 로그 폭주 cap 원문

08-31 11,453건 · 09-01 9,027건 폭주 실측, 래칫 부재 분석, 09-02 프리장 게이트 ~30초 구멍 발견과 cycle238 시정.

원문(`src/engine/strategies/CLAUDE.md:323-352`, 2026-09-17 이관):
...
→ CHANGELOG: 사이클 237 · cycle238 행

### 2026-07-14 사이클 209 — `max_breakout_extension_pct` 3.0→4.0 원문

AI 과튜닝 0.5% 병목과 상향 결정.

원문(`src/engine/strategies/CLAUDE.md:354-355`, 2026-09-17 이관):
...
### 2026-07-13 사이클 208 — 박스 수축 필터 완전 제거 원문

제거 경위와 삭제 목록, 2026-07-13 KB금융/금호타이어 실측.

원문(`src/engine/strategies/CLAUDE.md:360-363`, 2026-09-17 이관):
...
### 사이클 23 — VCP PARAM_RANGES 화이트리스트 원문

P2 신규 5키 중 3키 등록 서술은 사이클 208/209/223 의 순차 제외 뒤 코드와 어긋나 있었다.

원문(`src/engine/strategies/CLAUDE.md:365-367`, 2026-09-17 이관):
...
### 2026-09-05 cycle258 — 관측기 자기 실패 경로 승격 원문

`_trace_observer_failure` 메서드(cycle225 J-3)에서 모듈 함수로 승격된 경위.

원문(`src/engine/strategies/CLAUDE.md:375`, 2026-09-17 이관):
...
→ CHANGELOG: cycle258 행

### 2026-09-13 cycle290 — 장중 킬스위치 두 키 등재 원문

cycle287 배포 시점에 두 키가 어디에도 없어 `PUT /api/strategies/{id}/params` 가 `unknown_key` 422 이던 경위.

원문(`src/engine/strategies/CLAUDE.md:377`, 2026-09-17 이관):

---

> **cycle290 (2026-09-13) — 장중 킬스위치 두 키 등재.** cycle287 이 `order_engine._route_exchange_by_clock`(mode)·KRX 애프터 청산 호가유형 두 스위치를 만들었지만 배포 시점에는 어느 전략 `DEFAULT_PARAMS` 에도 `param_catalog` 에도 없어 `PUT /api/strategies/{id}/params` 가 `unknown_key` 422 였다. 이 사이클이 `order_exchange_clock_mode`(기본 `"enforce"`)·`after_market_exit_division`(기본 `"44"`) 두 키를 **7 전략 전부**의 `DEFAULT_PARAMS` 말미에 코드 상수(`order_engine._ORDER_EXCHANGE_CLOCK_MODE_DEFAULT`·`_AFTER_EXIT_DIVISION_DEFAULT`)와 같은 값으로 추가했다 — 등재 자체는 매매 행위 변경 0(값이 기본값과 같으므로 `params.get(key, DEFAULT)` 가 항등)이고, 여는 것은 그 키에 값을 넣을 **통로**뿐이다. 진입·청산·사이징 4 메서드(`check_buy_signal`/`check_exit_signal`/`calc_buy_quantity`/`prepare`) 는 28 세그먼트 전부 sha 불변 — `DEFAULT_PARAMS` 는 클래스 속성이라 그 세그먼트 밖이다. `PARAM_RANGES`/`INT_PARAMS` 편입 금지(AI 자문이 청산 수단을 끄는 스위치를 뒤집으면 안 된다). 사고 중 조작 순서·허용값·롤백 수단 상세는 `_workspace/00_leader_trading_rules.md` 「거래소 라우팅」 절 + `src/engine/CLAUDE.md` §order_engine.py.
