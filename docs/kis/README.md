...
> **예외 = 아래 「제도 변경 공지 반영」 절에 열거한 항목.** 워크북보다 나중에 나온 공지나 라이브 실측이라 자동 생성분에 없고, 손으로 넣었다. **재생성 시 이 절을 보고 다시 넣어야 한다.**
...
원본 워크북(`20260911`)에는 없고 KIS 공지나 우리 라이브 실측에만 있는 내용이다. **`docs/kis/*.md` 를 재생성하면 사라지므로
아래 표를 보고 다시 넣는다.** 공지 원문·실측 원문의 요약이 아니라 원문 값을 그대로 옮긴 것이다.
...
**규모(고객 공지 기준, 2026-09-09)** — 애프터마켓 거래 가능 종목이 **606종목**(NXT 단독 시절)에서 **2,459종목**(KRX+NXT)으로 넓어진다. ETF/ETN·투자경고 등 시장관리 필요 종목은 제외다. 가격제한은 전일 KRX 정규장 종가 **±30%**, 체결은 **실시간 연속**(종전 시간외 단일가는 10분 주기였다).
...
### 실측 정정 ① 장운영정보 통합(`H0UNMKO0`) — 라이브 프레임은 첫 칸이 종목코드다 (실측 2026-09-21~23)

출처: EC2 운영 로그 `src.realtime.handler` INFO 17건(2026-09-21~23) · 조사 메모
`_workspace/domain_consult/cycle359_mkop_field_offset.md` §3.1(원문 17건 전수).

워크북의 통합 절 Response Body 표는 종목코드 칸 없이 `[0]=TRHT_YN` 으로 시작한다(10칸).
라이브 프레임 17건은 **전부** KRX(`H0STMKO0`)·NXT(`H0NXMKO0`) 표처럼 `[0]=종목코드` 로 시작한다.
그래서 워크북 표를 라이브 데이터에 대면 **한 칸씩 어긋난다.** 맨 끝 `EXCH_CLS_CODE` 칸은 오지 않았다.
빈 칸은 빈 문자열이 아니라 `(null)` 글자 그대로 올 수 있다.

| 반영 파일 | 반영 위치 | 넣은 내용 |
|---|---|---|
| [domestic-stock-realtime.md](domestic-stock-realtime.md) | 국내주식 장운영정보 (통합)(`H0UNMKO0`) 절 머리 | 🔴 라이브는 `[0]=종목코드` 라 아래 표와 한 칸 어긋난다는 주의문 + 라이브 원문 예시 |

원문 예시(2026-09-22 11:13:06, 종목 100840): `100840^N^(null)^AB1^112^^^55^N^`
→ `[0]` 종목코드 · `[1]` `TRHT_YN`=`N` · `[2]` `TR_SUSP_REAS_CNTT`=`(null)` · `[3]` `MKOP_CLS_CODE`=`AB1` ·
`[4]` `ANTC_MKOP_CLS_CODE`=`112` · `[7]` `ISCD_STAT_CLS_CODE`=`55` · `[8]` `VI_CLS_CODE`=`N`.

우리 코드의 대응 = `src/api/market_operation.py::parse_market_op_payload` 가 `fields[0]` 이 `"Y"`/`"N"` 이면
표 모양, 그 밖은 라이브 모양으로 읽는다(`src/api/CLAUDE.md` 「칸 기준점」). KIS 가 통합 채널 표를 고치면
이 항목과 파서의 판별을 함께 다시 본다.
