...
### 관리종목(51)·단기과열(59) — 보유 청산 + 당일 매수 차단 (7 전략 공통)

사용자 결정 원문:
- 2026-09-25 21:5x 「관리종목51과 단기과열59가 발동했는데 해당 종목을 보유하고 있으면 시장가로 청산하게 하고 재구독 중지등으로 신규매수를 중단하자.」
- 2026-09-26 11시 — 신규 매수는 당일 재매수 차단 + 진입 필터로 막고 구독은 끊지 않는다. 보유 종목은 시장가로 청산한다. 「지정 첫날 매수를 막는거지」 — 보유하지 않은 후보의 지정 첫날 매수도 막는다.

| 규칙 | 값 |
|---|---|
| 판정 소스 | REST `FHKST01010100` 전용 플래그 — 관리 `mang_issu_cls_code == "Y"` · 단기과열 **지정·연장** `short_over_yn == "Y"`(예고는 `N`). 종목상태 `iscd_stat_cls_code` 51·59 는 플래그가 비었을 때만 쓴다 |
| 매도 판정 | **전용 플래그가 `Y` 일 때만 판다.** 종목상태 51·59 폴백만 맞으면 팔지 않고 `[status_exit_fallback_only]` 경고만 남긴다 — 그 코드는 정상 ETF·스팩·우선주에도 붙는다(cycle203 실측). 매수 차단은 폴백도 쓴다(막는 쪽이 안전) |
| 판정에 쓰지 않는 것 | `ssts_hot_yn`(공매도과열) · `short_over_cls_code`(예고 포함) · `stock_master` DB 값(하루 늦다) · `H0UNMKO0` 프레임 단독 |
| 청산 대상 | 전 전략 보유(꺼진 전략 포함). 전략 예외 없음 |
| 청산 시각 | KRX 정규장 **09:00:30 ~ 15:28** 에 읽은 값으로만. 창은 발사 직전에 한 번 더 본다. 장 전·15:28 이후·애프터마켓(16:00~20:00, 단기과열 거래대상 제외)은 주문하지 않고 다음 정규장에 판다. 휴장일로 확정된 날(수동 기동)은 조회도 청산도 하지 않는다 |
| 청산 방법 | `execute_sell(t, Signal.STATUS_EXIT, sid)` 시장가 · KRX · `asyncio.shield` 안. 조회 5분 격자 · 종목당 하루 3회 상한(재시작해도 그날 `system_logs` 발사 기록으로 이어 센다) · 정지(58) 겹치면 대기 |
| 재시작 뒤 걸린 주문 | 이전 청산 주문이 거래소에 걸린 채 재시작하면 한 번 더 쏜다. 그 주문은 `APBK0400`(매도가능수량 초과)으로 거부되고 `[sell_qty_locked]` 로 끝난다 — 포지션 보존 · `_selling` 유지라 그 종목은 더 쏘지 않는다. 두 번 팔리지 않는다 |
| 체결 지연 | 단기과열은 30분 단일가라 시장가도 다음 :00/:30 체결까지 최대 30분 걸린다(제도상 강제 — 새 취소 타이머를 두지 않는다) |
| 신규 매수 차단 | 그날 장중 조회에서 해당이면 그 종목 신규 매수 신호를 7 전략 전부 막는다(지정 첫날 포함). 판 종목은 `is_sold_today`, 다음 날부터는 진입 필터 `_is_master_blocked_for_entry` 도 막는다. 막는 순간 그 종목의 edge 기준가(VB·LTV `_prev_price[t]` · momentum `_prev_prdy_rate[t]`)를 비워 해제 뒤 거짓 돌파를 막는다 |
| 매수 차단 조회 시각 | P0 08:45(`pre_nxt` 전략이 있으면 07:59)부터 09:00:00 전까지 장 전 기록 · P1 **09:00:05** 후보 전수(스윙 donchian·kojiro 제외 — 스윙 폴 조회가 덮는다) · INC 60초마다 15:20 까지 · 기존 조회의 관측 훅 |
| 못 읽은 종목 | 막지 않는다(fail-open). 새어 산 종목은 다음 청산 패스가 판다(`bought_today=1`) |
| 구독 | 건드리지 않는다 — 보유 구독을 끊으면 청산 거부 시 손절이 멈춘다 |
| 킬스위치 | `system_config.status_exit_mode`(청산) · `status_buy_block_mode`(매수 차단) ∈ `enforce`/`observe`/`off`, 키 없음 = `enforce`, 모양이 틀린 행 = `observe`, DB 조회 실패 = 직전 값. 즉시 = `PUT /api/integrations/status-exit` — 메모리를 DB 쓰기보다 먼저 바꾼다. 청산 패스는 발사 직전마다 모드를 다시 읽어, 패스 도중의 `off` 도 뒤 종목부터 적용된다. DB 저장이 실패하면 그 축을 메모리에 고정한다(다음 성공 저장 또는 재시작까지) |
