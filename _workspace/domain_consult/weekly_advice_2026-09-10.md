...
⚠️ 그 1개의 **기대 효과는 작다**(step7 생존 +약 1.2건/일). 실제 일봉 재실행으로 확인한
...
{"strategy_id":"vcp_breakout","sample_status":"insufficient","closed_round_trips_30d":0,
 ...
 "reasoning":"funnel 5일 실측에서 VCP 가 죽는 곳은 step 8(거래량 수축)이 아니라 step 7(Pullback 점진 수축)이다 — 베이스 검출 생존 24/32/22/28/38 이 step7 에서 1/1/2/1/4 로 줄어 평균 92% 가 탈락한다. step7 은 세 술어의 AND(회수 [2,4] ∧ 직전 대비 폭 단조 감소 ∧ 마지막 폭 ≤ last_pullback_max)인데, 그중 last_pullback_max 만 PARAM_RANGES 에 있다. 탈락 종목의 실제 일봉을 GET /api/stock-master/{ticker}/daily 로 받아 _detect_base + _check_pullback_sequence 를 라이브 파라미터로 재실행한 결과(23행), 폭 단독 위반은 6행뿐이고 0.12 에서 정확히 그 6행이 통과한다(010950 S-Oil 5일 연속 + 204620 글로벌텍스프리 1일 = 2종목). 0.15 로 더 완화해도 추가 통과는 0 이다. 라이브 0.10 은 코드 DEFAULT_PARAMS 0.12(vcp_breakout.py:126, '사이클 48 — 0.08→0.12 한국 중소형주 변동성 현실화')보다 조여진 값이므로 이 권고는 신규 완화가 아니라 기본값 복원이며, 기대 효과는 step7 생존 +약 1.2건/일(5일 6행)로 작다. 2차 병목인 volume_contraction_ratio 는 라이브 1.00 = PARAM_RANGES 상한이라 더 완화할 수 없다.",
 ...
 "hypotheses":[
   ...
   {"claim":"step7 의 지배 병목은 last_pullback_max 가 아니라 단조 수축(strict, 등호 거부)이다","evidence":"회수가 [2,4] 안인 step7 탈락 51행 중 28행은 폭도 10% 이하라 남는 위반 술어가 단조뿐이고(코드상 연역), 나머지 23행은 실제 봉 재실행에서 17행이 단조도 함께 위반했다 = 45/51(88%). 단조 검사는 vcp_breakout.py:786-788 의 strict 비교(curr >= prev 면 탈락)이고 폭 검사(:791)보다 먼저 early-return 하므로 funnel 사유 문자열로는 구분되지 않는다","test":"pullback 폭 수열을 종목별로 덤프해 '거의 수축'(예: 직전 대비 +5% 이내 반등) 비율을 측정 — 등호/근사 허용의 효과를 오프라인 산출","required_sample":0,"confidence":0.9},
   {"claim":"0.12 로 완화해도 하루 prepared 는 3건을 넘지 않아 리스크 증가가 제한적이다","evidence":"step7 이 5일에 6행(2종목)만 늘고, 그 뒤 step8(거래량 수축) 통과율이 실측 09-08 1/2·09-10 2/4 = 약 50% 다. 현재 prepared 0.6건/일 → 추정 0.8~1.5건/일","test":"적용 후 10영업일 step99 일평균과 실제 체결 수","required_sample":10,"confidence":0.7},
 ...
 "needs_human_decision":[
   {"topic":"step7 의 진짜 병목은 단조 수축이고 그 키는 튜닝 대상이 아니다","issue":"vcp_breakout.py:786-788 은 pullback 폭이 직전보다 '반드시 작아야' 한다(strict, 등호 거부). 이 술어에 대응하는 파라미터가 없어 자문이 손댈 수 없는데, 회수 조건을 통과한 탈락 51행 중 45행(88%)이 여기에 걸린다. last_pullback_max 를 0.12 로 되돌려도 6행만 살아나는 이유가 이것이다","evidence":"28행은 폭도 임계 이하라 남는 위반 술어가 단조뿐(코드 연역) + 23행 실제 봉 재실행에서 17행 단조 위반 확인. 재실행은 GET /api/stock-master/{ticker}/daily 100일 + 라이브 파라미터로 _detect_base/_check_pullback_sequence 직접 호출, funnel 이 기록한 마지막 폭과 23/23 일치","options":["현행 strict 유지(변경 없음)","등호 허용 또는 허용 오차(예: curr <= prev × 1.05) 도입 — 코드 변경 + 매매 행위 변경이라 승인 + domain-consult 선행","pullback_count_max 를 4→5,6 으로 넓혀 긴 수축 수열을 수용(역시 PARAM_RANGES 밖)"]},
 ...
 "code_review_notes":"last_pullback_max 는 PARAM_RANGES (0.03, 0.15) 안이고 INT_PARAMS 가 아니며 §4 봉인 키 목록에 없다. ⚠️ funnel 의 step7 탈락 사유 문자열은 세 술어의 AND 를 통째로 서술하고 회수·마지막 폭을 검사 이전에 무조건 기록하므로(vcp_breakout.py:776-779), 어느 술어가 탈락시켰는지 로그만으로는 구분되지 않는다 — 단조 검사(:786-788)가 폭 검사(:791)보다 먼저 early-return 하기 때문이다. 이 자문의 수치는 로그 파싱이 아니라 실제 일봉 재실행으로 얻었다. 관측 개선 후보(코드 변경, 이번 권고 아님) = 탈락 사유에 실패한 술어명을 함께 기록."}
```
...
그래서 **실제 일봉을 받아 술어를 직접 다시 돌렸다** — `GET /api/stock-master/{ticker}/daily`
로 100일치를 받고, 스냅샷 날짜 이후 봉을 떼어낸 뒤, 라이브 파라미터로 `_detect_base` 와
`_check_pullback_sequence` 를 호출했다. funnel 이 기록해 둔 마지막 폭과 재실행 값이
**23행 전부 일치**해 재현이 충실함을 먼저 확인했다.
...
5일에 6행이니 step7 생존이 하루 약 1.2건 느는 셈이고, 그 뒤 거래량 수축(step 8) 통과율이
실측 절반이라 **prepared 는 0.6건/일 → 0.8~1.5건/일** 정도다. 작다.

그리고 이 재실행이 **더 중요한 것**을 알려줬다. 회수 조건을 통과한 탈락 51행 중
**45행(88%)이 ② 단조 수축에 걸린다**(폭도 임계 이하였던 28행은 코드상 남는 위반이 ②뿐이고,
폭 초과 23행 중 17행이 재실행에서 ② 위반 확인). ②는 **등호도 거부하는 strict 비교**라
직전과 폭이 같기만 해도 탈락한다. **VCP 가 후보를 못 만드는 진짜 이유는 여기다.**
그런데 ②에는 대응 파라미터가 없어 자문이 손댈 수 없다 → **결정 항목 D3'** 로 올린다.

그렇다면 0.12 권고는 왜 남기나. **모험이 아니라 원위치**이기 때문이다 —
0.12 는 우리가 지어낸 값이 아니라 코드에 적힌 기본값이고(`vcp_breakout.py:126`, 주석
"사이클 48 — 0.08→0.12 한국 중소형주 변동성 현실화"), 라이브 0.10 은 최소 2026-08-11 부터
그보다 조여진 상태였다(`/api/recommendations` 의 `current_params` 22행 전부 0.1).
효과는 작지만 방향이 옳고 되돌리기 쉽다. 표본 규약과도 충돌하지 않는다 —
§6 이 표본 부족 시 null 로 두라고 한 것은 **청산 축과 비중 축**이고, `last_pullback_max` 는
**후보를 만드는 진입 필터**다. 후보가 안 나오면 §6 이 요구하는 표본 10왕복은 **영원히 모이지 않는다.**

⚠️ **다만 이 한 키로 VCP 가 살아나지는 않는다.** 병목의 88%는 그대로 남는다.
...
| **D2** | **`vcp_breakout.last_pullback_max` 0.10 → 0.12** | 이번 주 유일한 권고. 코드 기본값 복원이며 방향은 완화. 실제 일봉 재실행 결과 23행 중 **6행**(2종목)이 이 값 하나에만 걸렸고 0.12 에서 그 6행이 통과한다. **효과는 작다** | ① 적용(권고) ② 보류 — **0.15 는 선택지가 아니다**(추가 통과 0 으로 실측) |
| **D3'** | **VCP step7 단조 수축 조건(strict)** | 회수 조건을 통과한 탈락 51행의 **45행(88%)** 이 여기 걸린다. 등호도 거부하는 비교(`vcp_breakout.py:786-788`)라 폭이 직전과 같기만 해도 탈락. **대응 파라미터가 없어 자문 권고 불가** — VCP 가 4개월 반 무체결인 진짜 이유 | ① 현행 유지 ② 등호 허용 또는 허용 오차 도입 — 코드 + **매매 행위 변경**이라 승인 + `domain-consult` 선행 ③ `pullback_count_max` 4→5,6 확대(역시 범위 밖) |
...
|---|---|---|---|---|
| V1 | D2 적용 시 VCP funnel | `GET /api/strategy-funnel/recent?strategy_id=vcp_breakout` step7·step8·step99 | step7 생존이 현재 1~4 → **2~5**(+약 1.2건/일), step99 가 0.6건/일 → **0.8~1.5건/일**. 이보다 크게 늘면 재실행 추정이 틀린 것이므로 재조사 | 5영업일 |
| V1b | D3' 정량화(코드 변경 없이) | 탈락 종목 일봉을 받아 pullback 폭 수열을 덤프 → 직전 대비 반등이 5%·10% 이내인 비율 | 등호/오차 허용 시 살아나는 종목 수의 오프라인 추정치 확보 → D3' 판단 근거 | 0(즉시 가능) |
...
| `GET /api/stock-master/{ticker}/daily?days=100` × 12종목 | 전부 200 (§3.1 술어 재실행용 — 읽기 전용) |
| 그 외 8개 엔드포인트 | 전부 200 |

### §3.1 재실행 방법 (재현 가능하도록 명시)

1. `GET /api/stock-master/{ticker}/daily?days=100` 으로 일봉 100행 수신 → 각 행의 `raw`(KIS 원본 키) 추출.
2. 스냅샷 날짜 D 기준 재현을 위해 `bas_dd >= D` 인 봉을 제거(= 그날 아침 prepare 가 본 것과 같은 창).
...
4. **검증** — funnel `excluded_sample` 이 기록한 "마지막 폭"과 재실행의 `base["last_pullback_pct"]` 가
   **23/23 일치**. 코드 실행 0줄 변경(순수 메서드 호출), DB·KIS 접근 0.
