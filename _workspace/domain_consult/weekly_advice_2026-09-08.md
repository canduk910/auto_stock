...
{"strategy_id":"vcp_breakout","sample_status":"insufficient","closed_round_trips_30d":0,
 ...
 "needs_human_decision":[{"topic":"VCP 5단계 EMA 정렬이 94% 를 죽인다 — 단 config 값이 아니라 실효 기간으로 읽어야 한다","issue":"라이브 config 는 ema_short/mid/long = 50/150/200 이지만 런타임 실효값은 50/65/75 다. prepare() 가 KIS 단일호출 100일 한도로 fetch_days=min(200+75+10,100)=100 을 받고 effective_ema_long = min(200, 100-20-5) = 75 로 캡한 뒤, _check_trend_filter 가 ema_mid(150) >= 75 이므로 ema_mid = max(51, 65) = 65 로 재축소한다(사이클 33·48, 소스 주석에 명시). 코드 기본 50/60/120 도 실효 장기선은 같은 75 이고 60 < 75 라 재축소가 없어 실효 50/60/75 다. 따라서 (가) 668→41 을 죽이는 것은 200EMA 가 아니라 실효 75EMA 정배열 + 20일 우상향 조건이고 (나) 코드 기본 복원의 실질은 중기선 65→60 한 칸뿐이며 (다) 진짜 120/200EMA 는 분할 fetch 인프라가 있어야 계산된다. 이 자문의 초안은 이를 3키 변경으로 잘못 기술했고 PR #20 codex 리뷰 지적으로 정정했다","evidence":"src/engine/strategies/vcp_breakout.py prepare() 의 KIS_DAILY_CANDLES_MAX=100 · effective_ema_long = min(ema_long, available_len - uptrend_days - 5) · _check_trend_filter 의 if ema_mid >= ema_long: ema_mid = max(ema_short+1, ema_long-10). 09-08 funnel 5단계 668→41, 최근 5일 41/45/50/57/41","options":["중기선만 65→60 으로 (config ema_mid 150→60) 한 칸 완화하고 5단계 통과 수를 관측 — 실효 변화가 가장 작고 되돌리기 쉽다","ema_short 를 50 에서 낮춰 실효 밴드(50/65/75)의 폭을 넓힌다 — PARAM_RANGES 밖 키라 별도 승인 필요","분할 fetch 인프라를 먼저 만들어 진짜 120/200EMA 를 계산 가능하게 한다(별도 사이클, 범위 큼)","현행 유지하고 7단계 완화 효과만 2주 관측(권고: 한 번에 한 축)","전략 중단하고 weight 0.10 을 회수"]},
 ...
 "no_change_reason":null,
 "code_review_notes":"base_depth_pct 는 라이브 0.35 / 기본 0.30 으로 이미 완화돼 있고 6단계(베이스 검출)는 41→22 로 병목이 아니라 손대지 않는다. breakout_volume_mult 라이브 1.2 / 기본 1.5 도 완화 방향이다. 즉 VCP 의 조임은 5단계 실효 EMA 정렬(50/65/75)·last_pullback_max·min_swing_atr_mult 에 집중돼 있고, 그중 화이트리스트 안에 있는 것은 last_pullback_max 하나뿐이다."}
```
...
| **D-3** | VCP 5단계 EMA 정렬이 668→41 로 94% 를 죽인다 — 판정선은 config 50/150/200 이 아니라 **실효 50/65/75** (복원의 실질은 중기선 65→60 한 칸) | funnel 5일 + 소스 | §3.7 |
