...
### 진짜 원인 — 설계 랏이 한 주 값보다 작다 (순자산 **5,032,151원**)

| 전략 | 설계 랏 | 실거래 중앙 주가 | 랏/주가 |
|---|---|---|---|
| volatility_breakout | **88,063** | 150,600 | **0.6주** |
| long_tail_volatility | **50,322** | 59,240 | **0.8주** |
| momentum | 62,902 | 54,395 | 1.2주 |

`q<1` 이면 주 분기가 항상 0 → **100% 폴백 경로** → 최종 명목 = **그 종목 주가 그 자체**.
189배의 산수가 이것이다. ρ축(cycle245)은 **위만** 막는다(최근 30일 위반 0건 = 정상 작동).

**마스터 부등식 `Σ(P_i × m_i) ≤ A / N`** — 제약은 전략 **개수**가 아니라 **총 슬롯 수**다.
현재 28 슬롯, N=5 기준 상한 **10 슬롯**, 현재 실효 **N=1.97**.
지금 구조는 **순자산 1,278만원짜리 설계**다.
...
### 결정 대기 4건
**E1** 슬롯 28→10(🔴 **끄지 말고 비중으로** — 아래 금기) · **E2** 스캔 단계 주가 상한 ·
**E3** LTV 상한가 모드 −5%+트레일링 부재(자문 최우선) · **E4** 랏 관측을 어디에 붙일까
(`llm_buy_evaluations` 는 현재 **8행**, 게이트가 켜진 전략이 **7개 중 3개** — 그래서 미집행).
