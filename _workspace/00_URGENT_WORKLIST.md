...
| D5 TLS | `auto.dkstock.cloud` | ✅ cycle255 구현·검증 완료(배포 = 야간 진행). **사용자 할 일 = ① DNS A 레코드 `auto.dkstock.cloud → 3.38.228.74` 등록 ② EC2 `sudo snap install certbot --classic` ③ `bash tools/ops/tls_enable.sh`(사후 검증·자동 원복 내장) ④ 루틴 2개 BASE `https://auto.dkstock.cloud` + 클라우드 환경 허용 도메인 추가**. 80 은 계속 유지(1단계), 리다이렉트·HSTS 는 2단계 |
