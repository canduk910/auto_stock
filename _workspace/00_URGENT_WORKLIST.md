...
### 후속 — 진짜 검증은 「휴식 창 시각 고정 스모크」다

지금 방식은 「그 시각에 돌려 봐야 안다」이고 창이 하루 30분이라 재현이 어렵다.
`freezegun` 으로 15:45 를 고정한 채 주문 경로 스모크를 돌리면 **시각 무관하게** 재현된다.
⚠️ 다만 freezegun 은 `_wait_until` 계열에서 동결된 monotonic 이 hang 을 만든 선례가 있어
(루트 `CLAUDE.md`) 주문 경로에 한정해 신중히 붙여야 한다. 별건 카드.
