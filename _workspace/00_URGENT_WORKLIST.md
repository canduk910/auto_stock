...
### 🔴 P0 — 09-14(월) 16:00~20:00 보유 종목 손절 수단이 없다

`OrderDivision` 은 `LIMIT="00"` / `MARKET="01"` 둘뿐이고, KRX 애프터마켓은 `ORD_DVSN`
**41~47**(지정가 계열 7종)만 받는다. 즉 16:00~20:00 에 손절이 발동하면 시장가(`01`)도
지정가(`00`)도 거부된다. cycle286 C4-a 는 그 거부가 **DB 를 오염시키는 것**을 막았을 뿐
**청산 자체는 여전히 불가**다. 보유 11종목에 4시간 사각이 생긴다.

시정 = `src/models/order.py` + `src/api/order.py`(8영역) + 거부 분류 + 폴백 경로.
**사용자 승인 + domain-consult 선행 대상.** 착수 전 필요한 실측 = 애프터 거부 `msg1` 원문
(위 실측 목록 2번)인데, 그 실측을 09-14 에 하려면 그날 손절이 발동해야 하므로 **선행
순서가 역전**된다 — 09-14 이전에 `41` 을 넣어 두고 거부 여부로 확인하는 편이 안전하다.
