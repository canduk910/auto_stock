...
| **08:00~09:00** | 프리장 라우팅 | `[order_channel]` **미발화**(프리장은 base 유지 = routed==base) · 매도 시 `[sell_market_preconvert_pre_nxt]` 정상 | `reason=krx_by_clock` 이 이 구간에 뜨면 프리장 clause 가 깨졌다. 🔴 **08:50~09:00 이 특히 위험하다** — NXT 프리마켓은 08:50 에 끝나지만 라우터의 프리장 clause 는 `_BOARD_SCHEDULE` 의 PRE_NXT(09:00 까지)를 본다(실측 08:55 → `pre_nxt_keep`). 그 10분이 KRX 로 떨어지면 매도 사전 지정가 변환이 함께 죽어 **시가 단일가에 시장가가 나간다**(cycle287 이 명시적으로 막은 것). 종전 표기 `08:00~08:50` 은 NXT 시장 시간이지 **판정 창이 아니었다** |
...
| 7 전략 `exchange` | 전략마다 상이 | **전부 `SOR`** | cycle286 C4-a 의 거래소 조건(`∈ {"NXT","SOR"}`)이 운영에서는 **항상 참**이다 ⇒ 실효 게이트는 **창(08:00~08:50) 단독**. 거래소 축은 미래 대비 가드로만 산다(`exchange=KRX` 로 바꾸는 날 즉시 발효) |
