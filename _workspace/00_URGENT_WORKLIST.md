...
- **A (cycle252, 비8영역)**: LOW no_feed 종목의 K watcher SEND·스탬프·history 만 중단(HIGH byte 동일, `_stale_retry_count` 는 계속 — universe guard 축출 경로 보존), `[no_feed_held]` 1회/일, `[stale_watcher_summary] no_feed_skipped=`. `[tick_coverage] stale` 은 **불변이 정상**(scheduler 무접촉·은폐 금지). D+1 = `[stale_force_retry]` 09:05~15:20 399→≈0, 보조 SUBSCRIBE ≈957→<150.
