...
오늘 배포에서 실측했다. **프로세스 부재는 56초**(`[tick_blind_boot] downtime_secs=56`)인데
**시세(tick) 구독 공백은 5분 16초**였다(16:11:55 부팅 완료 → 16:17:12 148종목 ACK).
배포창 규칙(D6·D8)이 말하는 "재시작 1~5분"은 프로세스 부재만 가리키므로, 손절이 실제로
멈추는 시간을 그 값으로 읽으면 과소평가한다.

**원인** = 15:20 이후 기동 경로에는 사전 구독 호출이 **하나도 없고**, 유일한 구독 경로인
`_scan_loop` 이 `while` 진입 직후 `await asyncio.sleep(SCAN_INTERVAL)` 을 **먼저** 한다
(`scheduler.py:2362-2363`, `SCAN_INTERVAL=300` `:79`). 구독은 그 뒤 `:2382` 다.
...
| **15:20 ≤ T < 15:30** | **15:35** | **최대 15분** | `:852-853` `_scan_task=None` → `:863` 15:30 대기 → `:874` 생성 + 300초 |
| **15:30 ≤ T < 19:50** | **T + 약 5분 20초** | **약 5분** | `:874` 즉시 생성, 루프가 300초 선잠 ← **오늘 실측 구간** |
| 19:50 ≤ T < 20:00 | 없거나 곧 해제 | 사실상 종일 | `:886-887` 이 scan_task cancel, `:888` `unsubscribe_all()` |
...
`POST /api/trading/restart` 는 오히려 300초 타이머를 처음부터 다시 돌린다.
...
- `_sync_positions_from_balance` 는 독립 15분 task 가 **아니다** — `_scan_loop` 안에서
  3회차마다 돈다(`:2464-2467`, `sync_counter % 3`). 루프가 안 돌면 이것도 안 돈다.

### 정본 문서의 오류 2건 (별건 시정 대상)

| 상수 | 문서 서술 | 코드 사실 |
|---|---|---|
| `TIME_BOOT = 07:55` | "07:55 `_boot()` 실행" | **런타임 참조 0건**(정의 `:58` + 주석뿐) — `_boot()` 는 07:45 직후 즉시 돈다 |
| `TIME_POST_NXT_OPEN = 15:40` | "15:40 NXT 애프터 진입" | **런타임 참조 0건**(정의 `:62` 뿐) — 실제 경계는 `TIME_KRX_MAIN_CLOSE`(15:30) 하나다 |

대상 = `src/engine/CLAUDE.md` `TIME_*` 표 · `_workspace/00_leader_trading_rules.md` 시각표 ·
`docs/architecture.md`. 값을 고치는 게 아니라 **"이 상수는 쓰이지 않는다"** 를 적거나 상수를 지운다.

### 그래서 무엇을 하면 되는가

1. 배포창 판단에 **프로세스 부재가 아니라 위 표의 공백**을 쓴다. 보유가 있으면
   15:20~15:30 기동(최대 15분)과 19:50~20:00 기동(사실상 종일)을 특히 피한다.
2. 공백을 줄이려면 `_scan_loop` 을 **sleep-먼저에서 구독-먼저로** 바꾸거나, 15:20 이후 기동
   경로(`:852` else)에 보유 종목 HIGH 구독 1회를 넣는다. 둘 다 `scheduler.py` 변경 =
   **사용자 승인 + 라인 상한(<3,900) 확인** 대상이라 별도 사이클이다.
