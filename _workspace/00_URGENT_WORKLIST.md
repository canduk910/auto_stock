...
2. **09:05 `[swing_poll] execute_buy 실패` 오탐 시정 — 🟢 09-10 cycle271 착수(기존 "고치자" 승인 근거)**
...
7. ~~**cycle269(보조 시세계정 토큰 매일 장중 재발급 드리프트 시정)**~~ — **✅ 09-08 22:xx
   ...
   **→ 🔴 D+2 실측(09-10) = 효과 없음.** 15:45 강제 발급 7/7 성공이나 `expired=` 가 그날 14:28~14:50
   ...
   이동 확인 → **✅ cycle270(leaf 에서 `revoke()`→`issue()`, `token.py` 무접촉) 커밋·푸시 `678c233`**(09-10 17:37,
   상세·D+1 판독 규칙은 위 "09-10 자율 구간 진행 기록" 절).
