...
🔴 **다만 시각이 그대로면 하루 밀린다.** cycle283 이후 일봉 적재는 **20:30** 인데 저녁 prepare 는
**16:20** 이다. 16:20 prepare 가 읽는 헤드는 D-1 이고, 20:30 적재가 D 봉을 채운 뒤 다음 날 아침
prepare 가 읽는 헤드는 D 다. **16:20 산출물을 그대로 쓰면 헤드가 하루 밀린 후보가 된다.**
⇒ 후보 확정은 반드시 **일봉 적재 뒤**(20:30 완료 ~20:32 이후)여야 한다. 16:20 funnel 캡처와는
별개 시각·별개 목적이다(그쪽은 전일 봉 기준 관찰이 의도).
