...
| 카드 | 결정 | 상태 |
|---|---|---|
| **D1** | 남은 과제 = **I+J 부터 차근차근 하나씩** | ✅ cycle321 |
| **D2** | 배포 분류 = **B안**(`.dockerignore` + 분류 동시 수정) | ✅ cycle322 |
| **D3** | VCP 3영업일 뒤 체결 0이면 = **ⓑ 원인 실측 후 재판단** | 기한 09-23(수) 장 마감 |
| **D4** | 자금 자동조정 = **끈 채로 둔다** | 사용자 사유 = "아직 시스템 가동 5개월째라 **표본이 더 필요**" |

그리고 추가 지시 —

> "`docs/trading_base` 하위 md 파일들을 검토해서 우리 시스템에 반영할 부분 점검해줘.
>  (자금/리스크 관리 중심으로, 피라미딩과 진입청산도 검토) 현재 변동성돌파, 모멘텀,
>  롱테일 변동성돌파가 **유닛개념을 쓰지 않아서 리스크관리가 제대로 되지 않는 것 같아.**"

## 🔴 D4 의 사유를 정본에 남긴다

「끈 채로」의 이유가 **기술이 아니라 표본**이다 — 가동 5개월이라 레짐 판정의
실적을 평가할 데이터가 아직 모자란다. 즉 **영구 결정이 아니라 보류**다.
나중에 켜는 판단을 할 때 이 사유를 먼저 확인한다(기술적 준비는 cycle315·316 에서 끝났다).
...
## 후속 A 해소 — `MacroCycleSection` 의 금지 패턴

`buffett_ratio > 10 ? v : v*100` 크기 추론 분기를 `formatBuffettRatio` 재사용으로 바꿨다.
계약(비율)을 어긴 값은 변환하지 않고 `⚠단위` 로 **드러낸다**.

🔴 **재도입 방지 가드 신설** = `frontend/src/utils/__tests__/_ast_no_magnitude_heuristic.test.ts`.
판정 셋을 모두 만족할 때만 잡는다 — ① 조건이 **숫자 크기 비교**(`===` 타입·enum 비교는 정당한 분기다)
② 두 분기에 **공통 식별자**(조건 변수와 변환 대상이 다른 폐기 패턴까지 잡으려면 여기를 봐야 한다)
③ **정확히 한쪽에만** ×100/÷100. 이 셋이 없으면 `total > 0 ? (a/total)*100 : 0`(0 나누기 가드)과
`spec.type === 'percent' ? v*100 : v`(타입 선언 읽기)가 전부 오탐으로 걸린다 — 실제로 9건이 걸려서 좁혔다.

뮤테이션 2건으로 실효 확인 — 오늘 고친 패턴과 **과거 실사고를 낸 비중 오염 패턴**
...
## 후속 H — **하지 않기로 했다** (`MACRO_RE` 좁히기)

`MACRO_RE='^macro/'` 가 `macro/tests/`·`macro/README.md` 까지 잡아 이미지 입력이 아닌 변경에도
macro 컨테이너를 재빌드한다 — 그래서 좁히자는 후보였다. **실측하고 접었다.**

- **실제 낭비 사례 0건.** `macro/` 를 건드린 최근 40커밋 중 「이미지 입력은 안 바꾸고
  tests/README 만 바꾼」 커밋이 **하나도 없다**.
- 좁히면 **누락 위험이 새로 생긴다** — COPY 목록에 파일을 더했는데 정규식에 안 넣으면
  그 변경이 배포를 안 탄다. 조용한 실패이고, 불필요한 재빌드보다 나쁘다.
- backend 는 `G-248-2` 가 COPY↔정규식 정합을 강제하는데 macro 에는 그런 가드가 없다.
  🔴 **`^macro/` 가 넓어서 가드가 필요 없는 것**이다 — 좁히는 순간 가드를 함께 만들어야 한다.
...
## 🔴 작업 중 발견 — CI 가 **매일 30분간** 붉어지고 있었다 (cycle317)

후속 A~H 를 마치고 전체 스위트를 돌렸더니 **59건 실패**. 내 변경과 무관한 **선재 결함**이었다.

**원인** = cycle295 의 15:30~16:00 「완전 휴식」 컷이 **벽시계**로 판정하는데
(`order_engine._market_rest_now(now)`), `execute_buy`/`execute_sell` 을 타는 테스트에
**시각 고정이 전혀 없다**. 그 시각에 돌면 주문이 전부 컷된다.

🔴 **컨테이너가 `TZ=Asia/Seoul` 이라 CI 도 KST 로 돈다** — UTC 06:30~07:00 에 push 하면
CI 가 붉고 **배포가 막힌다**. 2026-09-19 실측으로 오늘 push 두 건이 그렇게 막혔다
(`1575172`·`36ec0a9` CI failure → Deploy **skipped**). CI 로그에 그대로 찍혀 있었다 —
`[market_rest_window] start=15:30 end=16:00` · `[market_rest_blocked] side=buy … reason=market_rest`.

**고친 방법** — `tests/conftest.py` autouse 픽스처 `_neutralize_market_rest` 가
**판정 함수 `_market_rest_now` 자체를 갈아끼운다**(선례 = 같은 파일 `_neutralize_api_auth`).
🔴 시계가 아니라 판정 함수인 이유 = `order_engine.py` 는 **8영역**이라 손댈 수 없는데
그 파일에 시계 seam 이 없다(`datetime.now(_KST_TZ)` 직접). 프로덕션에 seam 을 파려면 승인이 필요하다.
다행히 그 함수는 모듈 전역 이름으로 불리므로(`order_engine.py:620`) 이름 하나만 바꾸면 된다 —
**프로덕션 무접촉**이다.

⚠️ **그 과정에서 함정을 밟고 빠져나왔다** — 픽스처를 넣자마자 cycle295 회귀 가드 **58건이
통째로 무력화**됐다. 「휴식 창에 주문이 안 나간다」를 검증하는 테스트가 중립화된 판정 앞에서
전부 공허하게 통과한 것이다. `@pytest.mark.real_market_rest` 옵트아웃으로 되살렸고,
**메타 가드**(`test_cycle317_*.py`)가 재발을 막는다 — 🔴 손 목록이 아니라 **자동 탐색**이다
(손 목록은 새 파일을 놓친다). 탐색은 주석·docstring 을 걷어내고 **실제 호출·단언**만 본다 —
그러지 않으면 설명 문장에서 그 이름을 부르는 파일(중립화의 **수혜자**)까지 옵트아웃시켜 되돌아간다.

⚠️ **그 자동 탐색에도 한계가 있다** — 키워드 기반이라 표현이 다르면 놓친다.
실측으로 `test_cycle287_exchange_routing.py` 가 판정 이름도 마커 이름도 안 쓰고
「cycle295 컷」이라는 **한글 표현만** 써서 빠져나갔고, 전체 스위트에서야 드러났다(59→5→1건).
즉 메타 가드는 **바닥선**이지 완전한 그물이 아니다.

### ✅ cycle317b — 「휴식 창 시각 고정 스모크」로 그 한계를 덮었다

`test_cycle317b_rest_window_smoke.py`(8케이스)가 `freeze_time` 으로 창 한복판을 고정하고
판정을 실제로 태운다 — **언제 돌려도 같은 답**이다. 중립화가 먹는지(기본)와
옵트아웃하면 컷이 사는지(cycle295 기능 보존)를 **같은 시각에서** 둘 다 재고,
경계 4개(15:29:59 / 15:30:00 / 15:59:59 / 16:00:00)를 표대로 핀한다.
뮤테이션(픽스처 무력화)으로 실효 확인.

🔴 **그 과정에서 `freeze_time` 의 tz 함정을 찾았다** — naive 문자열을 주면 freezegun 이
**UTC 로 해석**해서 프로덕션이 쓰는 `datetime.now(_KST_TZ)` 가 **9시간 앞선 값**을 돌려준다
(`"2026-09-21 15:45:00"` → `now(KST) == 09-22 00:45`). 즉 「휴식 창을 고정했다」고 믿는
테스트가 실제로는 자정 45분을 본다.
⚠️ **이 저장소의 기존 관례는 UTC 문자열**이고(실측 25파일, 예: `"2026-09-14 07:05:00"` = KST 16:05)
전부 의도적이라 **현재 버그는 없다**. 다만 읽을 때 +9 암산이 필요하고 naive 를 KST 로 착각하기 쉬워,
새로 쓸 때는 `+09:00` 을 명시한다.
⚠️ freezegun 을 **스케줄러 경로로 넓히지 않는다** — `_wait_until` 계열은 동결된 monotonic 이
hang 을 만든 선례가 있다(루트 `CLAUDE.md`). 여기서는 판정 함수만 부르고 대기 루프에 안 들어간다.
...
## ✅ cycle318 — 영향 인덱스가 낡아도 아무도 안 알려주던 것

**cycle317b 를 커밋한 직후 인덱스를 재생성하니 779줄이 추가됐다** — 그날 만든 테스트가
통째로 빠져 있었다. 후속 B 가 「여러 줄 import 를 놓쳐 39파일 46건이 사라진」 결함을 고쳤지만
**고쳐도 재생성을 안 하면 같은 결과**다. 낡은 인덱스는 `affected.py` 가 빈 목록을 돌려주게 해
회귀 테스트를 **조용히 건너뛰게** 한다 — 붉어지는 것보다 나쁘다.

`test_cycle318_impact_index_freshness.py`(5케이스)가 도구를 임시로 돌려 현재 인덱스와 비교한다.

🔴 **글자 그대로 비교하면 이 가드는 영원히 붉다.** 내용이 같아도 파일이 달라지는 이유 셋 —
...
셋 다 지도의 내용과 무관하므로 **파싱해서 그 도구가 소유한 구역만** 비교한다 —
정확히 `affected.py` 가 읽는 것이다. ③은 사소한 결함이지만 이 사이클의 범위가 아니다.

⚠️ **재생성 순서는 루트 `CLAUDE.md` 가 정본**(백엔드 먼저, 프론트 나중). 뒤집으면 내용은
같은데 들여쓰기가 통째로 바뀌어 **diff 가 12만 줄**이 된다(실측).

뮤테이션 = 모듈 하나를 지우고 가드가 **이름까지 짚는지** 확인.

## ✅ cycle319 — 병렬 작업 덮어쓰기 덫 (위 09:2x 절의 사고 후속)
...
`test_cycle319_no_clobbered_module.py`(3케이스) — 덮어쓰기는 거의 항상 **다른 파일의 통째
사본** 모양이므로 ① `src/` 안 동일 내용 쌍(기준선 **0쌍** / 145파일) ② **이름이 같고 계층이
다른** 모듈끼리의 내용 일치(`market_regime.py` 가 engine·models·routes 셋에 있다)를 덫으로 놓았다.
뮤테이션 = 사고를 그대로 재현해 두 가드가 파일 쌍을 이름까지 짚는 것을 실측.

⚠️ **덫이지 울타리가 아니다** — 부분 덮어쓰기는 못 잡는다. 본체는 루트 `CLAUDE.md`
「병렬 작업」 절의 두 습관이다(쓰는 에이전트는 `isolation: "worktree"` · 띄우기 전 `git add -A`).
기존 `_SRC_TREE_DIGEST` 핀도 `src/` 변경을 전부 붉히지만 **의도한 변경에서도 붉어서**
재핀이 반사적이 되면 사고를 통과시킨다 — 이 덫은 의도한 변경에 조용하다.

## ✅ cycle320 — 인덱스 통계가 도구 순서에 달려 있던 것 (cycle318 이 걸어낸 결함)

`stats.unmapped_modules` 는 **어떤 테스트도 가리키지 않는 모듈의 명단**이다.

| 도구 | 하던 일 | 결과 |
|---|---|---|
| `build_index.py` | 자기 목록으로 **덮어썼다** | 프론트 항목 5개가 매번 사라졌다 |
| `build_index_frontend.mjs` | 중복 제거 없이 **이어붙였다** | 두 번 돌리면 프론트 항목이 쌓였다(정렬이 숨긴다) |

둘 다 `backend`/`frontend` **섹션은** 정성껏 보존하면서 이 목록만 빠뜨렸다 — 그래서 방향이
명확했다. 각 도구가 **자기 접두사**(`src/` · `frontend/`)만 책임지고 나머지는 그대로 둔다.
복원 후 8개(백엔드 3 + 프론트 5) 전수 유지 확인. `test_cycle320_*.py` 4케이스 +
고치기 전 코드로 되돌리는 돌연변이 확인.

🔴 이 목록이 조용히 짧아지면 **「가려진 모듈이 줄었다」로 읽히는데** 실제로는 아무것도
안 좋아진 것이다 — 통계가 거짓으로 좋아지는 것이 결함의 본체다.

## 🔴 배포 실측 발견 — 문서 한 줄이 매매 백엔드를 재시작시켰다
...
그 커밋 자신이 바꾼 파일은 **테스트 1개**뿐이다. `full` 을 유발한 것은 누적 diff 안의

```
src/db/CLAUDE.md      ← 후속 D 가 고친 「dkstock.cloud 매크로 일일 스냅샷」 한 줄
```

**문서화된 함정 (b) 그대로다** — `src/**` 는 확장자 무관 이미지 입력이라 `src/*/CLAUDE.md`
수정도 full 이다(루트 Dockerfile 의 `COPY src/` 가 `.md` 까지 담는다는 구성적 사실).
토요일이라 무해했지만 **평일 09:00~15:30 이었다면 D6 위반**이고, 16:00~20:00 애프터마켓·
08:00~08:50 프리장도 같다.

실질 비용 = **디렉터리별 `CLAUDE.md` 를 고치는 문서 동기화 작업이 장외 창에만 묶인다.**
`/sync-docs` 가 매 코드 변경 사이클의 필수 단계라 이게 드물지 않다.
...
| 커밋 | 모드 | 컨테이너 | 그 커밋이 실제로 바꾼 것 |
|---|---|---|---|
| `4b862d9` 16:40 | **full** | **Recreated**(16:41 시작) | 테스트 1개 — 하지만 누적 diff 에 `src/db/CLAUDE.md` |
| `0c6f840` 17:26 | **none** | `Running` — 시작 시각 **여전히 16:41** | 테스트 3 · 도구 2 · 문서 3 (`src/` 0건) |

뒤쪽이 「배포가 매매를 안 건드렸다」의 실측 증거다 — 마커 `0c6f840` · attempt 없음 · `/health` 200.
...
| 결정 | 무엇 | 집행 |
|---|---|---|
| **D1** | VCP 완화 G안 적용 | ✅ `PUT /api/strategies/vcp_breakout/params` — `base_max_days 75→40` · `pullback_count_min 2→1` · `last_pullback_max 0.12→0.15`. DB 확인 완료. **후보 1 → 8** (실험실 10 과 다른 것은 stage4 가 80→83 으로 움직여서다 — 가격이 갱신됐다). 코드 `DEFAULT_PARAMS` 는 안 바꿨다(운영 DB 가 정본, 차이는 실측표 메모리에 기록) |
| **D2** | 매크로 첫 화면 107초 → ⓒ 둘 다 | ✅ cycle314 — nginx `/api/macro/` `proxy_read_timeout 60s→120s`(두 템플릿) + **신규** `tools/ops/macro_prewarm.sh`(00:05 KST) + 실측 107초 기준 가드 **추가**(기존 「CSV 최악 32초」 검사는 남겨 둔다). crontab `5 15 * * *` 등록 완료 |
| **D3** | SK케미칼 트레일링 미집행 | ✅ ⓐ 현행 유지 — **결함 아님**(아래 절). 코드 조치 없음 |
| **D4** | 외부 백테스트 서버 | ⚠️ 요청 결과 = **복구 대상이 없다**(아래 절). 전제가 바뀌어 사용자 결정으로 되돌림 |
| **D5** | 압축 로그 | ✅ cycle313 — EC2 `.gz` **37개 삭제**(logs 2.0G→**1.9G** · `.gz` 0개 · 회전 파일 16개) + `src/main.py` **무압축 금기** 주석 + `_SRC_TREE_DIGEST` 재핀. ⚠️ **보관 20일은 이 사이클이 아니라 09-18 cycle309 가 이미 넣은 값**이고, 「6.4G→2.0G · 루트 3.4G→7.8G」도 그쪽 성과다 — 두 창을 합산하지 않는다 |

커밋 `2f91565`(cycle313·314) — 가드 2037 passed. 06:24 push → CI success → **Deploy success 06:34**
(run 35397387182, **full 모드** — `src/main.py` 가 `BACKEND_RE` 히트). 마커 `2f91565e` · attempt 없음.
...
| 부팅 로그 | ERROR·CRITICAL 0. `주말 — 월요일 09-21 07:45까지 대기 (49시간)` |
| nginx | `default.conf:143` · `tls.conf:83` 둘 다 `proxy_read_timeout 120s` |
| prewarm 스크립트 | 5엔드포인트 전부 **200**. cron 환경(`env -i PATH=/usr/bin:/bin`) 모사에서도 동작 |
| crontab | `5 15 * * *` 등록 완료(=00:05 KST). 첫 실행은 오늘 밤 |
| VCP 파라미터 | 재시작 **후에도** `40 / 1 / 0.15 · full · 2.5` 유지(DB 에서 다시 읽는다) |

🔴 **오늘은 토요일이라 장이 열리지 않는다** — D1 완화의 체결 확인은 **월요일 09-21** 부터다.

**다음 확인 시점 (시각을 틀리지 않게)**

| 언제 | 무엇 | 근거 |
|---|---|---|
| **09-20(일) 00:05** | prewarm cron 첫 자동 실행 (`logs/macro_prewarm.log`) | crontab `5 15 * * *`(UTC)는 09-19 아침에 등록했으므로 첫 발화가 다음 00:05 이다 |
| **09-21(월) 07:45** | 기동 후 VCP 후보 수가 8 근처인지 | 주말 동안 프로세스는 살아 있고 세션만 대기한다 |
| **09-21(월) 09:00~** | VCP 첫 체결 여부 | — |
| **09-21(월) 20:30 이후** | 일봉 적재 `mode=` 가 `incremental` 로 떨어졌는지 | 🔴 **주말에는 적재가 돌지 않는다** — 실측 `auto_stock.log.2026-09-12`·`09-13` 의 `[stock_master_daily_load_summary]` **0건**, `09-14`(월) 4건. 토·일 20:30 을 기다리면 안 된다 |

### 🔴 D3 판정 — SK케미칼(285130) 고지로 트레일링은 결함이 아니다

샹들리에 = `57,000 − 2.5 × ATR` = **50,799.59원** (ATR ≈ 2,480.164 — 표시값 2,480.16 으로 계산하면 50,799.60 이 되어 0.01 어긋난다. 결과값을 정본으로 쓴다).

- 정규장 마지막 틱 15:30:23 **50,700원** → 조건 충족. 그러나 **15:30~16:00 완전 휴식**(cycle295 설계)에 막혔다.
- 애프터장(16:00~20:00) **틱 35건, 최저 50,800원** = 청산선보다 **0.41원 위**. 한 번도 안 닿았다.
- 일일 리포트의 가설(「틱이 안 와서 `on_tick` 이 평가를 못 했다」)은 **반증됐다** —
  `[kojiro_trailing]` 로그는 `current_price <= chandelier` 일 때만 찍힌다(`kojiro.py:1041-1047`).
  틱은 왔고, 평가도 됐고, 조건이 0.41원 차이로 안 걸린 것이다.

### 🔴 D4 판정 — 서버는 다운이 아니라 **철거**됐다. 그리고 dkstock.cloud 도 같이 갔다

`stock-manager-4d` 세션이 같은 AWS 계정(670441838377)에서 확인했다 —
**2026-08-18 12:56 KST root 가 `TerminateInstances`** 로 **두 인스턴스를 동시에** 없앴다.

| 인스턴스 | 주소 | 우리가 쓰던 용도 |
|---|---|---|
| `i-05c673384e1093d61` | 43.202.187.5:3846 | 외부 백테스트 MCP |
| `i-009db8569d9bd8324` | 3.39.188.29 = **dkstock.cloud** | **매매가 보는 매크로 레짐** |

`infra/terraform.tfstate` 리소스 0개 = terraform destroy 로 스택 전체 철거. 지금 그 계정의
EC2 는 `auto-stock-server`(3.38.228.74) 하나뿐이다. **기동할 대상이 없다.**

**auto_stock 쪽 실측이 정확히 일치한다:**
...
- `auto_regime_adjust = **false**` 라 `cash_usage_ratio` 자동 조정 경로가 **애초에 꺼져 있다**.
  현재 `cash_usage_ratio = 1.0`(기본, 전액 사용)로 고정. 레짐은 원래 **관찰 지표**라 매수를
  차단·축소하지 않는다(루트 `CLAUDE.md`).
- `parameter_recommendations` 는 **7전략 × 매일 정상 INSERT** 된다. 빠진 것은
  `backtest_summary` 뿐(09-11~09-18 42행 NULL). graceful 설계가 실제로 동작했다.
- 즉 지금 치르는 비용은 **매일 실패 행 6건 INSERT + 관찰 지표 공백** 둘뿐이다.

### 사용자 결정 필요 (보고서 결정 카드)

「D4는 복구요청」의 전제가 바뀌었다 — 복구가 아니라 **재구축(t3.micro + 50GB EBS + Docker
이미지 27.5GB + MCP 3846)이냐, 포기냐**의 선택이다.

- 포기하면 `KIS_MCP_ENABLED=false` · `DKSTOCK_REGIME_ENABLED=false` 로 내린다.
  **기능 토글 끄기는 승인 항목**이고 `.env` 편집이라 장외 창 + 수동 `docker compose up -d` 가 필요하다.
- 🟣 **레짐 쪽은 대체재가 있다** — 우리 `macro/` 컨테이너(`/macro` 5섹션)는 dkstock.cloud 와
  **같은 코드**(macro_lite vendor)라 `base_url` 만 로컬로 돌리면 파싱이 무변경이다. ⚠️ 다만
  **두 출처 값 대조는 아직 안 했다** — dkstock 이 철거돼 비교 상대가 없어졌고, 대체 경로는
  `macro/data/macro_regime_history_seed.json` 147행과의 날짜별 대조다(이관 2단계).
  3단계(주소 전환)까지 마치면 재구축 자체가 불필요해진다. 철거 사실이 그 이관의 시급성을 올렸다.
- 🔴 **재구축의 실익은 7전략 중 3전략뿐이다** (2026-09-19 실측, `target_date=2026-09-18` 14행 전수):

| 상태 | 건수 | 전략 | 이유 |
|---|---|---|---|
| `failed` | 6 | momentum · volatility_breakout · donchian_swing | 서버 부재 |
| `skipped` | 8 | bull_flag_breakout · kojiro · long_tail_volatility · vcp_breakout | 우리 쪽 YAML DSL 미구현(Phase 4-bis 로컬 어댑터) |

  서버를 되살려도 **skipped 8건은 그대로**다. 즉 재구축으로 돌아오는 것은 momentum·VB·donchian
  세 전략의 백테스트뿐이고, **실매매 비중이 가장 큰 kojiro 는 어느 쪽이든 백테스트가 없다**.
...
| 커밋 | 내용 | 상태 |
|---|---|---|
| `5421a90` | cycle295(15:30~16:00 완전 휴식 · 갭 홀드 제거 · bool 왕복 · 손절 재주문 매핑) + 4단계 방향 기록 | 01:03 배포 완료, 실측 8항목 통과 |
...
| `a61ded9`~`10af1c8` | **cycle296 + cycle297** + 문서 개편 3~6/6 | 16:00:12 push · CI/Deploy 성공 · 16:11 배포 |
| `19bf744` | **NUMERIC 응답 문자열 시정** — 거래내역 가격·매매손익 `-`(사용자 신고) + 대시보드 최근자산 천단위 소실 | 같은 push 에 포함 · 배포 완료 |
...
| 21:30 | 정산·일일 로그 분석 | ✅ 21:30:20 리포트 저장 완료(findings 6) |
...
일봉 채움 뒤 **같은 데이터로** 완화안을 하나씩 돌려 비교했다(`prepare()` 직접 호출, 읽기 전용).

**현행 깔때기** — 3583 → 1068(시총·거래대금) → 818(진입차단) → 723(일봉길이)
→ **80(EMA 정렬, −643)** → 61(베이스) → **5(Pullback, −56)** → 1(거래량) → **후보 1**
...
- **A 가 역효과인 이유** = `effective_ema_long = min(ema_long, 보유 − uptrend_days − 5)` 라
  `uptrend_days` 를 줄이면 실효 장기선이 **길어져** 더 빡빡해진다. 직관과 반대다.
- **E 가 단독 최강**인 것이 메모리 `project_vcp_zero_fills` 의 가설을 확증한다 —
  `_detect_base` 가 **가장 긴 적격 박스**를 골라 `base_high` 를 사실상 3개월 신고가로 만든다.
  박스 상한을 줄이면 돌파선이 내려온다.
- **D 는 무효** — 거래량 수축은 지금 병목이 아니다(5→1 단계에서 4개만 죽인다).
- 🔴 **4단계(EMA 정렬) 723→80 은 손대지 않았다.** 미너비니 원설계라 사용자 결정 D3 로
  50/150/200 을 막 맞춘 참이고, 여기를 풀면 전략 정체성이 바뀐다.

⚠️ 전부 **파라미터 값**이라 사용자 결정 항목이다. 적용은 `PUT /api/strategies/vcp_breakout/params`
(즉시) + `strategy_config` UPDATE(다음 재시작). 되돌리기는 같은 PUT 한 번.

## 🔴 2026-09-18 09:37 VCP 전환 D-day 실측 — 전환은 먹었고, **남은 구멍이 드러났다**
...
스냅샷 = `2026-09-17T22:56Z` = **09-18 07:56 KST** 부팅 prepare. 스위치(06:00 PUT) 이후다.

### 🔴 근본 발견 — 일봉 backfill 범위가 VCP 스캔 범위보다 좁다

5단계 탈락 사유에 **실효 장기선이 찍힌다**: `50EMA / 150EMA / **121EMA**`·`**74EMA**`·`**114EMA**`.
200 이 아니다. 종목마다 다르다 — `effective_ema_long = min(200, 보유 − 25)` 라 **보유 행수가 그대로 노출**된다.

실측(2026-09-18):

| 집합 | 종목 | 225행 이상 | 평균 행수 |
|---|---|---|---|
| 지수 편입(KOSPI200∪KOSDAQ150) | 348 | **348 (100%)** | 232 |
| 비지수 | 1,526 | **0 (0%)** | 125 |

**VCP 는 628종목을 평가하는데 깊은 일봉을 가진 것은 348종목뿐이다.** 나머지 약 280종목(45%)은
여전히 보유 100~165행이라 실효 정렬이 `50/65/75`~`50/140/140` 수준 = **간격 10 안팎의 동전던지기가 그대로다.**
원인 = `scanner.py` 의 225일 backfill 분기 조건이 `is_vcp_universe = is_kospi200 or is_kosdaq150` 이라
**지수 편입 종목만** 깊게 채운다(`_DAILY_LOAD_VCP_BACKFILL_DAYS` 는 그 분기 안에서만 쓰인다).

### 판정
- **전환은 먹었다.** 348종목은 실제로 200EMA 로 판정된다(탈락 사유에 121·74 가 찍히는 것 자체가 깊이가
  판정에 반영된다는 증거다 — 종전에는 전 종목이 75 로 묶여 있었다).
- **5단계가 61→69 로 는 것은 전환 실패가 아니다.** 간격 10 에서 `EMA_mid > EMA_long` 이 사실상 추첨이었고,
  간격이 벌어진 348종목에서는 추세가 실제로 살아 있으면 통과한다. 상승 국면에서는 늘 수 있다.
- **되돌리지 않는다**(사용자 결정 2026-09-18 D2).

### ✅ 결정·구현됨 — backfill 범위 확대 (선택지 ⓑ 변형, cycle302)

사용자 승인 2026-09-18 09:5x("전부 담는게 좋을듯한데? VCP평가대상이 어떻게 바뀔지 모르잖아.
나중에 다른 전략에서 더 긴 시계열을 쓸 수도 있고" → 계획 제시 후 "응 그렇게 하자").

**구현** = `scanner.py` backfill 분기에서 지수 소속 판정 제거(`use_deep_backfill = existing_count < 225`)
+ 그 판정에만 쓰이던 `vcp_universe_tickers` 집합 삭제. 적재 대상(index ∪ 자격 ∪ 보호 = 962종목) 전부가
같은 목표 깊이를 받는다. 8영역 sha 재핀 13곳 + `_SRC_TREE_DIGEST` 이동 완료. 나머지 7영역·`scheduler.py` diff 0.

🔴 **적재 시각은 앞당기지 않았다** — 무거운 비용이 **1회성**이기 때문이다. 정상 운영(수렴 후)은
종목당 증분 1콜 = **962 콜 · ≈120초**(종료 20:32)로 현행과 같고, 그 사실을 가드
`test_cycle302_backfill_scope_expansion.py::test_g302_3_converged_universe_costs_one_call_per_ticker`
가 봉인한다. 첫 채움(≈2,886 콜 · ≈345초)만 `TIME_QUOTE_TOKEN_REFRESH`(20:45) 불변식 창(20:35~)을
1분 침범하므로 **스케줄이 아니라 사람이 한 번 돌린다**.

**남은 운영 조치 (배포 후)**
1. 🔴 **첫 채움 수동 실행** — 배포(21:35 이후) 직후 `POST /api/stock-master/daily/refresh?force=1`.
   장 완전 종료 시각이어야 한다(20:45 토큰 재발급 창·21:30 정산과 겹치지 않게).
2. **D+1 실측** — ⓐ 비지수 종목의 `count_by_ticker` 가 125 대 → 230 대로 자라는가 ⓑ 그 다음 밤
   `[stock_master_daily_load_summary]` 의 **`mode=` 가 `incremental` 로 떨어지고** `elapsed_ms` 가
   120초대로 돌아오는가(`mode=mixed` 로 남으면 수렴 못 한 종목이 있다는 뜻 = 상장 이력이 짧은
   코호트 규모를 그 자리에서 잰다 — `summary["mode"]` 는 `backfill_count`/`incremental_count` 에서
   나오므로 추가 코드 없이 읽힌다) ⓒ VCP 5단계 탈락 사유에서 `74EMA`·`114EMA`·`121EMA` 가
   사라지고 `200EMA` 로 수렴하는가 ⓓ `funnel` 통과 종목 수 변화.

**알려진 귀결 2건(설계상 수용)**
- `< 50 → 100일 단발` 분기는 225 > 50 이라 **도달 불가**다(신규 상장도 첫 밤에 225일을 탄다).
  관계 핀 = `test_g302_8b_deep_target_dominates_incremental_threshold`.
- **상장 이력이 225 영업일보다 짧은 종목은 수렴하지 않는다** — 매일 밤 3콜을 쓰고 목표에 못 닿는다.
  자본 위험 0, 규모는 위 ⓑ 로 잰다. 구조적 시정(시도 마커 등)은 하지 않았다.

**미처리로 남긴 것 1건** — 상수 이름 `_DAILY_LOAD_VCP_BACKFILL_DAYS` 는 이제 VCP 전용이 아닌데
이름이 그대로다. 정정하려면 `src/api/condition.py` 호출자 주석이 딸려 움직여 "프로덕션 변경은
`scanner.py` 한 파일" 제약을 깬다. 정의 자리에 "VCP 전용이 아니다" 를 못 박아 뒀고, **다음에
`scanner.py` 를 승인받아 만질 때** 아래 cycle300 후속 ②(줄번호 정정)와 함께 처리한다.

## 🔴 재기동 시 시세 구독 공백 — 프로세스 부재 시간과 다르다 (2026-09-17 확정)

**시세 구독 공백은 프로세스 부재 시간보다 길다.** 배포창 규칙(D6·D8)이 말하는 "재시작 1~5분"은
프로세스 부재만 가리키므로, 손절이 실제로 멈추는 시간을 그 값으로 읽으면 과소평가한다.
2026-09-17 16:11 배포 실측 = 프로세스 부재 56초(`[tick_blind_boot] downtime_secs=56`) ↔
시세 구독 공백 5분 16초(16:11:55 부팅 완료 → 16:17:12 148종목 ACK), 보유 11종목 청산 평가 0회.

**구조** = 15:20 이후 기동 경로에는 사전 구독 호출이 **하나도 없고**, 유일한 구독 경로인
`_scan_loop` 은 `while` 진입 직후 `sleep` 을 **먼저** 한다(`SCAN_INTERVAL=300` `scheduler.py:79`).
cycle298 이 그 첫 회차 지연을 호출부 인자 `first_delay` 로 열어, 선행 구독이 없는
15:30 POST_NXT 전환 경로(`:876`)만 `first_delay=0` 으로 즉시 구독하게 했다. 09:30 진입(`:846`)은
선행 인라인 구독이 있어 **인자를 주지 않는다**(0 을 주면 같은 조건검색·구독을 두 번 한다).
계약 상세 = `src/engine/CLAUDE.md` 의 `_scan_loop` 항목.

### 기동 시각별 첫 시세 구독까지 (코드 확정)

분기는 전부 `scheduler.py:743` 의 `now` **단일 값**으로 갈린다(그 뒤 재대입 0건). 그 시각은
체결통보·H0UNMKO0 구독을 마친 뒤라 실질적으로 **「체결통보 구독 완료 시각 T」** 다.

| 기동 시각 T | 첫 시세 구독 | 공백 | 경로 |
|---|---|---|---|
| T < 07:45 | 07:59 | 고정 | run_daily 가 07:45 대기 → presubscribe |
| 07:45 ≤ T < 07:59 | 07:59 | 최대 14분 | `:746-748` 이 07:59 까지 대기 |
| 07:59 ≤ T ≤ 09:00:05 | 즉시 | **0** | `:775` presubscribe 즉시 발화 |
| 09:00:05 < T ≤ 09:30 | 즉시 | **0** | `:816` 중간 부팅 분기 |
| 09:30 < T < 15:20 | 즉시(+스캔 수십초) | ~0 | `:836` + `:846` scan_task 생성 |
| **15:20 ≤ T < 15:30** | **15:30** | **최대 10분** | `:852-853` `_scan_task=None` → `:863` 15:30 대기 → `:876` 이 `first_delay=0` 으로 생성 |
| **15:30 ≤ T < 19:50** | **즉시** | **≈0** | `_wait_until(advance_if_passed=False)` 즉시 반환 → `:876` `first_delay=0` |
| 19:50 ≤ T < 20:00 | 없거나 곧 해제 | 사실상 종일 | `:888-889` 가 scan_task cancel, `:890` `unsubscribe_all()` |
| T ≥ 20:00 | 다음 영업일 07:59 | 기동 거부 | `TIME_SESSION_START_CUTOFF` |

### 그 공백에 무엇이 죽고 무엇이 사는가

**산다** — 매수·매도 체결 등록(체결통보 H0STCNI0 은 시세와 독립 채널·독립 콜백).

**죽는다** — 손절·트레일링·익일청산 평가 **전부**. 청산 평가의 유일한 입구가
`risk.py:686` `check_exit_signal` 이고 그 문은 `risk.on_tick` 뿐인데, 시세가 0이면 열리지 않는다.
오늘 보유 11종목이 5분 16초 동안 한 번도 평가되지 않았다.

**보조 수단도 없다** — `_swing_rest_poll_loop` 는 09:00:30~15:20 창 밖이라 60초마다
시간 가드에 걸려 그냥 잔다(`scheduler.py:2907`). `_swing_buy_poll_loop` 는 09:05~09:30 이라
그 시각 이후 기동분은 첫 iteration 에서 영구 종료한다. `open_price_rest` slow 라운드도 15:20 까지다.
**KRX 애프터마켓(16:00~20:00) 청산을 시각 앵커로 도는 루프는 없다.**

**WS 4중 안전망 전부 무력** — K stale watcher(120초)·`_resubscribe_stale_priority`(5분)·
재연결 복원·F1 은 모두 **기존 구독 집합**을 소스로 쓴다. 구독이 0이면 stale 집합이 공집합이라
즉시 return 한다. `POST /api/realtime/resubscribe` 도 같은 이유로 `resubscribed=0` 이다.
`POST /api/trading/restart` 는 09:30~15:20 기동 경로에서 300초 타이머를 처음부터 다시 돌린다
(15:30 이후 경로는 `first_delay=0` 이라 즉시 구독한다).
구독 0 에서 새 종목을 붙일 수 있는 라우트는 `POST /api/realtime/channel-probe` 뿐인데
보유·후보 종목을 **409 로 거부**하므로 이 공백의 수단이 아니다.

### 판독 함정

- **`[tick_blind_boot] market_blind_secs=0` 을 「손절 사각 없음」으로 읽지 않는다.**
  그 지표의 창은 평일 **09:00~15:30** 이라 16:00~20:00 애프터마켓 blind 는 구조적으로 0 이다.
  오늘도 `market_blind_secs=0` 이었지만 실제로는 5분 16초 blind 였다.
- 공백 중 매수가 체결돼 **신규 포지션이 생겨도 자동 구독되지 않는다** — 다음 회차까지 blind 다.
- `_sync_positions_from_balance` 는 독립 15분 task 가 **아니다** — `_scan_loop` 안에서
  3회차마다 돈다(`:2495-2496`, `sync_counter % 3`). 루프가 안 돌면 이것도 안 돈다.

### 시각 상수 2개는 런타임 미사용이다

| 상수 | 코드 사실 | 실제 경계 |
|---|---|---|
| `TIME_BOOT = 07:55` | **런타임 참조 0건**(정의 `:58` + 주석뿐) | `_boot()` 는 `start()` 안에서 즉시 = 07:45 자동 기동이면 그 직후 |
| `TIME_POST_NXT_OPEN = 15:40` | **런타임 참조 0건**(정의 `:62` 뿐) | 스케줄러 전환·시가 확정은 `TIME_KRX_MAIN_CLOSE`(15:30) 하나다 |

정본 문서(`src/engine/CLAUDE.md` `TIME_*` 표 · `_workspace/00_leader_trading_rules.md` 시각표 ·
`docs/architecture.md` · `README.md` 외 4곳)에는 이 사실이 반영돼 있다. **상수 자체는 남아 있다** —
테스트 6파일이 참조하고 그중 2건이 값을 핀한다. 상수 정리(삭제)는 미착수 카드다.
⚠️ `session._BOARD_SCHEDULE` 의 보드 경계(`main` ~15:39:59 · `post_nxt` 15:40~20:00)는 **별개 축이고
여전히 사실**이다 — "스케줄러가 15:40 에 무엇을 한다"는 서술과 혼동하지 않는다.

### 그래서 무엇을 하면 되는가

1. 배포창 판단에 **프로세스 부재가 아니라 위 표의 공백**을 쓴다. 보유가 있으면
   15:20~15:30 기동(최대 10분)과 19:50~20:00 기동(사실상 종일)을 특히 피한다.
2. **남은 구간은 `15:20 ≤ T < 15:30` 기동의 최대 10분 하나다.** `:852` else 가
   `_scan_task=None` 으로 두고 15:30 까지 기다리기 때문이다. 닫으려면 그 else 분기에
   보유 ∪ `_pending_next_day_clear` **HIGH 구독 1회**를 넣어야 한다 — `scheduler.py` 변경이라
   **사용자 승인 + 라인 상한(<3,900) 확인** 대상이고 아직 착수하지 않았다.
   🔴 **이 10분은 무해하지 않다** — `order_engine._market_rest_now(15:22)` 는
   `(False, "krx_sendable")` 다(KRX 종가단일가 K4 가 우리 호가유형 `("00","01")` 을 받는다).
   **손절 주문은 접수되는데 평가할 틱이 없는 구간**이라, 주문 자체가 전량 컷되는
   15:30~16:00 휴식 창과는 성격이 다르다. 그래서 이 카드에 실효가 있다.
3. 재기동 직후에는 `GET /api/realtime/subscriptions` 의 `total` 이 0 을 벗어나는 시각을
   기록한다 — 그것이 손절 재개 시각이다.
...
- 🔴 **cycle303(매크로 이식 1단계) 후속 — EC2 첫 배포 전에 사람이 해야 할 일 2건.** 절차 원문 = [`docs/macro-lite.md`](../docs/macro-lite.md).
  ...
  ⓑ **seed 1회 적재**(`python -m macro_lite.seed`, 멱등) — 안 하면 하이일드 **차트**의 10년·5년 구간이 FRED 가 주는 최근 3년치로 줄어든다(초과분은 영구 재취득 불가). 백분위·5단계 심리는 매 호출 신선 fetch 로 계산되므로 seed 와 무관하다.
...
- **cycle307 후속 ① 원 패키지(stock-manager)로 넘길 항목 2건** — 우리가 고칠 자리가 아니다(`macro/macro_lite/` 는 무수정 vendor).
  ⓐ **백분위 baseline 이 누적 store 가 아니라 「그 호출의 신선 rows」다.** CSV 성공일은 881행·JSON 폴백일은 787행 기준이라 **날마다 잣대가 약 4개월 어긋난다**. 상대 세션이 "baseline 을 누적 store 로 통일" 을 개선 후보로 접수했고, macro-sentinel 자문 뒤 별도 진행한다. 그 전까지 우리는 **CSV↔JSON 순서를 바꾸지 않는다**(바꾸면 매일 짧은 창으로 판정한다).
  ⓑ `fetcher.py::_fetch_fred_oas` docstring 이 "전 기간 baseline (1996-12-31 ~ 현재, 약 28년) 사용" 이라고 적혀 있는데, ICE BofA 라이선스로 FRED 가 약 3년치만 준다(같은 파일 아래 주석이 그 사실을 적고 있어 서로 어긋난다). 이 문장이 "누적 store = 백분위 baseline" 오독의 진원지였다.
- **cycle307 후속 ② `docs/macro-lite.md` 를 정본 목록에 넣을지** — cycle303 에서 생겼는데 루트 `CLAUDE.md` 「문서 규약」 의 정본 열거에도, `.claude/commands/sync-docs.md` 의 `for f in …` 루프에도, 같은 파일 「대상 문서 정본 목록」 표에도 없다. 성격은 등재된 `docs/backtest-monitoring.md` 와 같은 운영 가이드다. ⚠️ **넣으려면 그 세 곳을 한 커밋에 같이 고쳐야 한다** — 하나라도 빠지면 `test_doc_overpaint_zero.py::test_sync_docs_bash_loop_covers_every_canonical_doc` 가 즉시 RED 다(루트 CLAUDE.md 파생 집합 == sync-docs 루프 집합을 강제한다).
- **cycle303 후속 ③ macro 패키지 테스트(195건)를 CI 에 편입할지** — 지금은 `pyproject.toml` 의 `testpaths=["tests"]` 가 우리 스위트와 분리하고 CI 도 안 돈다(`macro/requirements.txt` 설치 시간 + pandas/numpy 가 매매 CI 잡에 들어온다). 별도 job 으로 뗄지, 그대로 둘지 미결.
...
- **cycle300 후속 ② `scanner.py` 인용 줄번호 정정 — ✅ cycle302 가 처리**(그 사이클이 같은 주석 블록을 승인받아 만졌다). `vcp_breakout.py:162-164` → `:309-314` 로 고치면서, 함께 낡아 있던 "VCP 전략 실제 사용 100일" 도 현재 규칙(`daily_fetch_depth_mode` 가 깊이를 정하고 운영은 `"full"`)으로 덮어썼다
...
| 커밋 | 사이클 | 무엇 | 가동 |
|---|---|---|---|
| `15e8179` | **292·293·294** | 시세 채널을 KRX/NXT 전용 2채널로 분리 — 통합 채널 `H0UNCNT0` 폐기 | `system_config.tick_channel_resolver_mode = enforce` (09-14 21:5x PUT) |
| `8e1ec68` | — | 테스트 영향 인덱스 재생성 | — |

CI success(9분 24초) · Deploy success · `.deployed_sha = 8e1ec68a` = HEAD · backend 21:55 재생성.

## 🔴 N-1 해소 — 프리장 `H0NXCNT0` 라이브 프레임 실증

09-14 시점의 유일한 미실증은 **프리장 08:00~08:50 에 `H0NXCNT0` 이 실제로 프레임을 보내는가** 였다
(그날 프로브는 같은 채널의 **애프터 구간** 대리 증거였고, 자동 원복은 09:03 에야 돌아 그 창을 못 잡는다).

**09-15 08:00~08:10 실측 — 해소됐다.**

| 시각 | subscribed | acked | fresh | stale |
|---|---|---|---|---|
| 08:00:04 | 139 | 139 | 61 | 78 |
| 08:03:06 | 139 | 139 | **79** | 60 |
| 08:08:09 | 139 | 139 | **80** | 59 |
| 08:10:10 | 139 | 139 | 77 | 62 |

- `fresh` 가 61 → 80 으로 **증가**했다 = 프리장 NXT 체결 프레임이 실제로 들어온다.
- 경보 **0건**(`ALERT_PRE_BLIND`·`ALERT_KRX_BLIND` 둘 다 미발화).
- 리졸버가 설계대로 갈랐다 — `[tick_channel_resolve] mode=enforce`:
  `reason=pre_window|nxt_feed → H0NXCNT0` (대다수) · `reason=pre_window|no_feed → H0STCNT0`
  (009830·035720 등 `nxt_tradable=False`). 후자는 프리장에 시장이 없으므로 stale 인 것이 정상이고,
  잔여 stale ≈60 의 대부분이 이 코호트다.
- `provenance=cptt_key` — 07:46 KRX 일괄 도장 창의 오염을 피해 출처 검사가 작동했다.

## 🔴 프리장 실매매 실증 (09-15 08:01~09:05) — cycle274 enforce 결정의 첫 표본

프리장에서 **매수 3건과 손절 2건이 실제로 체결됐다.** 관측이 아니라 실매매다.

| 시각 | 종목 | 전략 | 행위 | 손익 |
|---|---|---|---|---|
| 08:01:47 | 124500 아이티센글로벌 | LTV | 매수 @34,200 | |
| 08:08:37 | 000670 영풍 | LTV | 매수 @56,200 (GTP `div=27 gtp=1`) | |
| 08:17:26 | 101730 위메이드맥스 ×4 | LTV | 매수 @5,930 | |
| **08:24:39** | 101730 | LTV | **프리장 STOP_LOSS** @5,630 | **−1,200** |
| **08:36:54** | 000670 | LTV | **프리장 STOP_LOSS** @53,500 | **−2,700** |
| 09:00:16 | 000500 가온전선 | BFB | TRAILING_STOP @241,000 | **+13,500** |
| 09:01:16 | 124500 | LTV | STOP_LOSS @32,350 | **−1,850** |
| 09:05:01 | 316140 우리금융지주 | donchian | 매수 @36,000 | |

오늘 실현 = **+7,750**(BFB +13,500 / LTV −5,750).

**확인된 계약 3건** — ① `_PRE_MARKET_EXIT_EVAL_STRATEGIES = {long_tail_volatility}` 화이트리스트가 의도대로
동작했다(LTV 만 프리장 청산 평가 → 08:24·08:36 실제 손절 / BFB·kojiro 는 `[pre_market_exit_deferred]` 보류)
② cycle291 GTP(27) 프리장 매수 배관 작동 ③ cycle245 ρ축 관측 `[oversized_fallback] ticker=000670 qty=1
notional=56200 cap=25378 ratio=2.21` — K_ρ=2.5 이하라 통과(캡이 실제로 재고 있다).

### 🔴 AI 매수평가(shadow) 3/3 적중 — enforce 규약 결정의 근거

| 종목 | score | would_block | post_order_drift | 실현 |
|---|---|---|---|---|
| 124500 | **36** | True | −43.5bp | −1,850 |
| 000670 | **48** | True | −123.5bp | −2,700 |
| 101730 | **44** | True | −100.5bp | −1,200 |

세 건 모두 `min_score=70` 미달로 "막았어야 한다" 고 판정했고 **세 건 모두 손실로 끝났다.**
`enforce` 였다면 **−5,750 을 피했다.** 000670 근거 = "돌파 초과폭 97.77bp가 작고 갭 7.11%로 추격"
(`rsi14=76.2 pos_ch20=149.5`). N=3 은 작지만 방향·근거가 일치하는 첫 실표본이다 —
09-10 밤 보고서의 **cycle274 enforce 규약 결정 카드**에 이 표를 붙인다.

⚠️ 판독 주의 = LTV 3건 모두 **1주 폴백 랏**이라 금액이 작다. 적중률과 금액 규모를 섞어 읽지 않는다.
...
## 🔵 운영 설정 실측 갱신 (2026-09-15 08:0x)

아래 「운영 설정 실측」 절(09-12 작성)에 더한다:

- **전략 비중 Σ = 1.00** — kojiro 0.40 · donchian/BFB/VCP 각 0.15 · momentum/VB/LTV 각 0.05.
  7전략 전부 `enabled=True`. 종전 "Σ≈2.98 오염" 은 역산 추정이었고 **실측이 반증했다**.
- ✅ **`max_lot_ratio_mult` 는 7전략 전부 2.5 로 해소됐다** (2026-09-19 00:1x 전수 재실측).
  09-15 에 「VCP 만 20.0」으로 적혀 있던 cycle245 §7.1 선반영값(ρ축 캡 해제 다이얼)이 그 뒤 복원됐고,
  09-10 밤 보고서의 「K_ρ 20 만료」 카드는 **더 이상 대기 항목이 아니다.**
  → VCP 완화안을 적용해 첫 체결이 나와도 **랏 명목 통제는 정상으로 걸린다.**
  🔴 이 항목이 **「실측표도 낡는다」의 실증**이다 — 값이 결정에 걸리면 표를 믿지 말고 다시 조회한다.
- `max_lot_units` 는 `sizing_mode` 무관 4전략(donchian·kojiro·VCP·BFB) 모두 2.0(K=20 선반영 해소됨).
  momentum·VB·LTV 는 키 자체가 없다(터틀 캡 범위 밖).
...
# 🗄️ (지난 이력) 09-08(화) 대기열 — 주간 사용량 소진, 09-10(목) 오후 재개까지 새 작업 보류
...
## 🗄️ 2026-09-14(월) 판독 계획 — 한 장으로 모은 것 (2026-09-13 작성, **실행 완료**)
...
| **08:00~08:50** | 프리장 라우팅 | `[order_channel]` **미발화**(프리장은 base 유지 = routed==base) · 매도 시 `[sell_market_preconvert_pre_nxt]` 정상 · 주문은 `00` 지정가로 나간다(NXT `N1` 이 받는 코드) | `reason=krx_by_clock` 이 뜨면 프리장 clause 가 깨졌다 |
| **08:50~09:00** | **주문 없는 구간(설계)** | **주문 0건** — NXT 는 08:50 에 끝나고(`N2` `order_divisions=()`) 우리는 KRX 시가단일가(`K1` 08:20~09:00)를 쓰지 않는다. 15:30~16:00 휴식과 같은 성질이다. 전 기간 실측 0건 | 주문이 **하나라도** 나가면 이상이다. 특히 `reason=krx_by_clock` 이 뜨면 매도 사전 지정가 변환(`in ("NXT","SOR")` 조건)이 함께 죽어 **시가 단일가에 시장가**가 나간다 — cycle287 이 그 구간을 `pre_nxt_keep` 으로 묶은 이유다. ⚠️ **지금의 0건은 규칙이 막은 결과가 아니다** — NXT 프리마켓이 끝나면 체결 틱이 멈추고 프리장에는 REST 폴이 없어(`_swing_buy_poll_loop` 는 09:05 부터) `risk.on_tick` 이 불리지 않을 뿐이다. **프리장에 REST 폴을 붙이는 날 이 구간이 열린다** — 그때는 명시 시각 컷이 필요하다 |
...
| 6 | **KRX 애프터마켓 20:00 종료 시 미체결 주문 처리** — 자동취소인가 잔존인가 | 정본 어디에도 없다. `src/engine/CLAUDE.md` 는 **정규장** 마감 후 자동취소만 말한다. 부분체결 잔여가 20:00 을 넘기면 다음 날 무엇이 남는지 모른다 |

### ⚖️ 16:00~20:00 재시작 배포를 금지할 것인가 — 결론을 미룬다 (2026-09-13 04:0x)

보고서 검토가 이 항목을 발의했고, **적대 검증이 그 논리에 구멍 넷을 찾았다.** 양쪽을 다 적는다.

**금지하자는 근거**
- 09-14 부터 16:00~20:00 이 KRX 애프터마켓 실시간 연속체결이고 **2,459종목**이 합류한다.
- cycle287 이 그 창의 청산 경로를 열었다 ⇒ 보유 11종목의 손절 주문이 그 시각에 나간다.
- 재시작은 1~5분 tick blind 를 만든다(D6 가 정규장에 대해 막는 바로 그 위험).

**금지하면 안 될 수도 있는 근거 (적대 검증)**
1. **"경로가 새로 열렸다" 는 부정확하다.** 그 위험은 09-14 이전에도 15:40~20:00 에 있었다 —
   cycle287 커밋 메시지가 **09-08 15:45 필옵틱스 NXT 애프터 실체결**을 근거로 그 경로를
   의도적으로 보존했다고 적는다. 달라지는 것은 **규모**(KRX 2,459종목·실시간 연속)다.
...
4. 🔴 **가장 중요** — 위 절이 말한 대로 **장중 킬스위치가 없다.** 급성 롤백의 유일한 길이
   "1커밋 revert + 재배포" 인데, 16:00~20:00 배포를 금지하면 **미검증 손절 코드가 처음 도는
   그 4시간에서 유일한 제동 수단을 빼는 것**이 된다.

**⇒ 그래서 이 결정은 킬스위치 등재와 묶어서만 답할 수 있다.**
...
| 킬스위치 없음 **+** 배포 허용(현행) | 급성 시 revert 배포가 가능하지만 그 배포 자체가 tick blind 를 만든다. **차악** |
| 킬스위치 등재만, 배포는 현행 | 손이 생기고 창은 안 좁아진다. **첫날 권고** |

**내 권고** = 마지막 줄(킬스위치 등재만 먼저, 배포 창은 09-14 실측 뒤 재논의).
등재가 월요일 전에 불가하면 **배포 금지를 넣지 않는다** — 넣으면 위 표의 "가장 위험" 칸이 된다.
그 대신 16:00~20:00 에는 (a) 지켜보고 (b) 끄는 대신 먼저 보고하고 (c) 필요하면 20:00 이후 revert.
...
- `src/**` 변경이라 cycle248 분류상 **full 모드 = backend 재시작**이다. 09-14(월)는 **정규장 중 · 보유
  11종목**이라 D6 로 금지. **20:00~21:35 도 금지**(D8). 창은 **15:30~16:00**(cycle295 착지 전에는 15:30~15:40) 또는 **21:35~익일 07:45**.
...
- ✅ **F-292-5 해소(cycle316 후속 E)** — `docs/HARNESS_CHANGELOG.md` 에 cycle290·291 행을 09-13 자리에 넣었다.
  각 커밋 메시지(`d4cfdec`·`adabbdf`)를 원천으로 썼고, cycle291 의 「거래소발 취소는 통보를 만들지 않는다」
  전제는 09-14 실측으로 반증됐으므로 **적지 않았다**. 실측 해석 정정(「18건 중 1건 5.6%」는 누출률이 아니라
  하한이다)은 그 행에 같이 남겼다.
...
> `session.py` 절. TR·필드 변경 = [`docs/kis/README.md`](../docs/kis/README.md) 「제도 변경 공지 반영」 절
> (+ 각 필드 자리에 직접 반영). 여기에는 **우리가 할 일과 실측 계획만** 둔다.
> (2026-09-12 사용자 지시로 구 `docs/market-changes-2026-09-14.md` 를 해체해 세 곳으로 나눴다.)

### 🔵 운영 설정 실측 (2026-09-12 토 · cycle286 배포 전)

코드 기본값과 **운영 DB 가 다르다.** 이 차이가 09-14 위험 판독을 바꾸므로 먼저 읽는다.

| 항목 | 코드 기본값 | 운영 DB 실측 | 무엇이 달라지나 |
|---|---|---|---|
| LTV `tradable_boards` | `("pre_nxt","main","post_nxt")` | **`["main","pre_nxt"]`** — `post_nxt` 없음 | LTV **야간 매수가 이미 꺼져 있다**. 그래서 cycle286 C2-a 는 오버나이트 노출을 `post_nxt` 로 **옮기는 게 아니라 없앤다**(적대 검증 C-M2 는 코드 기본값 기준이라 운영에선 성립하지 않는다). 반대로 누군가 `post_nxt` 를 다시 넣으면 그 노출이 되살아난다 |
| 7 전략 `exchange` | 전략마다 상이 | **전부 `NXT`** (2026-09-13 04:3x 결정 D3 로 `SOR`→`NXT` PUT 완료, SOR 잔여 0) | cycle286 C4-a 의 거래소 조건(`∈ {"NXT","SOR"}`)이 운영에서는 **여전히 항상 참**이다(`NXT` 도 그 집합 안) ⇒ 실효 게이트는 **창(08:00~08:50) 단독**. cycle287 의 `_CLOCK_ROUTED_BASES` 도 `("NXT","SOR")` 이라 09:00 이후 KRX 강제는 **불변**. D3 로 실제로 바뀐 것은 **프리장(08:00~08:50) 주문이 나가는 거래소**뿐이고, `pre_nxt` 보드를 가진 전략이 LTV 하나뿐이라 프리장 *매수* 변화는 LTV 한정이다 (나머지 6전략은 프리장 *청산* 경로만 이동). 거래소 축은 `exchange=KRX` 로 바꾸는 날 발효 |
| 보유 포지션 | — | **11종목** | 09-14(월) 09:00 부터 이 종목들이 손절 감시 대상이다. 아래 P0 가 이것들에 걸린다 |
| LTV 15:20 이후 매수 실적 | — | **전 기간 0건** | C2-a 는 **아직 안 터진 구조적 구멍**을 닫는다(사후 시정이 아니다). 그래서 배포 즉시 눈에 보이는 변화가 없는 것이 정상이다 |

### ✅ P0 (cycle287, 2026-09-12 — Green 완료, 배포·병합 대기) — 09-14(월) 16:00~20:00 보유 종목 손절 수단 확보

~~`OrderDivision` 은 `LIMIT="00"` / `MARKET="01"` 둘뿐~~ → `KRX_AFTER_BEST="44"`(1차,
최유리지정가, `ORD_UNPR="0"`) · `KRX_AFTER_LIMIT="41"`(폴백, `step_down(5)`) 2종 추가.
**규칙 1(시각이 거래소를 정한다)** — `order_engine._route_exchange_by_clock` 이
09:00~15:30·16:00~20:00(2026-09-14 신설 KRX 애프터마켓)엔 `KRX`, 그 밖(프리장·
15:30~16:00·15:40~16:00 NXT 애프터 등 우리가 보낼 수 있는 호가유형이 그 시각 그
거래소에 없는 창)엔 현행 base(SOR/NXT)를 유지 — 판정은 `market_state.MARKET_TABLE`
단일 정본만 읽는다(시각 리터럴 0). **규칙 2** — KRX 애프터마켓에서 시장가 거부 시
`44`(1차)→`41`(폴백) 구조적 폴백 1회(분류 무관, msg1 원문이 완전 미지라 키워드에
폴백을 인질로 주지 않는다) + 봉인 3종(TTL 등록 무조건·일일 포기 래치 5회·TTL 축
`_exit_capable_krx_window` 재정의). `api/order.py` 는 docstring 만(본문 byte 동일).

**Green 완료 사실** — 백엔드 3파일(`src/models/order.py`·`src/engine/order_engine.py`·
`src/api/order.py`) 딱 그 셋만 바뀌었다(8영역 diff 0, `scheduler.py` 3,897L 무변경,
전략 7파일·`strategy_base.py`·`market_state.py`·`sell_rejection.py`·`api/balance.py`
byte 동일 — sha 핀 다중 확인). Red 231 케이스 + 전체 백엔드 9,517 PASS(TZ=UTC 재실행
동일). **아직 안 한 것** — 커밋·push(메인 세션 소관, 이 사이클 규약상 금지) ·
cycle287b(프론트 `exchange` 파라미터 정직화·`param_catalog`·`market_state` FINDINGS 1행
· DB `exchange` SOR→NXT 마이그레이션, **코드 배포 후 D+1 판독 통과 뒤에만**) ·
09-14 실측(§1-A 표 §7 D+1 판독 항목 — 44 접수·체결가·슬리피지, msg1 원문 등 여전히
미지). 배포는 15:30~16:00·21:35~07:45·주말 장외 창(보유 중, D6). 16:00~20:00 은 KRX 애프터 실시간 체결이라 제외.

### 09-14 이후 첫 영업일에 재야 할 것 (순서대로)
...
| 18:10 저장분 vs 18:45 재조회 | **15/15 일치** → 현행 제도에서 18:10 은 충분했다 |
| 과거 4영업일 | 전부 최종값과 일치 → 7일 증분 창이 다음 날 보정한다 |

→ 09-14 부터 거래가 20:00 까지 이어지므로 **구 18:10 적재는 매일 부분 거래량을 담게 된다.**
cycle283(2026-09-12 배포, `fc82697`)이 봉확정 컷오프를 `20:00`·적재를 `20:30` 으로 올려 이 축은 닫혔다.

### 이 변경이 뒤집은 기존 판단

**cycle280(시세 채널 전환) 자문의 CRITICAL "저녁 시세 공백" 은 성립하지 않는다.** 그 판단은
`docs/kis/` 스냅샷(= 변경 전)을 근거로 "KRX 시간외 체결은 `H0STOUP0` 별도 TR" 이라고 읽은 결과다.
09-14 이후 그 세션 자체가 없고 애프터 체결은 기존 체결 채널로 `MARKET_CLS_CODE=3` 으로 온다.
따라서 자문의 3창 권고(08:55~15:33 KRX / 15:37~08:50 NXT)는 방향이 반대이고 원안 2창
(08:00~08:50 NXT / 08:55~20:00 KRX)이 맞다. **다만 그 자문의 나머지 지적은 여전히 유효하다** —
41슬롯 `tr_id` 무관 카운트 · `_ticker_to_session` 단일 키 · grace 키 불일치 · 등가 비교 9곳
(채널 전환 배관 문제라 제도와 무관).

## 🆕 대기 중인 사이클 (2026-09-11 밤 발의)

| 사이클 | 무엇 | 상태 | 착수 조건 |
|---|---|---|---|
| **cycle283** | 저녁 창 재설계 — 봉확정 `20:00` · 일봉 적재 `20:30` · 정산 `21:30` · 기동거부 경계 분리 · metrics 1차 저장 `20:05` · `insert_log_report` upsert | **진행 중** (워크트리 `auto_stock_wt283`, 브랜치 `cycle283-evening-window`) | 사용자 확정 완료 |
| **cycle284** | 종목마스터 파일 **조건부 GET 게이트** — 시계 기반 → 내용 기반. 적재 `16:30 → 20:40` | **명세만** → [`_workspace/red/cycle284_master_file_conditional_get_spec.md`](red/cycle284_master_file_conditional_get_spec.md) | **cycle283 병합 뒤** (둘 다 `scanner.py` 8영역 접촉) |
| **cycle285** | 장운영상태 화면 확장 — **야간작업 현황 타임라인 + 실시간 장운영(VI·서킷브레이커)** | **명세만** → [`_workspace/red/cycle285_market_state_ops_panel_spec.md`](red/cycle285_market_state_ops_panel_spec.md) | **cycle282 병합 + cycle283 배포 뒤** (신규 작업·바뀐 시각이 이 화면의 입력) |

### cycle283 — 왜

09-11(금) 16:05 배포 재기동 → 16:14 즉시 적재가 오늘 봉 1,005종목을 저장 → 18:10 정기 적재가
908종목 `skipped_fresh`. 실측: 일봉 OHLC 는 15:30 확정이나 **거래량은 시간외 동안 계속 증가**
(개별주 40종목 중앙값 +0.51%, 최대 +13.97% 현대차). 09-14 부터 애프터마켓이 20:00 까지 돌아
18:10 적재는 **매일** 부분 거래량을 담게 된다. 원인은 적재 시각이 아니라 오늘봉 컷오프(15:40)라
세 값을 함께 옮긴다. 순서 불변식 = `cutoff(20:00) ≤ 적재(20:30) < 정산(21:30)`.

🔴 **`TIME_SETTLEMENT` 은 정산 시각 말고 `start()` 기동 거부 경계(`scheduler.py:573`)와
`run_daily` 일자 전환(`:1081`)도 겸한다.** 기동 거부만 분리하고 `:1081` 을 안 바꾸면
20:00~21:30 에 **무한 재시작 루프**가 된다(run_daily 가 "아직 장 종료 전"으로 보고 `start()` 를
부르는데 `start()` 는 거부하고 즉시 반환). 두 지점이 같은 상수를 쓰는 것이 계약이다.

### cycle285 — 왜

cycle282 의 장운영상태 표는 **시각 기반 정적 표**다("지금 몇 시니까 어떤 상태여야 한다").
거기에 실측을 얹으면 **예정 대 실제**가 한 화면에서 대조된다 — 표는 "정규장" 인데 실제로는
서킷브레이커로 멈춰 있다면 그 불일치가 즉시 보이고, 표는 "20:30 일봉 적재" 인데 마지막 성공이
어제면 그날 적재가 없었다는 뜻이다. 지금은 그걸 알려면 EC2 에서 `system_logs` 를 grep 해야 한다.

**재료는 거의 다 있다(신규 수집 로직 0 목표)** — 실시간 장운영은 `market_operation_monitor` +
기존 `GET /api/realtime/market-operation`(백엔드 신규 0), 작업 현황은 `system_config` 의
`task_last_success_*` 마커 8개 + `refresh_progress` 5키 + 산출물 DB(자문·정산·리포트·일봉 헤드).

🔴 **세 가지를 반드시 화면에 적는다** — ① 시각은 `scheduler.TIME_*` 에서 **읽는다**(리터럴 0건,
안 그러면 cycle283·284 가 시각을 바꾸는 순간 거짓 화면이 된다) ② VI·거래정지는 `H0UNMKO0` 가
**005930 + 보유·익일청산만** 구독하므로 "VI 0건"은 "VI 없음"이 아니다 — `관측 n종목 기준` 병기
③ 서킷브레이커는 KIS 전용 필드 부재로 **휴리스틱 추정**(cycle186)이라 "추정" 표기 + 근거 병기.

### 🟨 보류 — 월요일(09-14) 운영상태 리뷰 뒤 판단 (사용자 결정 2026-09-12)

조사 완료했고 **둘 다 지금은 하지 않는다.** 판단 근거는 09-14 실측이 나온 뒤에 다시 본다.

**① 2패스 일봉 적재 (15:3x 1차 + 20:30 2차)** — 사용자 발의("증권사는 하루 두 번 적재한다").
적대 검증 **반증 성공(viable=false)**. 근거 셋:
(a) **정상일엔 편익 0** — 6전략이 `prev_idx` 로 오늘 날짜 봉을 잘라내므로 D 봉은 D 당일 아무도 읽지
않고 D+1 아침에야 읽힌다. 2차(force)가 도는 날엔 1차가 쓴 값을 같은 저녁 `ON CONFLICT DO UPDATE`
가 통째로 덮어 DB 최종 상태가 cycle283 과 **동일**하다. 1차는 매일 1,005종목·121초·KIS 1,005건을
쓰고 다음 날 상태에 흔적을 남기지 않는다.
(b) **유일하게 다른 날(2차 유실)엔 더 나쁘다** — cycle283 은 D 봉 부재로 `[daily_head_stale]`
WARNING 이 뜨는데(시끄러운 결손), 2패스는 1차가 D 행을 만들어 `head == expected` 가 되어 **완전
무음**이다(조용한 오염). 그 D 봉은 09-14 부터 애프터마켓 4시간이 빠진 부분값이다.
(c) 커트오프를 15:3x 로 내리면 그 시각~20:30 의 **모든** 실행(재기동 immediate·수동 refresh)이
부분봉을 확정봉으로 저장할 권리를 되찾는다 = 09-11 사고 창의 복원. 그리고 `scheduler.py` 라인
여유가 **1행**이라 배선 10~12행은 leaf 이전 없이 물리적으로 안 들어간다.
⚠️ 증권사 관행 자체는 옳다 — 그쪽 1차는 **당일 마감 데이터를 즉시 고객에게 서비스**하려는 것이고,
우리는 D 봉을 D+1 아침에만 읽으므로 그 서비스 가치가 없다. 소비 패턴이 다르다.

**② boot 재배치 (immediate 적재를 prepare 앞으로)** — 사용자 발의, 시각 게이트는 사용자가 기각
("장중이라도 환기하는 게 당연"). 적대 검증 **조건부 가능(viable=true)** 이나 보류.
빠져 있던 것 넷: (a) 적재 본체가 registry `positions`·`_pending_next_day_clear` 를 읽어(cycle273d D5)
prepare 앞에 두면 보호 집합이 **항상 공집합** = D5 무음 no-op — 하필 이 변경이 노리는 시나리오에서
(b) 마커 기록 ↔ D5 보호가 정면 충돌(쓰면 보호 실행 소실, 안 쓰면 스윕 2회 ~242초)
(c) 예외 미포장은 하루 손실이 아니라 **60초 주기 재시도 폭주** (d) 시각 게이트 `<09:00` 은 08:30
재기동과 07:59 사전구독 양쪽에서 샌다 → 상한은 **07:56**(= `TIME_PRESUBSCRIBE` 07:59 − 180초).
...
R1 아침에 prepare 가 DB 를 읽는 그 자리에서 KIS 폴백이 발화한다. 종목 단위·자동·이미 배선돼 있고
매일 도는 경로라 검증된다. 순서 문제도 D5 문제도 없다. (달력 기준으로 조이면 월요일마다 전 종목
폴백이므로 **반드시 영업일 기준**이어야 한다.)

**③ 후보 확정을 저녁으로 + 서버 중단을 새벽 2시로 (사용자 발의 2026-09-12)**
*"당일 시세를 활용하지 않는 전략들은 미리 전일 저녁에 후보들을 확정해서 조회할 수 있으면 좋겠어.
시세를 안 쓰는 전략들의 후보확정은 당기고 서버중단 시간을 새벽 2시정도에 극히 짧게 가져가는 방안."*

📌 **절반은 이미 돌고 있다 — 결과를 버리고 있을 뿐이다.**
`_evening_funnel_capture_once`(cycle171, 16:20)가 **이미 5전략 `prepare()` 를 호출한다**
(`scheduler.py` 의 `(2) 5 전략 prepare` 블록). 그런데 산출물을 `strategy_funnel_snapshots` 에
`is_provisional=True` 로 **관찰용만** 적고, 실제 후보(`_scanned_tickers`·`_candidates`·`_targets`)는
메모리에 두었다가 다음 날 아침 boot 의 prepare 가 통째로 덮어쓴다. 즉 같은 계산을 하루에 두 번 한다.
⇒ 제안의 정확한 표현은 "당긴다" 가 아니라 **"이미 당겨진 것을 버리지 않는다"** 이고, 그만큼 싸다.

🔴 **시각 문제가 아니다 — `prev_idx` 가 달력 날짜로 판정한다 (2026-09-12 정정).**
6 전략 전부 `prev_idx = 1 if candles[0].get("stck_bsop_date") == today_str else 0` 이다
...
**같은 코드가 실행 시점의 달력 날짜에 따라 다른 헤드를 본다:**
...
⇒ **일봉 적재를 아무리 앞당겨도(15:3x 1차든 20:30 2차든) 같은 날 저녁 prepare 는 구조적으로
D-1 을 본다.** D 봉을 적재해 두면 오히려 `prev_idx` 가 그것을 *일부러* 잘라낸다.
사용자 발의 "15:30~16:00 에 일봉적재+후보확정을 연속으로" 로는 **해소되지 않는다.**
cycle273f 가 이미 알고 적어 뒀다 — 16:20 캡처 주석 "캡처는 전일 봉 기준(전략 절단이 날짜 비교라 동일)".

✅ **그리고 사용자의 "새벽 2시" 가 바로 그 해답이다.** 자정을 넘기면 `today_str = D+1` 이 되어
`prev_idx = 0` → **헤드가 저절로 D 가 된다. 전략 코드 무접촉.**
두 발의가 우연히 같은 시각인 것이 아니라 **구조적으로 같은 해답**이었다.

```
20:30  일봉 2차 적재(거래량 최종) → 20:40 마스터(cycle284) → 21:30 정산
──── 자정 ────
02:00  배포 적용 + 후보 확정 prepare   ← today_str = D+1 → 헤드 = D ✓
07:45  boot 는 확정 후보를 읽기만      ← prepare 제거 → tick blind 축소
```

대안(`prev_idx` 에 기준일 주입)은 7 전략 전부 접촉 + 진입 판단 직결이라 훨씬 비싸다.

🔴 **선행 과제 — 02:00 은 세션 밖이다.** `run_daily` 가 정산(21:30) 뒤 07:45 까지 자므로 그 사이
주기 task 가 **존재하지 않는다**. 이것이 "루프 밖 시각" 으로 **두 번 기각된 계열**이다
(일봉 23:00/05:00 안 · cycle270-B 토큰 21:30 = 매일 0회 발화).
⇒ 세션 수명 재설계 또는 **FastAPI lifespan 레벨 독립 task** 가 선행. 후자는 cycle279(프로세스
분리 1단계 `llm_worker`)와 같은 축이므로 그 설계와 함께 본다.

**검토할 것 (월요일)**
1. **영속 구조** — `_scanned_tickers`/`_candidates`/`_targets` 를 어떤 테이블·형상으로 담나.
   전략마다 `_candidates` 의 값 형상이 다르다(ATR·밴드·베이스 메타 등). 통째로 JSONB 인가 정규화인가.
2. **밤새 바뀌는 것의 재검증** — 거래정지·관리종목 지정, 신규 상장. 아침에 **가볍게 재검증**
   (`_is_master_blocked_for_entry` 1단계 차단 7건만 다시 보기)이 필요한가, 전면 재계산이 필요한가.
   cycle284(마스터 조건부 GET)가 이 축의 선행이다 — 마스터가 하루 늦으면 재검증이 무의미하다.
3. **VB·LTV 는 반쪽만 가능** — 후보 집합은 전일봉 기반이라 저녁 확정이 되지만 **목표가는 당일 시가**가
   필요하다(cycle272 KRX REST, 09:00:35~). 후보만 저녁에 확정하고 목표가는 09:00 이후로 분리한다.
4. **momentum 은 대상 밖** — `prepare()` 가 빈 stub 이다(실시간 전일종가 +29% 돌파).
5. **아침 boot 가 얼마나 짧아지나** — 현행 boot 실측 소요가 미지다(`[tick_blind_boot] downtime_secs`).
   prepare 가 빠지면 그만큼 tick blind 가 줄고, 07:45~09:00 예산에 여유가 생긴다. **정량이 먼저다.**
6. **새벽 2시 중단** — 세션은 이미 21:30~07:45 사이 비어 있어 02:00 재기동은 아무것도 끊지 않는다.
   진짜 과제는 "배포를 그 시각에 **모으는**" 운영 수단이다(낮에 push → 02:00 적용하는 지연 배포).
   현행은 `git push origin main` → GitHub Actions 즉시 배포다. cron 기반 지연 적용이 필요하다.
   배포 창이 한 점이 되면 "언제 배포해도 되나" 를 매번 판단할 필요가 없어지는 것이 본 이득이다.
7. **되돌릴 수 있는가** — 저녁 확정 후보가 틀렸을 때 아침에 전면 재계산으로 낙하하는 경로
   (fail-open)를 반드시 둔다. 저장된 후보를 못 읽거나 형상이 안 맞으면 현행대로 아침 prepare.

**📌 09-14(월) 실측 2건이 세 설계 모두의 전제다 — 아직 가설이다**
1. KIS 일봉 `acml_vol` 이 **몇 시에 확정되는가** (20:05 / 20:20 / 20:40 / 21:00 재조회 비교).
   20:00 뒤면 cycle283 은 상수 2개(`_DAILY_LOAD_TODAY_BAR_CUTOFF`·`TIME_STOCK_MASTER_DAILY_LOAD`)만
   뒤로 밀면 된다.
2. **애프터마켓이 일봉 고가/저가에 반영되는가.** 참이면 영향 범위가 거래량 축 3전략(donchian/BFB/VCP)
   에서 **가격 축 5전략**(+VB/LTV)으로 넓어진다.

**📌 조사가 바로잡은 사실 2건 (문서·대화에 잘못 퍼져 있던 것)**
- **`TIME_BOOT`(07:55)은 죽은 상수다** — 소스 참조 0건. 실제 boot 진입은 `TIME_AUTO_START` = **07:45**.
- **LTV 는 15:30~16:00 에 매수를 발사할 수 있다** — 시각 컷이 없고 `tradable_boards` 에 `main`·
  `post_nxt` 를 둘 다 가져 15:20~19:50 내내 돌파 시 진입 가능. 그 구간 매수가 막혀 있는 것은 시간
  게이트가 아니라 나머지 6전략 각자 컷의 우연한 합이다. → 사용자 발의 **보드 재설계**(아래)로 이어짐.

---

### cycle284 — 왜

**KIS 마스터 파일은 `last-modified` 헤더와 조건부 GET(304/200)을 지원한다**(09-11 실측).
오늘 파일 갱신 = **KST 18:55:03**. 우리 적재는 16:30 → **갱신 2시간 25분 전**이라 매일
*전날 판*을 쓴다. 오늘 기준 상태 플래그 6종 3,549종목 대조 불일치는 **0건**이었으나 1일 표본이다.

부수 효과가 크다 — 마커에 `last_modified` 를 매일 남기면 ① 18:55 가 규칙적인지 ② 심야·새벽
**2차 갱신**이 있는지가 자동으로 측정된다. ②가 실재하고 거기 **익일 상장 종목**이 들어온다면
"마스터는 상장일 당일부터 담는다"는 현 결론이 뒤집힌다(현재 실측: 22:51 판에 미래 상장일 0건).

⚠️ **익일 상장 종목 사전 확보는 범위 밖이다.** `HHKDB669107C0`(상장정보일정)은 추가상장 중심,
...
- **⚠️ 이 데이터는 EC2 디스크(`_observation_dumps/`)에 쌓인다 — "로컬"(이 Mac)이 아니다.**
  목요일 세션 재개 시 최우선으로 `scp -i auto-stock.pem ubuntu@3.38.228.74:~/auto_stock/_observation_dumps/'*'  <로컬경로>` 로 끌어와야 한다(와일드카드라 아래 확인된 개별 파일명은
  **참고용** — scp 자체는 디렉터리 전체를 가져오므로 누락 걱정 없음). 프로브 성공 종목은
  `p1_ticker.txt`/`p2_ticker.txt`(수요일 설계, ~~`active_tickers.txt`~~ 는 화요일 최선시도 설계의
  구 파일명 — 09-09 실측으로 정정) 에 남는다.
  - **09-08(화) 최선시도 산출물**: `probe_2026-09-08.log` · `probe_dump_2026-09-08.json` ·
    `probe_raw_frames_2026-09-08.txt`(11,378행).
  - **09-09(수) 정확한 설계 산출물(09:15 완료 확인됨)**: `probe_2026-09-09.log` ·
    `probe_dump_2026-09-09.json` · `probe_raw_frames_2026-09-09.txt`(6,193행) · `p1_ticker.txt`
    (=010170) · `p2_ticker.txt`(=100840). 판독 요지 — P-1(010170, nxt_false 무송출 종목)을
    H0STCNT0 로 직접 구독하니 09:00:09~09:14:22 사이 5,527프레임 정상 수신(채널 리졸버 발의에
    긍정 증거 추가). P-2(100840) NXT→KRX 전환은 08:49:58 NXT 마감 침묵 → 08:55:01 전환 →
    09:00 KRX 개장 전 침묵 → 09:00:23 부터 정상 수신, 프레임 중복·유실 없이 깔끔했다.
  - **매일 20:00 KST 공통 덤프**(`dump_daily_observations.sh`): `kojiro_gap_observe_YYYY-MM-DD.json`
    · `open_scope_observe_YYYY-MM-DD.json` · `open_source_compare_YYYY-MM-DD.json` ·
    `breakout_open_confirm_YYYY-MM-DD.json` — 09-08 치는 이미 있음(9/8 11:00 KST), 09-09 치는
    오늘 20:00 KST 에 생성 예정(아직 없음).
  - **재현 가능성 확인됨**: 목요일 정리 명령(`crontab -l | grep -v ...`)은 **크론탭 등록만
    지우고 `_observation_dumps/` 아래 스크립트 파일 자체는 건드리지 않는다** — `wed_activate_nxt_and_p1.sh`
    /`wed_switch_p2_to_krx.sh`/`wed_dump_and_cleanup.sh` 는 정리 후에도 EC2 에 그대로 남고,
    `$(date +%F)` 로 날짜를 동적으로 잡아 로그 파일명을 짓기 때문에 **수정 없이 다른 날 재실행
    가능**(단, NXT 프리장·KRX 개장이 겹치는 08:40~09:15 KST 창에서 실행해야 의미 있음).
...
## ⚠️ 09-09(수) 프로브 원문 덤프 설계 결함 발견 + 시정 (당일 즉시 대응)

**결함**: 09-09 08:40~09:15 KST 실행한 수요일 정확 설계 프로브(P1=010170·P2=100840)의
원문 프레임 덤프(`probe_raw_frames_2026-09-09.txt`, 6,193행)가 **목적(채널별 시가 비교)을
달성 못 한다** — `websockets` 라이브러리의 DEBUG 로그가 긴 TEXT 프레임을 repr 로 축약해
**중간을 자른다**(예: `100840^084001^37100^2^500^1.37^3...63^0.71^0^0.00^0^^36600`). 시가
필드(`[7] STCK_OPRC`)가 정확히 이 잘린 구간 안에 있어 복원 불가. 09-08 설계 당시 "원문
프레임 로그 = 캡 없는 정본"이라 적었던 전제(19번째 줄 근방)가 **부분적으로 틀렸다** — 캡이
없다는 건 맞지만 필드 자체가 잘려 있다는 걸 확인 안 했다.

**부분 구제**: `[open_scope_observe]`(handler.py, **파싱된 정수 기반 — 잘리지 않음**) 마커가
MAIN 창(09:00~15:30) 게이팅 덕에 두 프로브 종목 모두 **정확히 09:00 직후 첫 틱**에서 발화해
오늘 목적을 대신 달성했다:
- `010170`(P1, H0STCNT0 무송출 종목): `oprc_hour=090009 tick_open=14260 in_main_window=true`
- `100840`(P2, 08:55~H0STCNT0 전환): `oprc_hour=090023 tick_open=37000 in_main_window=true`

둘 다 시가 시각이 08:00(NXT 프리장)이 아니라 09:00 언저리(진짜 KRX 개장)로 정확히 찍혔다 —
같은 날 000990(H0UNCNT0, 통합채널)이 `oprc_hour=080000`으로 오염됐던 것과 정확히 대비된다.
**결론(사용자 확인 포함) — KRX 전용 채널(H0STCNT0)로 구독하면 NXT 프리장 오염 없이 진짜
KRX 개장가가 나온다.** 채널 리졸버 제안(P1-7 B)을 뒷받침하는 실측 증거.

**시정 완료(09-09, 코드 변경)**: `GET /api/realtime/channel-probe` 응답의 `price` dict 에
`open_price` 키 추가(`src/routes/realtime.py::_build_probe_row`, `ticker_prices[ticker]`
의 기존 4키 중 하나를 노출만 — 신규 계산 없음). 이제 원문 로그 파싱 없이 **API 응답만으로**
채널 전환 전후 시가를 직접 비교할 수 있다(같은 종목이 채널을 옮겨도 `ticker_prices` 는
채널 무관 단일 dict라 전환 후 새 틱이 오면 값이 그대로 갱신됨). `src/routes/CLAUDE.md` 표
동기화 완료. 회귀 = `tests/unit/routes/test_cycle253_channel_probe.py` P5(2곳 `price` 딕셔너리
단언에 `open_price` 추가) — 28 PASS. **8영역 아님**(`src/routes/` — `src/realtime/**` 와는
다른 디렉터리), 읽기 전용 진단 응답 필드 추가라 매매 행위 영향 0.

**내일(09-10 목) 아침 프로브 설계 원칙(재발 방지)**:
1. 채널 비교가 목적이면 원문 텍스트 로그가 아니라 **파싱된 값**(구조화 마커 또는 위
   `GET /api/realtime/channel-probe` 의 `price.open_price`)을 1차 소스로 쓴다.
2. 원문 프레임 로그는 "구독 타이밍/전환 경계가 깨끗했는가"(중복 수신·유실 여부) 확인용으로만
   쓰고, 필드값 추출 용도로 쓰지 않는다.
3. 같은 종목을 채널만 바꿔 재구독하며 `GET /api/realtime/channel-probe`를 폴링하면 전환
   전후 `open_price`를 나란히 얻을 수 있다 — `[open_scope_observe]`의 종목당 1회/일 캡에
   의존하지 않아도 된다(이 캡은 오늘처럼 MAIN 창 게이팅과 우연히 맞아떨어지지 않으면 원하는
   순간에 안 찍힐 수 있다).

## 🟢 09-10(목) 자율 구간 진행 기록 (16:17~21:00, 사용자 허용 = 기존 결정 범위 커밋·푸시 포함 · 종료 후 아티팩트 리포트)
- **cycle272(D1 REST 시가 기준 교체) 구현·검증 완료 → 커밋·배포(09-10 밤)**: 워크플로 7 에이전트(이해 3 → 자문 → Red → Green → Verify GO, 2.54M 토큰·148분). 8영역 diff 0, scheduler 3,897L, 신규 테스트 180, 전체 7,745 PASS, 뮤테이션 27종 행위 ESCAPED 0. tester MEDIUM #1(준비 폴링 타임아웃 종결 → 일정 루프 낙하)은 커밋 전 반영(`test_c19_2` 낙하 단언 추가, 뮤테이션 KILLED 확인). D+1 12서명·롤백 = changelog 행. ⚠️ **금요일 07:45 `_boot` 시 VB·LTV enabled 유지**(20:39 재활성 상태 그대로). B안(채널 분리, 8영역 4파일)은 금요일 실측 뒤.

> **09-10 밤 자율 구간 보고서(09-11 03:5x)** = `_workspace/reports/2026-09-10_night_autonomous_work.md`(+ 아티팩트). 배포 6회(270-B·270-C·272·그룹2+핫픽스·그룹1·274) 전부 장외, D7 Basic 자격 회전 실행(새 비밀번호 EC2 `secrets/.rotated-20260910182818` — **사용자가 20:20 전 루틴 `REPORTER_BASIC_PASSWORD` 갱신 필수**). 결정 카드 회신 대기: K_ρ 20 만료 · cycle274 enforce 규약/범위 · cycle275 승인 · VCP 관측 · 한도 척도 · B안.

> **cycle274 — VB·LTV 매수 신호 LLM 평가 게이트: 자문 + shadow 배선 완료·병합 `477449b`(09-11 03:1x)**. 자문 `_workspace/domain_consult/cycle274_llm_buy_gate_20260910.md`(결정 10 + 열린 질문 Q1~Q10). shadow = 점수 기록만(`[llm_buy_score]`), 행위 0, 8영역 0. **사용자 결정 대기(enforce 설계)**: Q3 실패 규약(fail-open 권고) · Q4 판정 기간(2주 배관+4주 임계 권고) · Q7 보존(신규 테이블 여부) · Q8 donchian 편입 · Q2/Q9 enforce 스코프·래치 · **호출부 try/except(자문 "1줄" 원안 대신) + cycle233 가드 최소 확장 승인**. 후속 F-274-1 `[llm_buy_score]` 2행 분리 · F-274-2 VB `k_period=15` → 일봉 DB 경로 사망(`days=17<22`, 자문 §8.5) · F-274-3 단가 상수 이원화. D+1(09-11) 서명 = `[llm_gate_config]` VB·LTV 각 1행 · `[llm_buy_score]` 4~6행 · `_failed` <10% · cap 0행 · 매수 패턴 불변.
> **cycle276 — AI 매수평가 시점을 "매수 주문 발화 직후"로 이동 + 전용 테이블·UI 팝업(브랜치 `cycle276-order-time-llm`, 워크트리 `auto_stock_wt276`, **커밋 전**)**. 사용자 지시(09-11 05:4x)로 cycle274 의 신호 시점 훅을 전략 2파일에서 **제거**하고 `order_engine.execute_buy` 의 매수 `place_order` 두 경로 직후에서 `observe_order` 로 접수한다(8영역 중 `order_engine.py` 만 승인 접촉, A-ATOMIC 구간 byte 동일, 행위 변경 0). 신규 = migration 043 `llm_buy_evaluations`(PK 일자-계좌-종목-주문번호, 성공·실패 모두 1행) · `src/db/llm_buy_evaluations.py` · `GET /api/llm-evaluations[/{order_no}]`(계좌 마스킹) · `get_trade_pairs` 에 `buy_order_nos`/`sell_order_nos`/`pair_key` · 프론트 두 그리드 "AI 자문" 버튼 + `LlmEvaluationModal`. 래치 키 = **주문번호**, 마커 신규 `[llm_eval_persist]`, `slip_bp`→`post_order_drift_bp`(부호 의미 반대 — **cycle274 로그와 합산 금지**). **cycle274 D+1 서명은 이 배포 전까지만 유효**하고, 배포 뒤 `[llm_buy_score]` 는 "신호당 1행" 이 아니라 **매수 주문당 1행**이 된다. 남는 사용자 결정 = cycle274 Q1~Q10(enforce 설계)와 동일 + cap 초과 평가를 DB 행으로도 남길지 여부(현재는 `[llm_eval_persist] reason=cap_exceeded` 로그 1행만 — 남기려면 `eval_kind` 어휘를 현재 `'order'|'blocked'` 에서 확장해야 한다).
...
- **장 결과**: 매수 4(032820 donchian 2주 · 004990 kojiro 5주 · 066570 VB 1주 · 425420 BFB 1주) · 매도 1(066570 15:20 강제청산 **−2,000원**). 보유 7→10, 평가손익 +49,060, 순자산 2,553,398. ERROR 2건뿐(004990 잔여취소 APBK0927 — 포지션·DB 정상).
- **사용자 행위 기록(변경 금지)**: 09:09:09 비중 변경(PUT weights) — momentum 5% · VB 8% · **LTV 0% → `enabled=False`(보유 0)** · donchian 17% · BFB 17% · VCP 11% · **kojiro 42%(0.4166, 6/6 만석)**. ⚠️ 이 문서·주간 루틴 프롬프트의 "kojiro weight≈0.19" 는 낡은 값(어젯밤 DB 로드 시점 38%).
- **cycle269 D+2 = 효과 없음(실측)**: 15:45 강제 발급 7/7 성공이나 `expired=` 가 14:2x~14:50 **장중 자연 발급과 동일**(KIS 가 유효 토큰 존재 시 같은 토큰·만료 반환, `token.py:155-156` 은 응답값 그대로). 드리프트 계속(09-10 14:28~14:50). 증거 문서 `_workspace/analysis/2026-09-10_cycle269_anchor_not_moved.md`. **사용자 승인 하 17:13 1계정(fire) 실측**: revoke 0.20s→issue 0.23s, 만료 14:28:19→**17:13:51**(+24h) = 후보 A 전제 성립 → **cycle270**(leaf `quote_token_refresh.py` revoke→issue, `token.py` 무접촉) Red 8건 → **✅ Green·검증 완료·커밋·푸시 `678c233`(17:37, full 배포)** → **배포 완료 17:45**(CI `34456121116` 7m42s · Deploy `34456814515` · 마커 `cc639ad` · 재기동 6분 ERROR 0 · DB 포지션 10건 복구 · `fire` 캐시 토큰 만료 09-11 17:13:51 로드) · 문서 동기화 커밋 `cc639ad`(changelog 행·루트 표 15행·cycle269 서술 정정): leaf 단독 변경(계정별 revoke→issue, revoke 실패는 경고 후 issue 계속, 주계정 무접촉, 로그 접두 byte 보존 + ` revoked=` 꼬리), `token.py`·8영역 diff 0, 지정 4파일 52 PASS·전체 7,548 PASS, 뮤테이션 19종 KILLED 18/ESCAPED 0/등가 1, tester **GO**. **D+1 판독 — ⚠️ 시각 이력 15:45 → 21:30(cycle270-B, 사용자 승인 21:30 은 루프 밖이라 무발화 — D8 명세 §7 지적) → 19:00(cycle270-C, 같은 밤 시정)**: 배포 성공 시 **09-11(금) 19:00~19:08 첫 실행** = 계정별 `revoked=True` 7행 + 요약 `accounts=7 issued=7 failed=0` ≤19:08 · `expired` 09-12 19:0x · `[token] 분당 한도 대기` ≥8건이면 직렬화 overrun · **09-12 이후 장중 자연 재발급 0건**이 성공 서명. 09-11 장중은 옛 앵커(14:2x~14:5x 자연 재발급)가 한 번 더 날 수 있다. 후속 F-270-1(LOW) = issue 실패 로그에 `revoked=` 병기(`revoked` 초기화를 try 앞으로).
- **관측 덤프 결함(신규)**: `dump_daily_observations.sh` `limit=2000` > API 상한 1000 → 09-08·09-09 덤프 8파일 전부 147B(422 JSON). 16:4x 조치 = 3일치×6마커 `limit=1000` 로컬 구조(has_more 0, 09-08 은 purge 전 확보) + EC2 스크립트 `limit=1000` 수정(20:00 덤프로 검증). 크론 정리는 지시대로 **금요일 이후**.
- **항목 2(09:05 execute_buy 오탐) = cycle271**: Red 13케이스(5 RED/8 GREEN, `UniqueViolationError uq_trade_history_ticker_order_no_type` 정확 재현) 완료. **운영 실증** `2026-09-07 09:05:00 ERROR [swing_poll] execute_buy 실패: 003470`(현 보유 유안타증권 매수 시). 정정 2: 오늘 004990 은 이 결함 사례 아님(INSERT 성공) · "실손실 = `cached_buyable_at` 미무효화" 는 실재하지 않음(`_handle_buy_fill:1318`) — 실제 영향은 **매수 직후 HIGH WS 구독 누락(최대 5분 손절 사각, `scheduler.py:2688~2709`)** + WS 재연결 트리거 = 등급 LOW 재판정 필요. Green(권장안 b: `_completed_orders` 증거 판별 + discard, 두 경로) + sha 핀 4곳 절차 진행 예정. → **✅ Green·Verify GO(09-10 저녁)**: 헬퍼 `_insert_pending_buy_or_absorb_race`(증거 = `order_no in _completed_orders` 일 때만 흡수, 두 INSERT 지점, A-ATOMIC 무접촉), 신규 16(Red 13 + 검증 라운드 추가 3: `test_c7_2`·`test_c2_4`·`test_c2_5`) + 회귀 65 + 전체 7,564 PASS, 뮤테이션 13종 KILLED 12 + m4b 는 `test_c2_5` 로 KILLED(행위 ESCAPED 0), sha 핀 4곳 `773b4d2b…` 등록(커밋 뒤 후속 커밋으로 비움 — `test_g3_9a/9b` "항상 4곳"). → **커밋 `d4cbb58`(코드+문서) + `4d88aa6`(핀 비움) 18:49:14 푸시 → 배포 완료 18:58**(CI `34462744640` success · Deploy `34463460249` success · 마커 `4d88aa6` · 재기동 4분 ERROR 0 · DB 포지션 10건 복구 · 시세 풀 7/7 · 체결통보 구독). D+1 첫 서명 = `[buy_fill_during_insert]`(정상 흡수) · 09:05 `execute_buy 실패` ERROR 0.
- **자문 2건 완료**(커밋 `1821310`): 주간 자문 자동 적용 → **레벨 0(적용 후보 큐) 권고, weight 자동 적용 전면 제외, cycle210 안전장치 재사용 불가**(방향 게이트 조임 전용 `recommendation_engine.py:646-655`, Σ 정규화 `strategy_registry.py:39-42` — 메인 세션 대조) · 고지로 밴드폭 → **수준 최저 관문 비권고(전 이력 23건 밴드/ATR 중앙 0.128, 1×ATR 은 100% 차단), shadow 선행 + 랭킹 성분 2건 원설계 복원(`kojiro.py:1209` `macd3_slope` `/close` 누락 = 종가와 ρ+0.853 · `:1211` `band_expansion` 단일봉 분모+`1e-9`) 우선 — 매매 행위 변경이라 승인 사안**.
...
1. **cycle265(시가 오염 근본 시정 — VB·LTV 목표가 기준을 KRX 조회값으로 교체) 시기·방향 리포트**
   ...
   - **09-09(수) 09:05:44 관측 결과(2자 대조만 완료, 3자 대조는 대기)** — `[open_source_compare]`
     72행 확보(VB 42 + LTV 30). `used_src=ws`(진짜 WS-REST 비교) 62행 중 `|delta_bp|>100bp`
     (1%↑ 격차) 22종목, 최대는 000990/052690(WS 대비 REST 4.4~4.6% 높음), 000500(4.4%),
     267270(2.1%) — 시가 오염 방향(과소·과대 둘 다 존재)이 09-07 3~4일치 표본과 같은 패턴으로
     재현됨. **아직 안 한 것 = 3자 대조**(`used_open`↔`rest_oprc`↔`stock_master_daily` 그날 공식
     시가) — 09-09 치 `stock_master_daily` 는 **오늘 16:00 KST 배치 적재 이후에만** 존재한다
     (현재 12:xx, 아직 없음). 방법론 자체는 09-07 판독(`_workspace/analysis/2026-09-07_open_scope_dplus1_readout.md`
     §3, `rest_oprc` vs 일봉 시가 **98/98 완전 일치** 확인됨)에서 이미 검증됐으므로 재설계 불요,
     09-09 데이터에 그대로 재적용만 하면 된다. **사용자 결정(09-09)**: 목요일(09-10) 아침 작업
     큐에 추가 — 16:00 KST 적재 완료 후(오늘 밤 또는 내일 아침) `stock_master_daily` 09-09 행을
     끌어와 오늘 72행과 3자 대조 완성.
   - **→ ✅ 09-10 판독 완료(1단계 워크플로 16에이전트, 2렌즈 적대 검증 반박 46건 반영)**: 3일치
     (09-08/09/10) 3자 대조 `_workspace/analysis/2026-09-10_open_scope_3way_readout.md` — `used_src=ws`
     종목 **96.0%(193/201)** 가 공식 시가와 다른 값으로 목표가를 세웠고 KRX REST 는 확정 일봉과
     **나흘 343/343 일치**. 단 **진입 건수 영향은 날마다 부호가 뒤집힘(−24.4% / −3.6% / +17.1%)** →
     자문 §9 의 "−27.6%" 단일값을 승인 문서에 확정 수치로 쓰면 안 됨(레짐별 분포로 재계산). 09-10 아침
     같은 종목(100840) 채널만 교체한 A/B(NXT 38,350 → KRX 36,700 = 확정 시가)가 오염 원인이 "프리장
     체결 유무"가 아니라 **"구독 채널"** 임을 처음 보임(표본 3종목·5조합). **방향 리포트 + 카드 3장** =
     `_workspace/analysis/2026-09-10_cycle265_direction_report.md`(① 시기: 이번 주말 09-12~14 / 다음
     주말 / 관측 보류 · ② 방향: REST 기준가 먼저 / 채널 리졸버 먼저 / 병행(비권고) / ①→② 순차(자문
     권고) · ③ 범위: 비중·position_ratio·랏 캡·hold 90s·LTV 규약 무접촉 확인). 부수: `[open_scope_observe]`
...
2. **09:05 `[swing_poll] execute_buy 실패` 오탐 시정 — ✅ 09-10 cycle271 완료 — 커밋 `d4cbb58`(코드+문서) + 핀 비움 후속 커밋, 18:5x 푸시(full 배포)(기존 "고치자" 승인 근거; 배포 확인은 아래 진행 기록 절)**
   **후속(cycle271 tester r2 비차단 지적, 코드 무접촉 — 별도 승인 사안)**:
   ...
   - **F-271-3 (INFO)** `from asyncpg.exceptions import UniqueViolationError` 가 1st-party import 블록 한가운데(:26) — 핀 재산출을 피하려 커밋 전 미이동, 별도 정리 커밋(8영역 sha 재핀 절차 동반).
   - **방법론(영속)** 8영역 사이클의 뮤테이션 판정은 **sha 핀 가드를 제외하고** 읽는다 — 핀은 모든 뮤턴트를 동어반복으로 kill 해 행위 공백을 가린다(이번 m4b 가 그 사례: 핀 비운 뒤엔 가드 0 이 될 뻔했다).
   — 원인 = 체결통보가 `insert_trade` await 도중 착지하는 race(기존 가드는 "검사 전 도달" 만 덮는다).
   ~~실손실은 `cached_buyable_at` 캐시 미무효화 1건뿐 → 위험 LOW~~ **정정(09-10 tdd-engineer 코드
   확인)**: `cached_buyable_at` 은 `_handle_buy_fill:1318` 이 이미 무효화한다. 실제 영향 = 예외로
   `scheduler.py:2688~2709` `buy_succeeded` 미대입 → **매수 직후 HIGH WS 구독 누락(최대 5분 손절
   사각)** + `risk.on_tick` 경로는 `handler.py:637` raise 로 WS 재연결. **운영 실증 2026-09-07
   09:05:00 003470(현 보유)**. Red 13(5 RED)·명세 `_workspace/red/cycle271_execute_buy_fill_during_insert_spec.md`
   → Green·검증·sha 핀 절차 진행(위 09-10 기록 참조).
...
7. ~~**cycle269(보조 시세계정 토큰 매일 장중 재발급 드리프트 시정)**~~ — **✅ 09-08 22:xx
   ...
   신규 파일 `src/engine/quote_token_refresh.py`(매일 15:45 KST 강제 재발급 → 09-10 밤 cycle270-B 21:30(루프 밖 무발화) → **cycle270-C 19:00** 이동). ~~**D+1(09-09 수)
   확인 사항** — … D+2(09-10 목)부터 장중 자연 재발급이 완전히 사라지는지 확인할 것.~~
   **→ 🔴 D+2 실측(09-10) = 효과 없음.** 15:45 강제 발급 7/7 성공이나 `expired=` 가 그날 14:28~14:50
   장중 자연 발급과 **동일**(KIS 는 유효 토큰이 있으면 같은 토큰·만료를 반환; `token.py:155-156` 은 응답
   그대로). 드리프트 지속. 사용자 승인 하 17:13 `fire` 1계정 실측 revoke→issue 로 만료 14:28:19→17:13:51
   이동 확인 → **✅ cycle270(leaf 에서 `revoke()`→`issue()`, `token.py` 무접촉) 커밋·푸시 `678c233`**(09-10 17:37, **배포 완료 17:45** — CI `34456121116` 7m42s·Deploy `34456814515`·마커 `cc639ad`·재기동 ERROR 0·포지션 10건 복구·`fire` 캐시 토큰 만료 09-11 17:13:51 로드, 상세·D+1 판독 규칙은 위 "09-10 자율 구간 진행 기록" 절).
   증거 정본 `_workspace/analysis/2026-09-10_cycle269_anchor_not_moved.md`.
8. **고지로 밴드폭(`band_width` = EMA 중기-장기 너비) 을 순위뿐 아니라 최저치 필터로도 적용**
   ...
   **→ ✅ 자문 완료(09-10, `_workspace/domain_consult/kojiro_band_width_floor_filter_20260910.md`, 커밋
   `1821310`)**: 밴드폭 **수준** 최저 관문은 6→1 전환 조건과 논리 충돌·전 이력 23건 밴드/ATR 중앙 0.128
   (1×ATR 임계면 100% 차단)이라 **비권고** — 카드 ① shadow(`[kojiro_band_observe]`) 선행 / ② 정규화
   = band/ATR 1차 / ③ **랭킹 성분 2건 원설계 복원 우선**(`kojiro.py:1209` `macd3_slope` `/close` 누락 =
   종가 순위표, `:1211` `band_expansion` 단일봉 분모+`1e-9` 폭발 — 메인 세션 코드 대조 확인). ③은
   임계 신설 없는 원설계 복귀지만 **매수 순위 변경 = 매매 행위 변경 → 사용자 승인 사안**. 착수 여부 = 카드.
9. **필옵틱스(161580) 09:00 매도 미체결·중복 COMPLETED 기록 — 근본 원인 시정**
   ...
   **→ ✅ 09-10 조사 완료** `_workspace/analysis/2026-09-10_161580_root_cause.md`: 응급 정정 **반영
   확인**(0000267100=CANCELLED · 0001611100=COMPLETED · 09-08 momentum 실현 2,000원 1회). **근본 원인
   2건 확정** — ① `update_trade_status()` WHERE 에 `order_no` 가 없어 15:45 체결통보 1건이 같은
   종목·전략의 PENDING **2행을 동시에 COMPLETED** 로 덮음(`trade_history.py:107~117`; 매수 경로
   `_handle_buy_fill:1236-1239` 도 동일 구조) ② 09:00:14 세션트래커 30초 stale 이 익일청산 시장가를
   프리장 사전변환(`step_down` 지정가 38,800)으로 바꿨는데 그날 KRX 고가 38,600 이라 **종일 체결
   불가** → 열린 주문이 `selling_reconcile` 을 설계대로 붙잡아 `_selling` **6h45m26s** 잠김. 후속
   카드 = F-3 `[trade_status_multi_update]` 관측(affected>1 WARNING, 가장 싸고 무행위) · F-1/F-2 WHERE
   에 `order_no` 추가(테스트 참조 109곳/18파일) · F-4/F-5 사전변환 게이트 벽시계 이중판정(매매 행위 →
   domain-consult) · F-6 체결 0 매도 주문 만료·취소 규약(domain-consult) · F-9 CANCELLED 행 잔존
   `price=35250/profit_loss=2000` 되돌릴지(회계 영향 0) · **F-8 별건 반대 위험**: `get_daily_orders`
   (`balance.py:226-260`) 연속조회(`CTX_AREA_NK100`) 미구현 → 열린 주문 누락 시 `_selling` 이 **너무
   일찍** 풀릴 수 있음(double-sell 방향). 증거 보존: 09-08·09-09 `system_logs` 전량 덤프(7,449/7,422행)
   를 20:10 purge 전 EC2 `_observation_dumps/system_logs_2026-09-0{8,9}.json` + 로컬에 확보.
10. **주간 자문 자동 적용 확대 — domain-expert 자문 선행 (09-09 사용자 발의, 목요일 오후 4시**
    **주간한도 해제 이후 착수)** — 현재 주간 Claude 자문(PR #20 `weekly_advice_2026-09-08.md`)은
    100% 사람이 읽고 수동 반영한다. 사용자가 "프로세스를 조금 더 자동화할 수 있게 검토하자"
    는 요청에 3가지 방향(① 사이클210 안전 자동적용 패턴을 주간 루틴까지 확장 ② PR 자동 머지
    후 사람 검토만 사후 ③ 자문 결과를 자동으로 파라미터 후보 DB에 적재만 하고 적용은 별도
    승인 화면에서)을 제시했고, **①을 중심으로 진행하기로 결정**. 기존 `recommendation_engine.py`
    의 `auto_apply_recommendations()`(cycle23/cycle210, `auto_apply_enabled` 기본 False·
    `_CONSERVATIVE_KEYS` 빈 화이트리스트·weight 감액 50% 캡·`PARAM_RANGES` 화이트리스트 관문)
    가 이미 휴면 상태로 존재 — 이 패턴을 **주간 Claude 자문(PR 기반, `parameter_recommendations`
    테이블과 별개 경로)에 확장 적용**할 수 있는지가 자문 핵심 질문. 검토 대상 = (a) 주간 자문은
    PR·Notion 산출물이라 DB 행이 아님 — 자동 적용 트리거를 어디에 둘지(PR 머지 훅? 별도 파싱
    경로?) (b) 기존 cycle210 안전장치(비중 감액 전용·50% 캡·정체성 키 화이트리스트 배제)를
    그대로 재사용할지 아니면 주간 자문 전용 규약이 필요한지 (c) 승인 없는 자동 적용의 범위를
    weight 축소만으로 한정할지 파라미터까지 포함할지. **사용자 결정: "목요일 오후 4시 이후에
    주간한도가 해제된다. 그 이후에 작업하도록 대기작업에 올려두자"** — 09-10(목) 16:00 이후
    착수. `domain-consult` 스킬로 `domain-expert` 자문 선행 후 착수 여부·설계 결정.
    **→ ✅ 자문 완료(09-10, `_workspace/domain_consult/weekly_advice_auto_apply_20260910.md`, 커밋
    `1821310`)**: **cycle210 안전장치 재사용 불가** — 방향 게이트가 조임 전용(`recommendation_engine.py:646-655`)
    이라 이번 주 권고 3건 중 0건 통과, weight 감액은 `allocate_funds` Σ 정규화(`strategy_registry.py:39-42`)
    탓에 리스크 감소가 아니라 체결 0건 전략(VCP)으로 예산 이동(둘 다 메인 세션 코드 대조 확인). 카드 ①
    자동화 대상 = **A 레벨 0(적용 후보 큐 — 파싱·PUT 조립·게이트 사전판정만 자동, 적용은 사람이 항목
    단위 체크)** 권고 / ② weight 자동 적용 **전면 제외** 권고 / ③ `auto_apply_enabled` **OFF 유지 + 주간
    전용 신규 경로·토글** 권고. 착수 여부·레벨 = 카드.
...
- **20:30 주간 자문 루틴** — 09-08 05:44 에 http 예비 주소 제거 + `/health` 시정 완료. 09-08(화)
  정상 작동 여부 확인(TLS 2단계 가동 이후 최초 실행). **⚠️ 09-09 사용자 지시로 스케줄을
  화요일 → 목요일 20:30 KST 로 변경**(`trig_01H1TtfhP52CXKuyxwG2KnBW`, cron `30 11 * * 2`→
  `30 11 * * 4`, 다음 실행 09-10(목) 20:30 KST 확인됨). **다음 실행은 09-08(화)가 아니라
  09-10(목)이다** — 화요일에 자문이 안 와도 정상.
...
# ✅ 작업완료 — BFB·VCP 매수 전면 차단 (2026-08-25 확정 → 2026-08-28 사이클 228 게이트 전환으로 종결)

> **⚠️ 09-10 정정 — 이 배너는 더 이상 "최우선 작업 지시서"가 아니다.** 최신 진입점은 이 파일
> 맨 위 "🟡 대기열" 절이다(09-10 신뢰 점검 때 두 "여기부터 읽어라" 배너가 중복돼 있던 걸 확인).
> 아래 절 자체는 P0-1 이 사이클 228 로 종결됐다고 기록돼 있으나, **이 종결 기록이 이후 사이클
> (229~269)의 실제 배포 상태와 정확히 일치하는지는 이번 점검에서 한 줄씩 검증하지 않았다** —
> "커밋 대기"류 낡은 라벨이 문서 전체에서 17곳 발견된 전례가 있으므로, 다음 세션은 이 절
> 전체(257~305줄 대)를 git log 로 다시 대조해 P0-1 이후 잔여 항목(228-B `_effective_setup`
> 복원 등 "배포 대기"로 남은 하위 항목 포함)이 실제로 커밋·배포됐는지 검증하는 작업을 한 번
> 하고, 확인되면 이 배너 자체를 삭제하거나 완전히 접어라.
...
## ✅ cycle266 — 종목마스터 "일봉 (30일)" 탭 결함 3건 시정 (2026-09-07, 매매 행위 변경 0 · 커밋·배포 완료 `4d7a580`)
...
## ✅ cycle264 — 시가 `[7] STCK_OPRC` 스코프 **shadow 관측** (2026-09-06~07, **커밋·배포 완료 `38f2560`/`29b67ff`/`ff17470`, 09-10 라이브 관측 확인** · 행위 변경 0)
...
### 🔴 열린 채로 남는 것 — 근본 시정은 **cycle265**(후속 F-2) — **→ A안(REST 기준가) = cycle272 로 09-10 밤 시정 배포, B안(채널 분리) 금요일 실측 뒤**
...
## ✅ cycle263 — 일봉 적재 껍데기 봉 시정 (라) = (가) 신선도 게이트 + (다) 오늘봉 시각 필터 (2026-09-06, **커밋·배포 완료 `8b9b75a`/`0c878ec`**)
...
## ✅ 비터틀 랏 명목 ρ축 상한 — **cycle245 종결 (2026-09-04, `max_lot_ratio_mult` K_ρ=2.5 · 커밋·배포 완료 `d4fecb1`)**
...
## 🔐 cycle243 · API 인증 (2026-09-03 구현 완료 · 커밋·배포 완료 `233b663`) + 후속 F1~F9
...
> 기록 정정(09-06 00:1x): 폰트 TTF 3파일은 사용자가 스테이징해 둔 상태였고, 3부 보고서 커밋 `340a297`(09-05 22:0x)이 `git commit` 으로 스테이징 전체를 포함하면서 **먼저 들어갔다**(그 커밋이 CI + frontend 모드 배포 1회를 유발 — 백엔드 무접촉). cycle261 커밋 `ad2ffb4` 메시지의 "폰트 3개 커밋" 은 실제로는 `340a297` 이다. 기능 영향 없음.
 (2026-09-05 저녁, 사용자 지시 `dk-stock-design/handoff/` 절차대로 · frontend 전용 · src·8영역 diff 0 — **커밋·배포 완료 `ad2ffb4`**)
...
| **W3** | ✅ **생성 — 09-08 화 20:32 첫 실행(PR #20) → ⚠️ 09-09 사용자 지시로 화→목 이설**(상세는 이 파일 "📌 09-08 새벽 확인 필요" 절 참조). 주간 파라미터 자문 루틴 `trig_01H1TtfhP52CXKuyxwG2KnBW` — **현재 "auto_stock 주간 파라미터 자문 (목 20:30 KST)"(cron `30 11 * * 4`), 다음 실행 09-10(목) 20:30 KST**(09-09 API 실측 확인). 프롬프트 = `advisor_prompt_review_20260903.md` §F v2 원칙(목적함수·절대 금지 키·사람 결정·표본 N=10·전략당 최대 3키) 이식 + 09-09 개정(요일·`/health` 경로 정정), 산출 = PR `_workspace/domain_consult/weekly_advice_YYYY-MM-DD.md`(브랜치 `claude/weekly-advice-*`, 자동 적용 없음) + Notion 상위 https://app.notion.com/p/3d1438fa0d2681ae9502c26d1a2c4ebe . 첫 산출물 검토 후 프롬프트 §4/§5(봉인 키·결정 로그)를 코드 정본(`advisor_policy.py`, 사이클 A)으로 옮길지 결정 |
...
### ▶ G-8. VCP/BFB 브레이크이븐 승격 — ✅ **VCP 활성(08-18~)** · ✅ **BFB 활성(2026-09-10 21:45 사용자 결정 "BFB도 켜보자" → PUT 1.5, 라이브·DB 확인)** — 남은 것 = 발화 관찰(`[vcp_breakeven_promote]`·`[bfb_breakeven_promote]`)과 whipsaw 판정
...
## ✅ G0 · 1주 폴백 = 진입 시점 과잉 피라미딩 — **cycle242 ⓑ 종결 (2026-09-03, `max_lot_units` K=2.0, 커밋·배포 완료 `c350129`)**
...
## ✅ P1-6 · donchian 프리장 청산 보류 게이트에 **매일 ~30초 구멍** — **cycle238 종결 (2026-09-02, 시정안 A, 커밋·배포 완료 `4cbea99`)**
...
**신규 결함 2건**:
- **N1 — ✅ cycle235 시정 완료(08-29, 커밋·배포 완료 `f2b831f`)**. 근본 = handler 가 체결수량을
...
- **N2 — ✅ cycle236 시정 완료(08-29, 커밋·배포 완료 `a14bc98`)**. `is_sell_qty_exceeded`(APBK0400 ∧
...
### P1-4 · `silent_inactive` 오판 — 하루 16~24 접속키 낭비 (08-31 포렌식 확정 결함 ⓐ, MEDIUM) — ✅ cycle241 종결 (2026-09-02, 커밋·배포 완료 `6ac2bc2`)
...
### P1-7 · 5분 우선 재구독 핑퐁 (08-31 포렌식 확정 결함 ⓑ, MEDIUM) — ✅ cycle240 종결 (2026-09-02, 커밋·배포 완료 `140a79a`)
...
### ⚠️ 운영 결함 발견 — 일봉 16:00 적재가 매일 no-op (2026-09-06 실측) — **✅ cycle263 이 시정(커밋·배포 완료 `8b9b75a`)**
