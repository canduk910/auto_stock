...
| E3 cycle369 자문 | 관리종목 51·단기과열 59 보유 청산 설계 — [메모](domain_consult/cycle369_status_51_59_exit.md). 구현 = cycle369(아래 남은 일 A1 — 미커밋) | 메모 커밋 |
| 정리 | 세션 임시 워크트리 3개 제거(변경분 패치 백업) · 병합된 브랜치 3개 삭제 | — |
| VCP 터틀 전환 | 사용자 답 b 「VCP도 터틀로」 — 보유 0 에서 PUT `sizing_mode=turtle`·`risk_pct=0.01`(BFB 결정값에 맞춤, 09-26 10:22), API·DB 확인 | 설정 |
| 문서 정리 | architecture.md 7전략 확대(§4 그림·멤버 표, §8 매수 흐름 8.3~8.7) + Mermaid 때 찾은 불일치(컨테이너 3개·배포 모드·README kojiro·깨진 앵커) + `param_drift.py` 모듈 맵 + changelog cycle363 정정 부기 | `8d3faf5` |
| **cycle370** 장운영 상태 화면 | RealtimeHealth 종목별 행: VI 배지는 활성일 때만 · 거래정지 배지(58 포함, 행당 1개) · 종목상태 라벨(51·52·53·54·59) | `72e2572` frontend 선택 배포 |
...
**C. 판단 없이 할 수 있는 것** — ✅ 전부 끝(09-26 21:47 full 배포 `825c6dd`, 백엔드 12,640 passed · 실제 Postgres 342 passed)
- C1 문서: architecture §15.4·15.5 `order_engine.py` 앵커 → 심볼 · README momentum 진입 「09:00~15:20(자체 후보는 09:30 스캔부터)」 (`a642cec`). 남은 것 = §15.2 `order_engine.py` 앵커 3곳 · 다른 파일 앵커(`token.py`·`handler.py`·`scheduler.py`) 밀림
- C2 테스트: 부팅 테스트 60초 초과 원인 = 공용 토큰 누수로 테스트가 **실제 KIS 모의 서버**로 나감 → `tests/conftest.py` 외부 네트워크 차단(루프백만, `real_network` 마커) · `test_cycle64` 순서 의존 = 가격필터 메모리 fallback 누수 → 초기화 fixture (`d319355`)
- C3·C5 = **cycle371** (`e0b4b30`): 과거 날짜 번들 프로세스 키 3개를 그날 저장값으로 · K watcher 동시호가 창에서 보유(HIGH)는 재구독 유지(LOW 만 skip — cycle216 과 같은 규칙, 대가 = 그 창에 보유 종목 재등록 SEND 가 상한 안에서 다시 생김) · 51~54·59 만 걸린 종목도 화면 행
- C4 = **cycle372** 피라미딩 S0 과거 재현(5년 보관소, `825c6dd`): 표본 ① 7,162 ② 8,023(문턱 1,500 충족) · C* 사다리 Δ **−0.086R, 95% [−0.112, −0.060]** · 2021~2024 매년 음수 → 🔴 **S0 문턱 불통과 = 3단계(실제 피라미딩) 문이 닫힌다.** 문턱 재설계 항목은 불필요해졌다(표본 부족 해소 + 원 문턱이 판정을 냈다). 메모 `domain_consult/cycle372_s0_archive_replay.md` — **사용자 보고 대상**(피라미딩 설계 D-1~D-5 후속 판단)
- cycle368 후속 중 남은 것: 한 메시지 여러 레코드(미관측, 8영역 — 실측 증거가 생기면)

**D. 원래 목록 뒤쪽** — 계획 [`red/bundle_D_plan.md`](red/bundle_D_plan.md) · 실측 [`domain_consult/cycle373_bundleD_S2_measurements.md`](domain_consult/cycle373_bundleD_S2_measurements.md)(09-26, 읽기 전용)
- 방침: **09-28 전에는 매매 동작 변경을 더 넣지 않는다**(cycle364 S1·368·369·371 의 첫 실측을 흐리지 않기 위해). D 의 동작 변경은 전부 항목별 자문 + 사용자 결정 뒤
- ✅ S1 = J-1 문서 정정(`cancel_remaining` 이 잔량을 회수한다는 거짓 약속 제거 — 호출자 0, 잔량은 거래소가 정리). `order_engine.py:3089-3090` 주석의 같은 약속은 다음 승인된 order_engine 사이클에서
- ✅ S2 실측 결과가 카드 전제를 바꿨다:
  - **⑨A 매수 대사** — 남는 PENDING 은 GTP 자동취소가 아니라 **접수 뒤 거래소 거부**였다(437730 momentum 09-15, `rjct_qty=1`, WARNING 로그 0줄). 우리 GTP 매수 자동취소 0건. 하루 주문행 최대 24(100행 절단 없음), `ODNO` 단건 조회 가능 → 설계는 「주문번호별 양의 증거로만 해제」. 가치는 작다(슬롯·금액이 21:30 까지 묶이는 기회비용). 🔴 **거래소 거부가 로그에 안 남는 관측 공백**이 더 급하다 → **cycle374(09-27, 사용자 결정 「안되어있다면 바로 해야해」)** 가 접수 전문 기록을 넣었다: INFO `[order_notice]` + 거부면 WARNING `[order_rejected_notice]`(system_logs). 기록만, 상태 변경 0. 커밋·배포는 메인 세션 결정
  - **⑨B 거래소 자동취소 통보** — 15:30·20:00 취소 프레임 표본 0(그 시각 열린 주문이 없었음). 같은 주문의 두 번째 프레임은 취소 신호가 아니다(004990 SOR 분할 체결). 장중 실측 + 사용자 MTS 1주 시험 주문 필요 → **대기**. 관측 채널은 cycle374 `[order_notice]`(`rctf=`·`rfus=`·`acpt=` 칸)로 갖춰졌다 — 남은 것은 표본(취소 프레임 1건)뿐
  - **J-3** — 인용 표본 257720 은 사실 전량 체결(2/2). 부분체결 후 취소 행은 장부에 0건 → 전제 불성립(코드 결함 자체는 실재, 발생 0)
  - **⑦ 부팅 복구 momentum 폴백** — F1(잔고 보완) 30일 2건 모두 LTV 로 바르게 귀속, momentum 폴백 발화 0 · F2(미체결 매수 복구) 15회 부팅 모두 복구 0 → 우선순위 낮음
- **09-27 사용자 답**
  - ① 거래소 거부 기록 = 09-14 에 정해 놓고 미구현이었다 → ✅ **cycle374** `b060631`: 체결통보 채널의 접수·정정·취소·거부 통보를 `[order_notice]`(INFO)·`[order_rejected_notice]`(WARNING, system_logs)로 기록(개인정보·원문 제외, 상태 변경 없음). ⑨B 판별 필드 실측이 이제 쌓인다
  - ② ⑨A = 설명함. 🔴 결정 남음: 같은 날 재진입 허용 여부(momentum·VB·LTV 는 「오늘 샀음」 표시가 없다)
  - ③ B7 = **분할 매도 허용** — 단, 부분 매도 뒤 남은 수량을 실시간으로 추적해 잔여분 추가 매도·손절이 가능해야 한다(= B7 본 수정: 부분 청산 체결 때 `pos.quantity` 차감·저장, 전량 판정은 보유 수량 기준). 8영역 · 자문 선행 · **09-28 실측 뒤 다음 주말**. 그 전까지 수동 부분 매도는 피한다
  - ④ ⑨B = 설명함. cycle374 기록 며칠치로 접수·취소·거부 판별 규칙을 정한 뒤 설계
  - ⑤ B4-6 = 설명함(매도 함수 리팩터링 6단계, 프리장 지정가 변환 구간의 코드 해시 고정 때문에 결정 필요). 권고 = 건너뛰기 — 🔴 결정 남음
  - ⑥ 피라미딩 재검토 = **cycle375** 자문 `domain_consult/cycle375_pyramid_incremental_betting.md`: 깡토 점진적 베팅은 「다음 종목 크기」 규칙이고 같은 종목 추가(불타기)는 3R 뒤 새 신호 때 체감 매수 · 4전략 × 깡토식 변형 4 + 쿠션 조건 80구성 → **세 기간 모두 유의하게 이기는 구성 0** · 효율이 보인 유일한 자리 = VCP 청산을 느슨하게 + 불타기(+0.14R, 상위 1% 폭등주 빼면 −0.05R — 효과 대부분이 청산 변경) · 시장 유닛(코스피 60일선)은 방향만 + · 곁 정정: cycle351 「강세 1년 +0.046」 표본 855 중 157 이 ETF(빼면 음수). 🔴 결정 남음 3개: 같은 종목 피라미딩 검토 종료 + VCP 청산 규약 자문을 따로 열지 · 시장 유닛 관찰 항목 · ETF 정정 기록
- 🔴 **사용자 결정 대기**(계획 §결정): ⑦ 동작 변경 여부 · ⑨A `scheduler.py` 수정과 같은 날 재진입 정책 · B7 임시 가드(수동 부분 매도 경고/거부) · ⑨B MTS 시험 주문(`handler.py` 관측은 cycle374 로 해결) · B4-6 동결 구간 재핀 · 257720 DB 정정(가격 50원 차)
- 남은 순서(승인 뒤): J-1 코드 주석 → ⑦ 관측 로그 → ⑦-F2 → ⑨A → ⑦-F1 → 카드 E+J-4 → B7+J-2 → B4-3 → B4-5 → ⑨B → B4-4(폐기 권고)·B4-6·B4-7(기본 꺼짐)
...
| 장중 | cycle374 `[order_notice]` | 주문마다 1줄(SOR 분할이면 2줄). 거부는 `[order_rejected_notice]` WARNING — `rfus=` 값이 명세(0/1)와 같은지 첫 실측 |
| 09:00:30~ 300초 | cycle369 청산 패스 | 보유 12종목 조회. 지정 보유가 없으면 발사 0(09-26 조회 기준 해당 0). 발사 시 `[status_exit_fire]` WARNING(system_logs) |
| 첫 주문 뒤 | cycle374 `[order_notice]` (배포된 경우) | 우리 주문마다 접수 1행(system_logs). 행에 HTS ID·계좌번호·계좌명이 **없음**. 거부 주문이 있으면 `[order_rejected_notice]` WARNING. `qty=`·`price=` 가 접수 전문에서 무엇을 싣는지 기록 |
...
## 지금 상태 (2026-09-22 00:5x, 야간 자율 구간 — 사이클 6개 전부 배포 완료)

사용자 지시 = **07:00까지 진행, 승인은 권고대로. MACD 적용까지. 아침에 보고서.**

### 이 구간에 착지한 것 (EC2 마커 `8acbfbc`)

| 사이클 | 무엇 | 배포 |
|---|---|---|
| cycle336 | 넥스트트레이드 거래가능 여부를 **매수 평가에서 제거**(전 전략). 사이클 156 Q0 **원복** | ✅ `596eb91` |
| cycle337 | 거래내역 목록에 **AI 매수평가 점수 배지**(팝업 없이) | ✅ `596eb91` |
| cycle338 | 표 **24곳**을 창 크기에 맞춰 가로·세로 스크롤(`ScrollPane`) | ✅ `0b5306f` |
| cycle339 | 잔고에 종목별 **손절가·목표가**(+ `engine_idle` 구분) | ✅ `0b5306f` |
| cycle340 | 대순환 **MACD 962종목 실측** + 관측기(**배선 대기**) | ✅ `8acbfbc` |
| cycle341 | 인덱스 테스트 CI 타임아웃 + **마크가 조용히 덮이던 것** + 재발 덫 | ✅ `8acbfbc` |

🔴 **09-22 09:00 이 cycle336 아래 맞는 첫 거래일이다.** 매수 평가 대상이 **81→158 종목(+95%)**
늘어난다(09-21 09:30 실측 `[tick_buy_gate] stamped_no_feed=77 stamped_feed=81`). **동시 상한은
1원도 안 바뀐다**(슬롯 19·예산 2,253,881원) — 바뀌는 것은 후보 밀도와 슬롯 회전 속도다.
...
### 결정 완료 (2026-09-24 사용자 「권고대로」)

1. **MACD 관측기 배선** = **그대로 둔다**(cycle344 유지). 09-22 13줄 · 09-23 14줄로 작동 확인
...
| ⑤ | F3 LTV 상한가 트레일링(자문 선행) | ✅ 09-25 D5 ① 채택 → cycle352 `74aba2d` 배포(15:20 1회 점검). 아래는 결정 전 기록 —  실손실은 이미 −3.5% 에서 멈추고, 당일 트레일링은 평균 수익을 깎는다(건당 평균 — 폭 7% −2.1%p · 10% −1.5%p · 15% −0.9~0%p). 권고 ① **15:20 1회 점검**(상한가 모드인데 임계 아래면 종가 매도 — 평균 −0.07%p, −5% 이하 손실 6→3건). 선택지 ① / ② 15% 트레일링 / ③ 둘 다 / ④ 현행 + 문서 정정. 자문 [cycle352](domain_consult/cycle352_ltv_limit_up_trailing.md) |
| ⑥ | F2 스캔 단계 주가 상한(자문 선행) | ✅ 09-25 D6 = 메모 저장(`c52a53f`, `domain_consult/cycle353_scan_price_cap.md`) · shadow 안 함 · VB·LTV `max_lot_ratio_mult` 1.0 PUT 적용. 아래는 결정 전 기록 —  자문 결론: VB·LTV 에만 주가 상한 「설계 랏 ÷ N」(momentum 은 기록만). N=1 권고(N≥2 면 VB 성격이 바뀐다). shadow 선행(`DEFAULT_PARAMS` 새 키 = 승인 대상). 코드 없는 임시안 = VB·LTV `max_lot_ratio_mult` 1.0 PUT. ⚠️ 자문 메모는 자동 모드 안전 검사에 막혀 **파일로 저장되지 않았다**(세션 기록에만 있다) — 저장 여부 사용자 결정 |
...
| ⑦-2 | **F-A** BFB `position_ratio` 매수가 다음 날 부팅 재도출(`_rederive_entry_atr`)로 ATR 손절 경로(−7% 받침선)로 넘어간다. 100840 −7.1% 사례 — 메인 세션이 코드·운영 DB 로 확인 | ✅ cycle355 `6074c58` 배포(09-25 08:34). 재도출을 「현재 설정 turtle」 일 때만. 메모 `domain_consult/cycle355_bfb_entry_atr_gate.md` |
| ⑦-3 | **F-B** `H0UNMKO0` 파서 한 칸 밀림 → 가짜 VI · 보유 종목 재구독 skip(`src/api/market_operation.py` + 8영역 `src/realtime/handler.py`) | 조사 ✅ cycle359 `2b15b11` · 수정 = 09-25 저녁 결정(아래 절 ②) |
| ⑦-4 | **F-C** `stock_master_daily.change_rate` 전 행 0(매매 코드는 이 칸을 안 읽는다) | 앞으로 계산 ✅ cycle365 `98526e6` · 과거 행 백필 = 09-25 저녁 승인(아래 절 ⑤) |
...
| ⑪ | N1 `[no_feed_held]` 판정 시각 · N2 청산 사유 표기(`TRAILING_STOP` 오표기) | N1 ✅ cycle357 `e8c8f31`(KRX 연속체결 창에서만 판정). N2 → cycle367 §4.3 이 「8영역 동반」 을 반증(`risk.py` 는 `!= Signal.NONE` 만 본다) → P2 카드 4-1 (나) 세 이름 한 번에, 09-25 저녁 승인(아래 절 ④) |
| ⑫ | B4 3~7단계 · B7 · J-1~J-4 | 대기 |
| ⑬ | README.md · docs/architecture.md 의 ASCII 구성도를 Mermaid 로(그림 블록만, bash/env·트리·로그는 유지, `mmdc` 문법 검증) | ✅ `78b35bf` |

### 🔴 2026-09-25 사용자 결정 (09-25 아침 보고서 결정 카드 · 추가 작업 제안 답)

순서 = ⑬ Mermaid 먼저 → 아래를 순차로.

| 카드 | 결정 |
|---|---|
| D1 F-A | **배포 진행**. 그 뒤 **BFB 를 터틀식 관리로 전환**. 터틀식이 아닌 전략은 momentum·VB·LTV 3개뿐(사용자 전제 — ⚠️ 운영 실측은 VCP 도 `position_ratio` 이고 터틀은 donchian·kojiro 둘뿐이다. VCP 전환은 별도 결정). ⚠️ F-A 수정(재도출을 「현재 설정 turtle」 일 때만)과 BFB 터틀 전환이 겹치면 **기존 비중 매수 보유분이 재시작 때 ATR 표식을 받는다** — 전환 전 대책 필요(보유분 처리 또는 랏별 사이징 기록) |
| D2 F-B | 조사 진행 |
...
| D8 카드 E | 지금 준비 |
| D9 청산 사유 표기 | 청산 규약 자문(P2)에 합쳐 새로 추가 |
| P1 | = D2 |
| P2 | = D9 (BFB 청산 규약 자문: F-A·달력일 시간 청산·N2·주간 D3/D4) |
| P3 | = D5 로 흡수(주간 자문 D2 는 F3 에 합침) |
| P4 | `change_rate` 앞으로 계산 — 진행 |
| P5 | a `trailing_stop_rate` 도움말 적용 범위 — 진행 · b `PARAM_RANGES max_scan_stocks` 를 **운영값(4000)대로** |
| P6 | 리포트 정확도 관측 — **휴장일 표시 포함** 진행 |
...
### 🔴 2026-09-25 저녁 결정 (21:5x, 낮 보고서 카드 ①~⑤ 답) + 후속 순서

근거 = [낮 보고서 원문](reports/2026-09-25_day_decisions_and_cycles_352_366.md) §6·§9. 한 건씩 차례대로, 건별 커밋+배포(장외 창 — 주말·휴장일 종일, 평일 15:30~16:00·21:35~07:45).

| 카드 | 사용자 답 | 해석·조건 |
|---|---|---|
| ① BFB | 「BFB 매수 멈춰, 전환 때 risk_pct 0.01」 · 설정은 **지금** 바꾸고 **09-28 08:55 재확인** | `entry_end` 09:04 PUT(청산·손절 무접촉, `enabled` 무접촉) → 보유 3종목이 다 나가면(달력 규칙상 늦어도 10-01) 장외에 `{"sizing_mode":"turtle","risk_pct":0.01,"entry_end":"14:30"}` PUT. 전환 전에 `param_catalog.py` `sizing_mode` 도움말 정정 |
| ② F-B | 「칸 위치는 바로 잡아. 거래정지 판정은 58만 봐, 대신 관리종목51과 단기과열59가 발동했는데 해당 종목을 보유하고 있으면 시장가로 청산하게 하고 재구독 중지등으로 신규매수를 중단하자. VI발동 후 풀림을 못받았으면 (나-10분뒤 자동해제)」 | ⚠️ 권고와 다름. (a) 칸 밀림 **전부** — `market_operation.py` + 8영역 `handler.py:746` mkop 칸까지 (b) 거래정지 skip = `{58}` (c) 🔴 **신설**: 51·59 발동 + 보유 → 시장가 청산, 그 종목 신규 매수 차단 — 매매 행위 변경이라 **domain-consult 선행**, 크리티컬 분기가 나오면 그 부분만 보류 (d) VI = 마지막 `Y` 10분 뒤 자동 해제 |
| ③ D3 | 「D3 카드는 모두 권고대로」 | A1 카드 1~5 (가) · `scheduler.py` 승인. S1 → 첫 거래일 실측 → S2 |
| ④ P2 | 「P2 나머지 모두 권고대로」 | 카드2 (가) kojiro 동형·래칫 없음·부분봉 가드 · 카드3 (가) 영업일 `>5` — 🔴 **BFB 보유가 다 나간 뒤 배포** · 카드4-1 (나) `TIME_EXIT`+`TAKE_PROFIT`+`TREND_EXIT` · 4-2 (가) 표기만 · 5-1 (나) 주간 D3 보류 · 5-2 (가) 주간 D4 적용 안 함. 코드 사이클 = 10-03~04 장외 |
| ⑤ P4 | 「백필하자」 | 과거 367,178행 `change_rate` DB UPDATE — 장외, 백업 표 먼저, 건수·표본 대조 |
| A·B·C | A 위 답대로 · B 판단대로 진행 · C 가능한 부분 마무리 | B·C 목록 = 낮 보고서 §7 |

**후속 순서** (09-28 07:45 전에 끝내야 하는 것부터)
...
| E3 | F-B 51·59 보유 청산 + 신규 매수 차단 — 자문 → 명세 → TDD | 09-28 07:45 전 목표 | 자문 ✅(`domain_consult/cycle369_status_51_59_exit.md`) · 구현 cycle369 라운드 1~3 끝(미커밋 — 맨 위 「남은 일」 A1) |
...
09-24~26 휴장 → 다음 거래일 09-28(월). 배포한 행위 변경은 그 전에 되돌릴 수 있다.

R1 매수 빈도 = 그대로 · R3 비중 재배분 = 보류(VB 실적 부진) · 목표가 1전략 = D-1~D-5 뒤 재논의.

---

## 배경 (2026-09-21 04:1x, 자율 구간 — 사이클 9개 전부 배포 완료)
...
| cycle335 | **매수 축 「접수 후 경계」 부재**(2B). 자문 선행 + 돌연변이 11종 + 관문 G1~G5 | ✅ `171da97` |

🔴 **오늘 09:00 이 cycle327~335 아홉 사이클 전부 아래 맞는 첫 거래일이다.** 배포·기동은 실측했지만
**매매 중 실제 동작은 아직 아무도 못 봤다** — 효과는 실측 0 이다. 두 가지를 섞지 않는다.

⚠️ **「3회 발사」는 돌연변이 재현이지 운영 사고가 아니다**(2026-09-21 사실 대조에서 드러났다).
경계를 `UniqueViolationError` 로 좁히자 `test_sell_does_not_refire_on_generic_insert_error` 가
시도 1/2/3 을 재현한 것이고, **매도 부분체결 실적은 5개월간 0건**이다. 정본 2곳(루트
`CLAUDE.md` · `src/engine/CLAUDE.md`)의 「실측」 표기를 고쳤다 — 이 결함은 아직 실현되지
않았고, 그것이 시정을 미룰 근거는 아니다(부분체결은 유동성이 얇아지는 순간에 몰린다).
...
- **남은 것** = A~F · B4 3~7단계 · B6(LTV 상한가 당일 트레일링 부재 — **아래 09-20 절의 `F3` 와 같은 항목**이다) · B7(`_handle_sell_fill` 전량 판정이 `ordered_qty` 기준) · J-1~J-4(자문 `cycle329`·`cycle332` 의 별건 카드) · D1(F1 배분 재조정 — **월 07:45 부팅 후**)
  > ⚠️ **라벨 주의** — `F3` 는 이 문서에서 세 가지를 가리킨다: ① 09-20 절의 **LTV 상한가 트레일링**(= 여기 `B6`) ② 09-19 이전 절의 파라미터 화이트리스트 ③ 자문 별건 카드. 인용할 때는 **`B6` 와 `J-1~J-4`** 를 쓴다. 이 줄에서 `F3/F4` 로 뭉뚱그리던 표기를 걷어냈다(2026-09-21 사실 대조).
...
## 🔴 trading_base 검토 결과 — 사용자 가설의 판정

> 사용자: "변동성돌파·모멘텀·롱테일이 **유닛개념을 쓰지 않아서** 리스크관리가 제대로 되지 않는 것 같아"

**결론은 맞고 원인은 틀렸다.** 자문의 표현 —
「저울이 고장난 게 아니라, 재려는 물건이 저울 눈금 한 칸보다 작다」

### 맞는 부분 (운영 DB 실측 2026-09-20)
고정%손절이면 1회 위험 = 명목 × 손절률이므로 명목 분산이 곧 위험 분산이다.

| 전략 | n | 평균 명목 | 최소 | 최대 | 최대/최소 |
|---|---|---|---|---|---|
...
→ 루트 `CLAUDE.md` 의 「고정%손절이면 `position_ratio` 가 이미 리스크 균등」은 **실측에서 성립하지 않는다**.

### 틀린 부분 — 유닛을 **이미 쓰는** 전략도 같다
donchian **76%가 1주**(최대 5주) · kojiro **52%가 1주**(최대 5주).
정수 절삭은 유닛식에도 그대로 있다.

### 진짜 원인 — 설계 랏이 한 주 값 언저리다

🔴 **순자산 정정(2026-09-20)** — 초판은 5,032,151원을 썼는데 **이중 계산**이었다.
`daily_performance` 에는 전략 7행 외에 **`strategy='total'` 행이 따로** 있고 그것을 함께 더했다.
실제 = **2,516,078원**(09-18, `total` 행). 각 전략 `total_asset ÷ weight` 가 전부 2,516,07x 로 수렴한다.
**사용자 입금(250만 → 500만)은 2026-09-20 에 방금 이뤄졌다** — 그전까지 500만이었던 적이 없다.

`q` = 설계 랏 ÷ **중앙 주가**(명목 아님 — 다주 랏이 섞이면 갈라진다).

| 전략 | 중앙 주가 | 설계 랏(250만) | **q(~9/19)** | 설계 랏(500만) | **q(9/20~)** | 현 비중 |
|---|---|---|---|---|---|---|
| volatility_breakout | 118,750 | 44,031 | **0.37** | 88,063 | 0.74 | 0.05 |
| long_tail_volatility | 45,400 | 25,161 | **0.55** | 50,322 | 1.11 | 0.05 |
| donchian_swing | 55,900 | 75,482 | 1.35 | 150,965 | 2.70 | 0.15 |
| momentum | 14,490 | 31,451 | 2.17 | 62,902 | 4.34 | 0.05 |
| bull_flag_breakout | 33,350 | 94,353 | 2.83 | 188,706 | 5.66 | 0.15 |
| kojiro | 33,050 | 167,068 | 5.06 | 334,135 | 10.11 | 0.40 |

🔴 **과거 체결은 전부 왼쪽(250만) 구간이다.** VB 설계 랏 44,031원 < 중앙 주가 118,750원 =
**반 주도 못 샀다.** 189배 분산이 그래서 나왔고, 초판이 보고한 것보다 **두 배 심했다.**

`q<1` 이면 주 분기가 항상 0 → 폴백 → 최종 명목 = **그 종목 주가 그 자체**.
ρ축(cycle245)은 **위만** 막는다(최근 30일 위반 0건 = 정상 작동).

### 🔴 입금이 바꾸는 것 (초판은 「영향 없다」고 했고 그것이 틀렸다)

입금은 **모든 `q` 를 정확히 2배로** 만든다 = 명목 오차 상한(`1/q`)을 절반으로 줄인다.
Σ 필요 비중(q=5)도 **1.94 → 0.97** 로 떨어져 **가능 범위로 들어온다.**
즉 사용자의 입금이 이 문제의 절반을 직접 해결했다.

⚠️ **반영 시점** — 예산은 `_boot()` 의 `allocate_funds(순자산 × cash_usage_ratio)` 에서 갱신된다.
DB `daily_performance` 도 21:30 정산에서 갱신된다. 즉 **월요일 07:45 부팅 후에야** 새 예산이 선다.
F1 집행은 **그 뒤**에 해야 비중 계산의 분모가 맞는다.

### 🔴 1순위 위험 = 성과 귀인의 붕괴
전략 손익 = 엣지 × **랜덤 가중치**. 가중치가 189배 흩어지면 134·71·56건 표본으로
엣지를 **구조적으로 못 잰다**. → **D4 와 정면 충돌** — 사용자가 "표본이 더 필요"해서
자동조정을 미뤘는데 **그 표본이 오염돼 있다**.
⚠️ 균형: 전 기간 VB 최대 위험 = 527,000×3% = **순자산의 0.31%**. 응급은 아니다.

### 원전을 잘못 읽을 뻔한 것
「유닛 1 미만이면 거래하지 않는다」는 **다음 줄에서 「미니 상품으로 바꾸면」으로 이어진다**
(`자금관리.md:194-195`). 규칙의 정신은 "거래하지 마라"가 아니라 **"계약 크기를 계좌에 맞춰라"**.
주식에서는 **더 싼 종목**이다. 매수 관문에 두면 거래 중단이지만
**스캔 단계에 두면 「다른 종목 매매」** 가 된다 — 같은 규칙도 자리가 다르면 결과가 정반대다.
q=3 기준 주가 상한 = VB **29,354원** · LTV **16,774원** · momentum **20,967원**.

### 결정 대기 4건
라벨은 **F1~F4** 다 — 09-19 절에 다른 E1~E3(백테스트 MCP 끄기·VCP 관찰·만료 cron)이
살아 있어 「E1 진행」이 어느 것인지 갈린다.

- **F1** 배분 재조정 — kojiro 0.40(필요 0.197) ↔ VB 0.05(필요 0.236). 축이 셋(비중·슬롯·종목)이고
  **어느 축을 쓸지가 결정 내용**이다. 🔴 **전략을 끄지 않는다**(아래 금기)
- **F2** 스캔 단계 주가 상한 — 원전 규칙의 올바른 번역(매수 관문이 아니라 후보 단계)
- **F3** LTV 상한가 모드 −5% + 트레일링 부재(자문 최우선). 반납 폭은 이론 상한 26%,
  `min_prdy_rate 5.0` 반영 시 **23% 안팎**
- **F4** 랏 관측을 어디에 붙일까 — 그래서 미집행

### 🔴 F4 의 전제 정정 (초판이 틀렸다)

초판은 「게이트가 켜진 전략이 7개 중 3개」라고 적었다. **틀렸다.**
`llm_gate_mode` 코드 기본값이 **7전략 전부 `"shadow"`** 이고 `_load_strategy_config` 가
**키별 덮어쓰기**(`scheduler.py:425-427`)라 DB 에 키가 없으면 코드 기본값이 산다.
DB 의 「키 없음」을 「꺼짐」으로 읽은 오독이다.

**실효 = 7/7 shadow.** 그런데 —

| 구간 | 매수 | 평가기록 |
|---|---|---|
| 09-10 ~ 09-13 | 7건 | 0건 (기능 배포 전) |
| 09-14 ~ 09-18 | 13건 | **8건 = 62%** |

전부 켜져 있는데 **38%가 샌다.** 결론(표본이 얇다)은 유지되지만 이유가 다르고,
**그 결손 자체가 새 조사 항목**이다.
...
2. **시정 수단이 싸졌다** — 잔존 상태(`pending_buys` 등 + `trade_history` PENDING) 정리에
   ...
   `scheduler.py` 무접촉.
3. 🔴 **아직 못 잰 것 = 접수와 취소를 가르는 필드.** 그 프레임의 `fields[5]`(RCTF_CLS 정정구분)·
   `fields[12]`(RFUS_YN)·`fields[14]`(ACPT_YN 1:주문접수 2:확인 3:취소) **실제 값을 모른다**.
   이걸 확정하지 않고 분기를 짜면 접수 통보를 취소로 오인해 **살아 있는 주문의 상태를 지운다**.
   관측 채널 = **cycle374 `[order_notice]`**(09-27 — 세 칸을 `rctf=`·`rfus=`·`acpt=` 로 싣는다).
   남은 것은 GTP 미체결 1건 표본이다. 🔴 옛 계획 「DEBUG 줄에 `fields[:20]` 을 싣는다」 는 쓰지
   않는다 — `[0]` HTS ID·`[1]` 계좌번호·`[17]` 계좌명이 로그에 섞인다.
