...
### 진짜 원인 — 설계 랏이 한 주 값 언저리다 (순자산 **5,032,151원**)

`q` = 설계 랏 ÷ **그 전략이 사는 종목의 중앙 주가**.
⚠️ 분모는 **주가**이지 명목이 아니다 — 다주 랏이 섞이면 갈라진다(VB 주가 118,750 vs 명목 150,600).
초판에서 이 둘을 혼동해 VB 를 0.6주로 적었다. 실제는 **0.74주**다.

| 전략 | 중앙 주가 | 설계 랏 | **q** | 현 비중 | **필요 비중(N=5)** |
|---|---|---|---|---|---|
| volatility_breakout | 118,750 | 88,063 | **0.74** | 0.05 | **0.236** (4.7배 부족) |
| long_tail_volatility | 45,400 | 50,322 | **1.11** | 0.05 | 0.180 (3.6배 부족) |
| momentum | 14,490 | 62,902 | 4.34 | 0.05 | 0.058 |
| donchian_swing | 55,900 | 150,965 | 2.70 | 0.15 | 0.167 |
| kojiro | 33,050 | 334,135 | **10.11** | 0.40 | 0.197 (**2배 과다**) |
| bull_flag_breakout | 33,350 | 188,706 | 5.66 | 0.15 | 0.133 |

`q<1` 이면 주 분기가 항상 0 → **100% 폴백 경로** → 최종 명목 = **그 종목 주가 그 자체**.
ρ축(cycle245)은 **위만** 막는다(최근 30일 위반 0건 = 정상 작동).

### 🔴 자본 부족이 아니라 **배분** 문제다 (초판 정정)

필요 비중 `w_i = N × P_i × m_i / A`. **Σ 필요 비중 = 0.970**(체결 있는 6전략, N=5)
→ **현재 자본으로 충분하다.** N=5 필요 자본 **4,882,300원 < 순자산 5,032,151원**.

⚠️ 초판의 「필요 자본 12,778,700원 · 슬롯 28→10」은 **7전략 중 4개에 주가 10만원을 가정**해
계산한 값이었다. 실측으로 바꾸니 뒤집혔다. 어긋난 것은 자본이 아니라 배분이다 —
**kojiro 는 랏이 10주라 정밀도가 남는데 0.40 을 쥐고, VB 는 1주도 못 사는데 0.05 뿐이다.**
