...
1. **MACD 관측기 배선** — 관측 전용(매매 무변경)이나 `kojiro.py` 를 12개 가드가 핀한다. kojiro 는
   실매매·최대 비중·보유 4종목. (제안: 예)
2. **MACD 진입 규약 변경(국면1→6)** — 🔴 **반대**. 실측이 개선을 못 보였고 원전이 요구한
   **수량 3단 분할이 없다**. 상세 = [`domain_consult/cycle340_kojiro_macd.md`](domain_consult/cycle340_kojiro_macd.md)
3. **수량 3단 분할 + 피라미딩** — 순자산 500만 도달로 해상도 임계 교차. 별도 사이클.
4. **UI 스크롤 제외 2곳** — 자문 파라미터 표(표 안 툴팁이 잘린다) · 매크로 지표표(이득 없음). (제안: 그대로)
5. **목표가는 `눌림목 돌파` 하나뿐**(부분 익절 트리거). 나머지 6전략은 개념 자체가 없어 `—` 가 정상. (제안: 유지)
