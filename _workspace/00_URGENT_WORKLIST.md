...
- ✅ S2 실측 결과가 카드 전제를 바꿨다:
  - **⑨A 매수 대사** — 남는 PENDING 은 GTP 자동취소가 아니라 **접수 뒤 거래소 거부**였다(437730 momentum 09-15, `rjct_qty=1`, WARNING 로그 0줄). 우리 GTP 매수 자동취소 0건. 하루 주문행 최대 24(100행 절단 없음), `ODNO` 단건 조회 가능 → 설계는 「주문번호별 양의 증거로만 해제」. 가치는 작다(슬롯·금액이 21:30 까지 묶이는 기회비용). 🔴 **거래소 거부가 로그에 안 남는 관측 공백**이 더 급하다
  - **⑨B 거래소 자동취소 통보** — 15:30·20:00 취소 프레임 표본 0(그 시각 열린 주문이 없었음). 같은 주문의 두 번째 프레임은 취소 신호가 아니다(004990 SOR 분할 체결). 장중 실측 + 사용자 MTS 1주 시험 주문 필요 → **대기**
...
- 🔴 **사용자 결정 대기**(계획 §결정): ⑦ 동작 변경 여부 · ⑨A `scheduler.py` 수정과 같은 날 재진입 정책 · B7 임시 가드(수동 부분 매도 경고/거부) · ⑨B `handler.py` 관측 1줄 + MTS 시험 주문 · B4-6 동결 구간 재핀 · 257720 DB 정정(가격 50원 차)
