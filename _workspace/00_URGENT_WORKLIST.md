...
### P1-5 · VB 15:30 종가 매수 발사 + APBK3013 [단일가매매] 미분류 → 매일 15:30 WS 재연결 (2026-08-27 신규 확정)
...
- **🚨 단순 키워드 추가 금지 — 거부가 우연히 가드 역할 중** (2026-08-06 NXT 프리장과 동일 부류):
  근본 원인은 MAIN 흡수 마진(15:30~15:39:59)에 보드가 MAIN 유지라 **VB 가 15:30 종가 체결
  틱으로 매수를 발사**하는 것. 15:20 강제청산(`_force_clear_main_only`)이 이미 지나간 뒤라,
  키워드를 추가해 지정가 폴백이 **체결되면 VB 가 오버나잇 보유**(OVERNIGHT 금지 정책 위반)가
  된다. 지금은 KIS 거부가 그걸 막아주고 있다. 시정은 (a) 매수 시간 가드(15:20 또는 15:30 컷)
  선행 + (b) 그 위에 분류 보강(예외 전파만 차단, 폴백 아닌 정중한 skip) 순서여야 하며
  **domain-consult 선행 필수**. VB 시간 가드는 `volatility_breakout.py`(8영역 아님) /
  분류기는 `balance.py`(8영역 아님) / `execute_buy` 는 8영역.
