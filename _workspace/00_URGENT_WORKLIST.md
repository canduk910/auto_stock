...
| **D2** | 매크로 첫 화면 107초 → ⓒ 둘 다 | ✅ cycle314 — nginx `/api/macro/` `proxy_read_timeout 60s→120s`(두 템플릿) + **신규** `tools/ops/macro_prewarm.sh`(00:05 KST). 예산 가드를 실측 107초 기준으로 교체. crontab 등록은 배포 후 |
| **D3** | SK케미칼 트레일링 미집행 | ✅ ⓐ 현행 유지 — **결함 아님**(아래 절). 코드 조치 없음 |
| **D4** | 외부 백테스트 서버 | ⚠️ 요청 결과 = **복구 대상이 없다**(아래 절). 전제가 바뀌어 사용자 결정으로 되돌림 |
| **D5** | 압축 로그 | ✅ cycle313 — EC2 `.gz` 37개 삭제(6.4G→**1.9G**, 루트 여유 5.8G) + `src/main.py` `_LOG_BACKUP_DAYS=20` + **무압축 금기** 주석. `_SRC_TREE_DIGEST` 재핀 |
...
샹들리에 = `57,000 − 2.5 × 2,480.16` = **50,799.59원**.
...
- 🟣 **레짐 쪽은 이미 대체재가 있다** — 우리가 만든 `macro/` 컨테이너(`/macro` 5섹션)가 같은
  판정을 로컬에서 낸다. `macro_lite` 이관 2·3단계(값 비교 → `market_regime.py` 주소 전환)를
  마치면 dkstock.cloud 재구축 자체가 불필요해진다. 철거 사실이 그 이관의 시급성을 올렸다.
- 별개로 매일 8건씩 쌓이는 `skipped "YAML DSL 미지원"` 은 서버가 아니라 우리 쪽
  미구현(Phase 4-bis 로컬 어댑터)이다 — 서버를 재구축해도 이 8건은 그대로 skipped.
