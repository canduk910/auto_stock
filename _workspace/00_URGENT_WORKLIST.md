...
1. **`[kojiro_gap_observe]` 판독 — 반드시 09-10(목) 전에 로그를 뽑아 둘 것.**
   이 마커는 INFO(참고) 등급이라 `system_logs` 보존이 **2일**이고 20:10 리포트 집계에도
   안 들어간다. 09-08(화) 장중 발화분이 09-10 을 넘기면 **영구히 사라진다.**
   - ⚠️ **경로 B(`risk.on_tick` 틱 경로)는 이 마커로 오염 여부를 못 잰다** — `risk.py` 495행이
     `ticker_prices[ticker]["open_price"]` 를 덮은 뒤 648행이 **같은 값**을 `check_buy_signal`
     에 넘겨 `ws_cmp=ws_eq` 가 산술적으로 보장된다(자기참조, cycle264 `used_src=rest→delta_bp=0`
     함정과 동형). 경로 B 판정은 `stock_master_daily` 와의 **오프라인 조인만이 유일한 경로**다.
     명세 = `_workspace/specs/cycle268_kojiro_gap_observe.md` §5.
   - 화 09:30~10:30 채널 프로브(아래)와 같은 날이니 로그 덤프를 한 번에 처리한다.
2. **09-08(화) 09:30~10:30 채널 프로브** — 사용자 승인 완료("내일 오전에 한시간동안 재보자").
   `POST/GET/DELETE /api/realtime/channel-probe`(cycle253, 다크런치, 기본 OFF) 로 3종목을
   KRX 단독 채널(`H0STCNT0`)에 구독시켜 `[open_scope_observe]` 의 `oprc_hour` 를 본다.
   - **판정**: `09xxxx` 나오면 발의(프리장=NXT 채널·정규장=KRX 채널 분리)에 긍정 증거,
     `08xxxx` 나오면 **그 자리에서 발의 기각**.
   - ⚠️ **비대칭이다** — 08시대가 나오면 결정적이지만, 09시대가 나와도 **3종목 표본일 뿐**이고
     "전 종목 긍정" 이 아니다(자문 정본이 명시). 전면 판정은 실제 배포 후 D+1 에나 가능.
   - 자문 정본 = `_workspace/consult/2026-09-07_channel_split_by_session.md`.
   - 프로브 종료 후 반드시 `DELETE` 로 구독 해제(다크런치 원복).
3. **오늘 07:45 부팅 확인** — 포지션 12종목 복구 확인 · cycle263(일봉 껍데기 시정) 두 prepare
   수렴이 09-07 에 이미 확인됐으므로 이번엔 **회귀 확인**(같은 패턴 유지)만.
4. **오늘 20:20 루틴** — 모델을 `claude-opus-5` 로 바꿔 뒀다(cycle264/263 크레딧 소진 재발 방지).
   첫 시도가 재시도 없이 성공하는지 확인.

## 🔵 결정 대기 — 사용자 답 필요

1. **TLS 2단계 가동 시점** — 09-07 21:0x 1차 시도가 검증 스크립트의 정규식 결함(CRLF 미인식)
   으로 실패 → 자동 원복(사이트 무손상). 시정(cycle267)은 배포됐고 운영 GNU grep 으로 재검증
   완료. **언제든 켤 수 있다** — 시점만 정하면 됨. 절차 = `_workspace/specs/tls_stage2_activation_runbook_0907.md`.
...
3. **cycle265(시가 오염 근본 시정 — VB·LTV 목표가 기준을 KRX 조회값으로 교체) 시기·방향 리포트**
   — 사용자가 요청함("웹소켓TR 변경검토가 끝나면 결과를 받아서 재검토 한 뒤에"). 채널 프로브(위 1)
   결과가 나와야 값어치가 있다 — 프로브가 08시대면 cycle265 만 남고, 09시대면 채널 분리와의
   순서(자문 결론 = **cycle265 를 먼저**, 영구 보완재)를 리포트에 담는다.
4. **F-3(고지로 갭 판정 오염 실제 시정 — `risk.py:646` skip 목록에 kojiro 추가)** — 관측(cycle268)
   만 배포됐고 시정은 **행위 변경**이라 별도 승인 필요. 오늘 관측 데이터가 쌓이면 착수 여부 판단.
5. **F-3b(momentum·LTV 익일청산 갭률 동일 오염, 방향은 반대로 손실 확대가 아니라 기대수익 훼손)**
   — 워크리스트에 등재만 하고 미착수. 착수 여부 결정 안 됨.
6. **오늘 판독 문서 2건 커밋 여부** — `_workspace/reports/2026-09-07_evening_four_defects.md` 가
   워킹트리에 미커밋 상태(git 미추적). 문서 전용이라 서버·매매 영향 0.
