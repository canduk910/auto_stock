...
## Red 가드 (사이클 103 strategy.py dead code 폐기 패턴 100% 답습)
`tests/unit/ast/test_cycle167_ast_no_dead_market_cap_funcs.py`:
- HIGH-1.1~1.3: 3 함수 모듈 def 영구 부재 (정규식 `^def name`)
- HIGH-1.4: 정의/호출 패턴 `name(` 잔존 0건 (주석 외 코드 영역)
- HIGH-2: 보존 영역 변경 0 (`_is_master_blocked_for_entry` / `apply_master_block_filter` / `MIN_MARKET_CAP`)
