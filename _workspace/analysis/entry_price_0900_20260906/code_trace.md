...
> ⚠️ **정정(2026-09-06 자문 §1.1, cycle264 동봉)** — 뒷문장의 "**0/55 확정**" 은 **표시 버그**를
> 읽은 것이다. 09:00:05 경로는 실제로 VB **51/65**(09-03)·**46/55**(09-04)를 확정한다
> (`[breakout_open_confirm]` 이 `get_targets_status()` 의 `active ∩ tradable_boards` 마스킹을
> 통과한 값을 세고 있었다 — 상세는 §2.5(a) 앞 정정 주석). **앞문장(기준가가 통합 `[7]` 이다)은
> 그대로 유효하다** — 09:00:05 경로의 WS 폴링이 읽는 것도 **같은 오염된 `[7]`** 이기 때문이다.
> 바뀌는 것은 "누가 확정했나" 뿐이고(대부분 09:00:05 WS 폴링, 나머지 9~14 종목 + 09:30 이후가
> 인라인 폴백), "무엇으로 확정했나" 는 어느 쪽이든 오염된 `[7]` 이다.
...
| 09:00:11~13 | `[breakout_open_confirm]` — **VB confirmed=0** (실측 09-03/09-04). ⚠️ **정정**: 이 0 은 표시 버그이고 실제 확정은 **51/65·46/55**(§2.5(a) 앞 주석) — 즉 이 시각에 이미 대부분의 목표가가 (오염된 `[7]` 로) 굳어 있다 | system_logs |
...
- **부작용 (중요)**
  ...
  - 대신 **`_open_confirmed` 도 동결**된다(`:866-869` 미도달). 09:00:05 경로가 실측상
    ...
    > ⚠️ **정정** — "09:00:05 경로가 실측상 0/55" 는 표시 버그였다(실제 51/65·46/55). 다만
    > **결론은 그대로다**: 그 경로의 WS 폴링이 확정에 쓰는 값도 같은 오염된 `[7]` 이라,
    > 보류가 있든 없든 목표가 기준가는 오염된 채로 굳는다.
...
- `TIME_KRX_OPEN_CONFIRM = time(9, 0, 5)`(`scheduler.py:59`)를 늦추는 것은 **보류가 아니다** —
  ...
  폴백 기회만 더 뒤로 밀린다.
  > ⚠️ **정정** — "이미 0/55 무동작" 은 표시 버그였다(실제 51/65·46/55). **늦추지 말라는 결론은
  > 오히려 강화된다** — 이 경로는 실제로 대부분의 종목을 확정하고 있고, 늦추면 그 확정이 통째로
  > 인라인 폴백으로 밀린다.
...
- 단 ①은 **원인 시정이 아니다**. 원인 시정 후보는 (가) 인라인 폴백을 지우고 09:00:05 REST(KRX)
  ...
  > ⚠️ **정정(2026-09-06 자문)** — (다)의 전제 "REST 폴백이 침묵 실패한다" 는 **반증**됐다.
  > 폴백은 동작하며 다만 **WS 폴링이 먼저 이겨 차례가 오지 않는다** ⇒ 후보는 "폴백을 살리기" 가
  > 아니라 **"main 보드 기준가에서 REST 를 WS 보다 우선시키기"** 로 재서술된다.
  > 그리고 자문 §0 은 **(나)의 fail-closed 변형(`[7]` 을 0 으로 강등)을 명시적으로 기각**했다 —
  > `[7]` 은 익일청산 갭 판정·LTV 프리장 목표가·kojiro 갭스킵/시가아래 가드·momentum 익일청산
  > 갭률까지 **여섯 소비처**가 공유해 0 이면 가드가 조용히 꺼지거나 강제 청산으로 뒤집힌다.
  > 채택된 방향 = `[7]` 은 그대로 두고 `board="main"` **목표가 기준가만** KRX REST 로 교체.
  > **cycle264(2026-09-06~07)는 이 중 어느 것도 하지 않고 관측만 심었다** — 시정은 cycle265.
...
4. 09:00:05 경로가 왜 0/55 인지(구독 슬롯 부족 vs REST 폴백 실패)는 이 추적으로 확정하지 못했다.
   ...
   > ⚠️ **정정(2026-09-06 자문 §1.1)** — 이 열린 질문은 **해소됐다.** 답은 "구독 슬롯" 도
   > "REST 폴백 실패" 도 아니고 **관측기 자신의 표시 버그**였다(`get_targets_status()` 보드
   > 마스킹). 실제 확정은 VB 51/65·46/55. `scheduler.py:1668` 의 `logger.debug` 침묵은 여전히
   > 별개의 한계로 남는다(cycle264 `[open_source_compare]` 의 `reason=rest_zero|rest_error` 가
   > 09:05 시점에 한해 그 침묵을 대신 깬다).
