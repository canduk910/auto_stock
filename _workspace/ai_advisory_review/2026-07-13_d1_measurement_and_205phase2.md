...
> 배경: 사이클 203(iscd 과차단 제거)·204(투자주의/투자유의 해제)·205-1(list_by_filter DB-side 필터)이
> 2026-07-11(토) 배포됨 → **월요일 07:50 _boot부터 실반영**. phase-2는 D+1 실측 후 진행(사용자 결정).
...
## 3. 사이클 206 (독립, 205와 무관)
BFB flag_retracement 완화 (0.382→? domain 자문 + `tools/measure_bfb_pole_flag.py` 실측). 폴 급등폭 대비 조정폭 현실화.
