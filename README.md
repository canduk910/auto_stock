...
| 08:55 | 사전 구독 — 돌파 전략(VB/MB) 스캔 종목 + 보유 포지션 (09:00 시가 즉시 수신용). 유니버스가 비어있으면 prepare 재실행 |
| 09:00:00 | 모멘텀 익일 청산(백그라운드 60초 안정화) + 돌파 시가 확정(0.5초 폴링) 동시 시작 |
| 09:00:05 | 변동성돌파(VB) / 모멘텀 브레이크아웃(MB) 매매 시작 (시가 확정 직후) |
| 09:30 | 모멘텀(상한가) 종목 스캔 시작, 매수 감시 |
| 15:20 | 신규 매수 중단, VB + MB(상한가 미도달 종목) 전량 강제 청산 |
