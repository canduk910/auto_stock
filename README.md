...
| 매도 좀비 폭주 차단 (사이클 52 / B-1) | `is_market_closed_rejection` (APBK0918 / KIOK0320 장운영시간 외) 거부 시 `OrderEngine._market_closed_blocked[ticker] = _compute_next_market_open_kst(now)` 등록 — ticker별 TTL (다음 KST 09:00 만료). `execute_sell` 진입 게이트에서 TTL 미경과면 KIS 호출 없이 조용히 skip (INFO `[market_closed_blocked]` 1줄/ticker/일 cap). `risk.on_tick` / `_swing_rest_poll_loop` 의 외부 재호출로 인한 KIS REST 폭주 차단 (2026-05-31 운영 진단: 10분간 500+건 폭주 후 도입). `OrderEngine.reset_daily_state()` 가 dict + cap set 일괄 clear, `scheduler._reset_daily_state` 위임 호출. positions 보존 + NXT 사후 보강은 기존 동작 그대로 |
