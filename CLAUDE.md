...
- **NXT 매도 거부 좀비 차단** — `is_market_closed_rejection` (APBK0918 + 장운영시간 외) 이면 `execute_sell` 이 positions(메모리/DB) 보존 + 재시도 중단. `is_insufficient_quantity` / `is_insufficient_cash` 와 분리. **사이클 B-1 (2026-06-01) 진입 차단까지 보장**: `_market_closed_blocked: dict[str, datetime]` ticker별 TTL (다음 KST 09:00 만료) — TTL 미경과 시 `execute_sell` 진입 게이트에서 KIS 호출 없이 skip (INFO 1줄/ticker/일 cap). `_reset_daily_state` 동행 clear (`OrderEngine.reset_daily_state()` 캡슐화, scheduler 위임)
