...
- **VB 당일 15:20 일괄매도 정책 (2026-05-15, 결함 D)**: VB `DEFAULT_TRADABLE_BOARDS = ("pre_nxt", "main")` — POST_NXT 제외, NXT 애프터 비활성. `_force_clear_main_only` 가 keeps_post_nxt=False 분기로 15:20 일괄 청산. 19:50 청산 코드가 누락된 상태에서 POST_NXT 활성 시 OVERNIGHT 자연 보유 결함(005930 5/13~5/15 멀티데이 보유 후 손절 사고) 회복. DB `strategy_config` 동시 갱신 필요. 회귀 가드: `tests/unit/engine/test_vb_force_clear_at_15_20.py`
