...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-13 | 사이클 126 — 종목마스터 UI/데이터 결함 4건 통합 시정 (LOW~MEDIUM). 사용자 보고 = (1) 전체현황 카드 수치 1000 cap (실측 2,697), (2) 목록 데이터 부족 (모달 대비 4 키 미노출), (3) NXT/거래정지/관리종목 모두 False (KRX 1차 폴백 하드코딩 + KIS lazy 미발화), (4) 일봉 0건 (사이클 122 task 미발화 추정). **사용자 결정**: Q1=count="exact" 디코딩 + Q2=4 컬럼 (현재가/전일대비/시가총액/거래대금) + Q3=자동 task + 수동 POST + Q4=진단 로그 강화 + 수동 POST. **시정 영역**: (백엔드 5 파일) `src/db/stock_master.py::get_stats()` count="exact" 분리 + .range(0, 9999) raw 쿼리 + count 실패 graceful fallback / `src/engine/scanner.py::_stock_master_daily_load_once()` 진단 강화 + db_write_failures 카운터 분리 / `src/engine/scanner.py::_stock_master_basics_refresh_once()` 신규 (KIS CTPF1002R 매스 보강, 50ms sleep, graceful) / `src/engine/scheduler.py` 시간 상수 TIME_STOCK_MASTER_BASICS_REFRESH=16:10 KST + `_stock_master_basics_refresh_task_loop` 신규 (사이클 122 답습) + task_attrs 3곳 갱신 + `_stock_master_basics_refresh_task` create_task / `src/engine/stock_master_basics_metrics.py` 신규 (record/flush 페어링, 사이클 122 답습) / `src/routes/stock_master.py` POST `/basics/refresh` + POST `/daily/refresh` 신규 + 별도 asyncio.Lock 2 + 409 Conflict + _POST_ONLY_PATHS 갱신. (프론트엔드 5 파일) `types/stock-master.ts` BasicsRefreshResult + DailyRefreshResult 2 interface / `api/stock-master.ts` refreshBasicsNow + refreshDailyNow 2 함수 / `pages/StockMaster.tsx` 리스트 4 컬럼 추가 (stck_prpr / prdy_vrss / hts_avls / acml_tr_pbmn, 모바일 hidden md:table-cell) + formatMarketCap + formatTradeAmount + formatPriceChange 신규 + 2 신규 버튼 (기본정보 새로고침 emerald + 일봉 새로고침 amber) + 2 신규 토스트 + 2 신규 useMutation retry:false / `test/handlers.ts` MSW 2 mock / `e2e/fixtures/api-mocks.ts` Playwright LIFO 정합 2 라우트. **회귀 가드**: 16 케이스 (DB 2 + engine 5 + AST 5 + routes 4). 사이클 84 L-2 화이트리스트 갱신 (3 라우트) + 사이클 124 G-AST1 _WHITELIST 갱신 + 사이클 79 G-AST2 expected_members 14종 갱신 + 사이클 124 mock chain `.range` 갱신. **검증**: 백엔드 unit 풀 2,250 PASS + 140 xfailed + 1 xpassed (회귀 0) / 프론트 289/289 PASS / 사이클 126 격리 16 PASS × 3회 (flakiness 0, 0.50s ±0.04s) / TypeScript build ✓. **매매 안전성 무영향 확정**: scanner 단계 매수 진입 전 영역만 변경 + risk.on_tick / order_engine / realtime/ / auth/ 변경 0 + 매도/익일청산/15:20 강제청산 hot path 무관 + 사이클 38 명문화 영속. **영속 의무 매트릭스**: 사이클 17 KIS LMS chain (50ms sleep) / 38 명문화 / 79 G-AST2 task_attrs / 84 L-2 POST 화이트리스트 / 88 G-REJECT graceful / 90 _refresh_universe_lock 패턴 / 107 CTPF1002R + FHKST01010100 merge / 122 일봉 task 100% 답습 / 124 UI 동기화 8단계 절차. **운영 효과 (push + EC2 자동 배포 후)**: (a) 전체현황 카드 = 정확한 ticker 수 표시 (1000 → 2697+) / (b) 목록 4 컬럼 추가로 시장 동향 즉시 파악 / (c) 매일 16:10 KST KIS CTPF1002R 매스 보강 자동 발화 (~4.5분 소요, 2,697 종목 × 100ms) → NXT/정지/관리종목 정상화 / (d) 운영자 UI "기본정보/일봉 새로고침" 버튼 즉시 trigger 가능 / (e) [stock_master_daily_load_begin] + [stock_master_basics_refresh_begin] 진단 로그 + db_write_failures 카운터 가시화. **사이클 49→126 누적 81 사이클 + hotfix 17 영역**. **사이클 127+ 인계**: 익일 (2026-06-14 토) Supabase MCP READ-ONLY 운영 실측 (basics 16:10 발화 + 일봉 16:00 발화 + DB nxt_true/halted_true/admin_true 비율 + stock_master_daily total_rows > 0 + UI 카드 8 정상 표시). | `src/db/stock_master.py` (+18L count 분리) / `src/engine/scanner.py` (+~130L _stock_master_basics_refresh_once + 진단 로그 강화) / `src/engine/scheduler.py` (+~85L task_loop + 시간 상수 + create_task + task_attrs 3곳) / `src/engine/stock_master_basics_metrics.py` 신규 (~80L) / `src/routes/stock_master.py` (+~90L 2 POST + 2 Lock) / `frontend/src/types/stock-master.ts` (+25L 2 interface) / `frontend/src/api/stock-master.ts` (+24L 2 함수) / `frontend/src/pages/StockMaster.tsx` (+~150L 4 컬럼 + 2 버튼 + 2 useMutation + 3 포맷터) / `frontend/src/test/handlers.ts` (+~25L MSW 2 mock) / `e2e/fixtures/api-mocks.ts` (+~40L Playwright LIFO 2 라우트) / `tests/unit/db/test_cycle126_get_stats_exact_count.py` 신규 (2 케이스) / `tests/unit/engine/test_cycle126_basics_refresh_once.py` 신규 (5 케이스) / `tests/unit/ast/test_cycle126_ast_basics_refresh_persistence.py` 신규 (5 케이스) / `tests/unit/routes/test_cycle126_basics_daily_routes.py` 신규 (4 케이스) / `tests/unit/ast/test_cycle84_ast_no_update_route.py` (_ALLOWED_POST_ROUTES 3 라우트 갱신) / `tests/unit/ast/test_cycle124_ast_endpoint.py` (_WHITELIST 3 라우트 갱신) / `tests/unit/engine/test_scheduler_stop_zombie_tasks.py` (expected_members 14종 갱신) / `tests/unit/db/test_cycle124_stock_master_stats_extended.py` (mock chain `.range` 갱신) / `_workspace/red/cycle126_stock_master_quad_fix.md` Red 명세 / `CLAUDE.md` 루트 사이클 126 행 | 사용자 보고 4결함 동시 시정 — Phase 1 진단으로 단일 사이클 통합 발주. team-leader Phase 1 진단 결정적 발견 = (1) PostgREST 1000행 한도 + (2) 모달 vs 리스트 데이터 비대칭 + (3) KRX 1차 폴백 하드코딩 + (4) 사이클 122 코드 완벽 but 미발화. 사용자 결정 전수 채택 (Q1~Q4 4 의제). domain-expert 자문 생략 (LOW~MEDIUM 위험, scanner 단계 매수 진입 전 한정, 사이클 38 명문화 영속). 사이클 90 _refresh_universe_lock 패턴 100% 답습 + 사이클 122 일봉 task 100% 답습. 사이클 84 L-2 + 사이클 124 G-AST1 화이트리스트 영속 갱신 (POST 1→3). 사이클 79 G-AST2 expected_members 14종 영속 갱신. 매매 안전성 무영향 확정. 사이클 127+ 후속: D+1 운영 실측 의무 (Supabase MCP READ-ONLY).
