...
- **주문이 나간 뒤의 실패로 재발사 금지 (cycle327)** — `place_order` 가 성공하면 그 주문은 이미 거래소에 있다. 그 뒤의 어떤 예외(UNIQUE 위반·DB 타임아웃·페일오버)도 **「발사 실패」가 아니므로** 재시도 루프로 되돌리지 않는다. 매도 경로는 접수 후 구간을 자체 `try/except` 로 닫고 `[sell_post_send_error]` 를 남긴 뒤 종료하며, 재시도 직전에는 `strategy.state.positions.get(ticker)` 를 **재조회**해 포지션이 사라졌으면 `[sell_position_gone]` 후 중단한다. 루프 밖 지역 참조 `pos` 는 체결 뒤에도 옛 수량을 들고 있어 **이미 판 것을 다시 판다**(실측 3회 발사). 조용한 흡수 금지 — 두 마커가 유일한 관측 채널이다. 피라미딩 착수 시 그 재조회가 수량의 유일한 진실원이 된다. 매도 두 경로는 **같은 헬퍼**(`_persist_sell_pending_after_send`)가 그 경계를 닫는다 — 🔴 그 `except Exception` 을 **좁히지 않는다**(cycle328 격리 실측: 좁히면 기존 행위 회귀 13건이 **전부 초록**인 채로, 폴백+`KisApiError` 에서 **접수 성공한 주문을 장부에 「거부」로 적고 익일청산 큐에 넣어 다음날 09:00 에 다시 판다**. `positions` 는 안 변해 화면·DB 로는 정상으로 보인다). 행위 테스트로는 못 막고 구조 가드 `test_cycle328_sell_pending_helper.py::test_g328_3b` 가 유일한 방어다
