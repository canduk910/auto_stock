...
- **VB 익일 청산 대상 미포함 (2026-05-15, 잔여 결함)**: `_execute_next_day_clear()` 가 `momentum + long_tail_volatility` 만 처리, VB 는 대상 아님. 따라서 어떤 이유로든 15:20 청산이 누락되면 (예: 시세 미수신, 시장가 거부, 코드 결함) VB 보유 종목은 -3% 손절선까지 자연 도달 외에는 청산 트리거가 없어 영구 보유 가능. 안전망으로 VB 도 `_execute_next_day_clear` 대상에 추가 권고 — 사용자 결정 대기. 2026-05-15 LG전자(066570) 14:33 매수가 본 결함으로 청산 트리거 부재 상태.
