...
**⏳ 사이클 227 (2026-08-25) — Stage 0 구현 완료·미커밋**: 배관(handler `fields[13]` →
`on_tick(*, acml_vol=)` → `tick_volume` 신규 모듈)과 `would_pass` 관측 로그만 넣었고
**게이트는 byte 불변 = 매수는 여전히 차단**(의도된 상태 — 관측 우선 배포 전략, 사용자 결정).
1~2영업일 관측 후 게이트 전환은 별도 사이클: 주 3건↑ would_pass = 전환 / 0건 = P1-3 래치
선행 / 일 10건↑ = 재조사. ⚠️ 회귀 테스트 3파일의 `acml_vol` 손주입은 Stage 0 에선 유지
(주석 마커 부착) — **게이트 전환 사이클에서 tick_volume 주입으로 의미 전환 의무.**
상세·전환 의무 = `_workspace/00_URGENT_WORKLIST.md` P0-1 시정 현황.
